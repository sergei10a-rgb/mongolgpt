...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve mongolgpt route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "mongolgpt")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "mongolgpt")).toBe("openai")
    expect(statProvider("big-pickle", "", "mongolgpt")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "mongolgpt"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep mongolgpt as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "mongolgpt"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "mongolgpt"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep mongolgpt or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "mongolgpt"), country: "US" })).toMatchObject([
