import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@mongolgpt/plugin/tui"
...
import type { useMongolGPTKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useMongolGPTKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getMongolGPTModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getMongolGPTModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
    ...
    slots: {
      register() {
        throw new Error("slots.register үйлдлийг зөвхөн нэмэлтийн орчинд ашиглах боломжтой")
      },
    },
    plugins: {
      list() {
      ...
      async install() {
        return {
          ok: false,
          message: "plugins.install үйлдлийг зөвхөн нэмэлтийн орчинд ашиглах боломжтой",
        }
      },
    },
    theme: {
      ...
      async install(_jsonPath) {
        throw new Error("theme.install үйлдлийг зөвхөн нэмэлтийн орчинд ашиглах боломжтой")
      },
      mode() {
