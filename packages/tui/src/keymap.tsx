...
export const MONGOLGPT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const MONGOLGPT_MODE_KEY = "mongolgpt.mode"

export const MongolGPTKeymapProvider = KeymapProvider
export const useMongolGPTKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type MongolGPTModeStack = ReturnType<typeof createMongolGPTModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, MongolGPTModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createMongolGPTModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(MONGOLGPT_MODE_KEY, MONGOLGPT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(MONGOLGPT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(MONGOLGPT_MODE_KEY, stack.at(-1)?.mode ?? MONGOLGPT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? MONGOLGPT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(MONGOLGPT_MODE_KEY, undefined)
...
export function useMongolGPTModeStack() {
  return getMongolGPTModeStack(useMongolGPTKeymap())
}
...
export function registerMongolGPTKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createMongolGPTModeStack(keymap)
