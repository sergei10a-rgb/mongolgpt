  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "mongolgpt")
        draft.active = typeof active === "string" ? active : "mongolgpt"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "mongolgpt"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "mongolgpt")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.mongolgpt, store.mode)
    })
