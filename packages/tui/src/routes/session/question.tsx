...
import type { QuestionAnswer, QuestionRequest } from "@mongolgpt/sdk/v2"
...
export function QuestionPrompt(props: { request: QuestionRequest; directory?: string }) {
  ...
  const modeStack = useMongolGPTModeStack()
  ...
  useBindings(() => ({
    ...
    commands: [
      {
        name: "prompt.clear",
        title: "Хариултын засварыг цэвэрлэх",
        category: "Асуулт",
        run() {
      ...
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        <Show when={!single()}>
          <box flexDirection="row" gap={1} paddingLeft={1}>
            ...
            >
              <text fg={confirm() ? selectedForeground(theme, theme.accent) : theme.textMuted}>Батлах</text>
            </box>
          </box>
        </Show>
        ...
        <Show when={!confirm()}>
          <box paddingLeft={1} gap={1}>
            <box>
              <text fg={theme.text}>
                {question()?.question}
                {multi() ? " (таарах бүхнийг сонгоно уу)" : ""}
              </text>
            </box>
            <box>
              <For each={options()}>
                  ...
                  <box flexDirection="row">
                    ...
                    <box backgroundColor={other() ? theme.backgroundElement : undefined}>
                      <text fg={other() ? theme.secondary : customPicked() ? theme.success : theme.text}>
                        {multi() ? `[${customPicked() ? "✓" : " "}] Өөрийн хариултыг бичих` : "Өөрийн хариултыг бичих"}
                      </text>
                    </box>
                  ...
                  <Show when={store.editing}>
                    <box paddingLeft={3}>
                      <textarea
                        ...
                        initialValue={input()}
                        placeholder="Өөрийн хариултыг бичих"
