...
import type { AssistantMessage } from "@mongolgpt/sdk/v2"
import { Locale } from "../../util/locale"
import { useTerminalDimensions } from "@opentui/solid"
import { useCommandShortcut, useMongolGPTKeymap } from "../../keymap"
...
export function SubagentFooter() {
  ...
  const subagentInfo = createMemo(() => {
    const s = session()
    if (!s) return { label: "Дэд агент", index: 0, total: 0 }
    const agentMatch = s.title.match(/@(\w+) subagent/)
    const label = agentMatch ? Locale.titlecase(agentMatch[1]) : "Дэд агент"
