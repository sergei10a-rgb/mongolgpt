...
import { MONGOLGPT_BASE_MODE, useBindings, useCommandShortcut, useMongolGPTKeymap } from "../../keymap"
...
export function Session() {
  ...
  createEffect(() => {
    ...
    void (async () => {
      ...
      if (!result.data) {
        toast.show({
          message: `Сешн олдсонгүй: ${sessionID}`,
          variant: "error",
          duration: 5000,
        })
        navigate({ type: "home" })
  ...
  const keymap = useMongolGPTKeymap()
  ...
  function enterChild(sessionID: string) {
    ...
    if (status?.type === "retry") void DialogAlert.show(dialog, "Дахин оролдох алдаа", status.message)
  }
  ...
  const sessionCommandList = createMemo(() => [
    {
      title: session()?.share?.url ? "Хуваалцах холбоос хуулах" : "Сешн хуваалцах",
      value: "session.share",
      ...
      run: async () => {
        ...
        await sdk.client.session
          ...
          .catch((error) => {
            toast.show({
              message: error instanceof Error ? error.message : "Сешн хуваалцаж чадсангүй",
              variant: "error",
            })
          })
        dialog.clear()
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!selectedModel) {
          toast.show({
            variant: "warning",
            message: "Энэ сешнийг хураангуйлахын тулд провайдер холбоно уу",
            duration: 3000,
          })
          return
        }
    ...
    },
    {
      title: (() => {
        ...
        if (next === "hide") return "Бодолтыг хураах"
        return "Бодолтыг задлах"
      })(),
    ...
    {
      title: showDetails() ? "Хэрэгслийн дэлгэрэнгүйг нуух" : "Хэрэгслийн дэлгэрэнгүйг харуулах",
    ...
    {
      title: showGenericToolOutput() ? "Ерөнхий хэрэгслийн гаралтыг нуух" : "Ерөнхий хэрэгслийн гаралтыг харуулах",
    ...
    {
      title: "Хэрэглэгчийн сүүлийн мессеж рүү үсрэх",
    ...
    {
      title: "Туслахын сүүлийн мессеж хуулах",
      value: "messages.copy",
      category: "Сешн",
      run: () => {
        ...
        if (!lastAssistantMessage) {
          toast.show({ message: "Туслахын мессеж олдсонгүй", variant: "error" })
          dialog.clear()
          return
        }
        ...
        if (textParts.length === 0) {
          toast.show({ message: "Туслахын сүүлийн мессежид текст хэсэг олдсонгүй", variant: "error" })
          dialog.clear()
          return
        }
        ...
        if (!text) {
          toast.show({
            message: "Туслахын сүүлийн мессежид текст агуулга олдсонгүй",
            variant: "error",
          })
          dialog.clear()
          return
                        ...
                        {(function () {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <text fg={theme.textMuted}>{revert()!.reverted.length} мессеж буцаагдсан</text>
                                <text fg={theme.textMuted}>
                                  <span style={{ fg: theme.text }}>{redoShortcut()}</span> эсвэл /redo ашиглаж сэргээнэ
                                </text>
      ...
      <Show when={text()}>
        ...
        >
          ...
          >
                    ...
                    return (
                      <text fg={theme.text}>
                        <span style={{ bg: theme.secondary, fg: theme.background }}>
                          {directory ? ` ${Locale.ui.directory} ` : ` ${Locale.ui.file} `}
                        </span>
            ...
            >
              <text fg={theme.textMuted}>
                <span style={{ bg: color(), fg: queuedFg(), bold: true }}> ДАРААЛАЛД </span>
              </text>
            </Show>
          </box>
        </box>
        ...
        <box paddingTop={1} paddingLeft={3}>
          <text fg={theme.text}>
            ...
            >
              ...
              {backgroundShortcut()}
              <span style={{ fg: theme.textMuted }}> арын горимд шилжүүлэх</span>
            </Show>
          </text>
        </box>
      </Show>
          ...
          <box ref={(el: BoxRenderable) => alwaysSeparate.add(el)} paddingLeft={3}>
            <text marginTop={1}>
              ...
              <Show when={props.message.error?.name === "MessageAbortedError"}>
                <span style={{ fg: theme.textMuted }}> · тасалсан</span>
              </Show>
            </text>
          </box>
        </Match>
    ...
    <Switch>
      <Match when={!props.done}>
        <box flexDirection="row">
          <Spinner color={fg()}>{props.title ? "Бодож байна: " + props.title : "Бодож байна"}</Spinner>
        </box>
      </Match>
      <Match when={true}>
...
function GenericTool(props: ToolProps) {
  ...
  return (
    <Show
      ...
      fallback={
        <InlineTool icon="⚙" pending="Команд бичиж байна..." complete={true} part={props.part}>
      ...
      >
        <box gap={1}>
          ...
          <Show when={collapsed().overflow}>
            <text fg={theme.textMuted}>{expanded() ? "Хураахын тулд дарна уу" : "Задлахын тулд дарна уу"}</text>
          </Show>
        </box>
      </BlockTool>
    </Show>
  ...
  return (
    <Switch>
      <Match when={props.metadata.diagnostics !== undefined}>
        <BlockTool title={"# Бичсэн " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
      ...
      <Match when={true}>
        <InlineTool
          icon="←"
          pending="Бичихэд бэлдэж байна..."
          complete={stringValue(props.input.filePath)}
          part={props.part}
        >
          Бичих {pathFormatter.format(stringValue(props.input.filePath))}
        </InlineTool>
      </Match>
    </Switch>
  )
...
function Glob(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="Файлууд хайж байна..." complete={stringValue(props.input.pattern)} part={props.part}>
      Файл хайлт "{stringValue(props.input.pattern)}"{" "}
      <Show when={stringValue(props.input.path)}>{pathFormatter.format(stringValue(props.input.path))} дотор </Show>
      <Show when={numberValue(props.metadata.count)}>({numberValue(props.metadata.count)} тохирол)</Show>
    </InlineTool>
  )
}
...
function Read(props: ToolProps) {
  ...
  return (
    <>
      <InlineTool
        icon="→"
        pending="Файл уншиж байна..."
        complete={stringValue(props.input.filePath)}
        spinner={isRunning()}
        part={props.part}
      >
        Унших {pathFormatter.format(stringValue(props.input.filePath))} {input(props.input, ["filePath"])}
      </InlineTool>
      <For each={loaded()}>
        {(filepath) => (
          <box paddingLeft={3}>
            <text paddingLeft={3} fg={theme.textMuted}>
              ↳ Ачаалсан {pathFormatter.format(filepath)}
            </text>
          </box>
        )}
      </For>
...
function Grep(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="Агуулга хайж байна..." complete={stringValue(props.input.pattern)} part={props.part}>
      Агуулга хайлт "{stringValue(props.input.pattern)}"{" "}
      <Show when={stringValue(props.input.path)}>{pathFormatter.format(stringValue(props.input.path))} дотор </Show>
      <Show when={numberValue(props.metadata.matches)}>({numberValue(props.metadata.matches)} тохирол)</Show>
    </InlineTool>
  )
}
  ...
  return (
    <InlineTool
      ...
      onClick={() => {
        ...
        const status = retry()
        if (status) void DialogAlert.show(dialog, "Дахин оролдох алдаа", status.message)
      }}
    >
      {content()}
    </InlineTool>
...
export function formatSubagentTitle(agent: string, description: string, background: boolean) {
  return `${agent} даалгавар${background ? " (арын горим)" : ""} — ${description}`
}

export function formatSubagentRetry(attempt: number, message: string) {
  return `Дахин оролдож байна (оролдлого ${attempt}) · ${message}`
}
  ...
  return (
    <Switch>
      <Match when={stringValue(props.metadata.diff) !== undefined}>
        <BlockTool title={"← Зассан " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
          <box paddingLeft={1}>
            <diff
              diff={diffContent()}
              view={view()}
      ...
      <Match when={true}>
        <InlineTool
          icon="←"
          pending="Засварт бэлдэж байна..."
          complete={stringValue(props.input.filePath)}
          part={props.part}
        >
          Засах {pathFormatter.format(stringValue(props.input.filePath))}{" "}
...
function ApplyPatch(props: ToolProps) {
  ...
  function title(file: { type: string; relativePath: string; filePath: string; deletions: number }) {
    if (file.type === "delete") return "# Устгасан " + file.relativePath
    if (file.type === "add") return "# Үүсгэсэн " + file.relativePath
    if (file.type === "move") return "# Зөөсөн " + pathFormatter.format(file.filePath) + " → " + file.relativePath
    return "← Нөхсөн " + file.relativePath
  }
        ...
        <For each={files()}>
          {(file) => (
            <BlockTool title={title(file)} part={props.part}>
              <Show when={file.type !== "delete"} fallback={<text fg={theme.diffRemoved}>-{file.deletions} мөр</text>}>
...
function Question(props: ToolProps) {
  ...
  function format(answer?: ReadonlyArray<string>) {
    if (!answer?.length) return "(хариулт алга)"
    return answer.join(", ")
  }
    ...
    <Show when={errors().length}>
      <box>
        <For each={errors()}>
          {(diagnostic) => (
            <text fg={theme.error}>
              Алдаа [{diagnostic.range.start.line + 1}:{diagnostic.range.start.character + 1}] {diagnostic.message}
            </text>
          )}
        </For>
      </box>
