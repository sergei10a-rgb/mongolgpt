...
import type { TuiPlugin, TuiPluginApi, TuiRouteCurrent } from "@mongolgpt/plugin/tui"
import type { SnapshotFileDiff, VcsFileDiff } from "@mongolgpt/sdk/v2"
import {
  TextAttributes,
  type BorderSides,
  type BoxRenderable,
...
const PLAIN_TEXT_FILETYPE = "mongolgpt-plain-text"
      ...
      <PanelGroup axis="y" width="100%" height="100%">
        ...
        <Panel flexShrink={0} gap={2} paddingLeft={1} border="none">
          ...
          <Show when={nextFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>дараагийн файл</span>
              </text>
            )}
          </Show>
          ...
          <Show when={previousFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>өмнөх файл</span>
              </text>
            )}
          </Show>
          <Show when={switchSourceShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>эх сурвалж солих</span>
              </text>
            )}
          </Show>
          <Show when={markReviewedShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>хянасан гэж тэмдэглэх</span>
              </text>
            )}
          </Show>
          <Show when={helpShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>бүгд</span>
              </text>
            )}
          </Show>
        </Panel>
...
function DiffViewerHelpDialog() {
  ...
  const rows = [
    ...
    },
    {
      ...
      action: "Дараагийн файл",
      description: "Файлын модны дарааллаар дараагийн өөрчлөгдсөн файлыг сонгох",
    },
    {
      shortcut: useCommandShortcut("diff.previous_file"),
      action: "Өмнөх файл",
      description: "Файлын модны дарааллаар өмнөх өөрчлөгдсөн файлыг сонгох",
    },
    {
      shortcut: useCommandShortcut("diff.toggle_file_tree"),
      action: "Файлын мод асаах/унтраах",
      description: "Файлын модны хажуу самбарыг харуулах эсвэл нуух",
    },
    {
    ...
    },
    {
      ...
      action: "Бүх хавтсыг дэлгэх",
      description: "Файлын мод дахь бүх хавтсыг нээх",
    },
    {
      shortcut: useCommandShortcut("diff.mark_reviewed"),
      action: "Хянасан гэж тэмдэглэх",
      description: "Сонгосон файлын хянасан төлөвийг асаах/унтраах",
    },
  ]
  ...
  return (
    <box paddingLeft={2} paddingRight={2} paddingBottom={1} gap={1}>
      ...
      <box flexDirection="row">
        ...
        <text fg={theme.textMuted}>Тайлбар</text>
      </box>
      <For each={rows}>
        {(row) => (
          <box flexDirection="row">
