import type { TuiPlugin, TuiPluginApi, TuiPluginStatus } from "@mongolgpt/plugin/tui"
...
function state(api: TuiPluginApi, item: TuiPluginStatus) {
  if (!item.enabled) {
    return <span style={{ fg: api.theme.current.textMuted }}>идэвхгүй</span>
  }

  return (
    <span style={{ fg: item.active ? api.theme.current.success : api.theme.current.error }}>
      {item.active ? "ажиллаж байна" : "ажиллахгүй байна"}
    </span>
  )
}
        ...
        void props.api.plugins
          ...
          .then((out) => {
            ...
            return props.api.plugins.add(mod).then((ok) => {
              ...
              props.api.ui.toast({
                variant: "success",
                message: `${mod}-г одоогийн сешнд ачааллаа.`,
              })
              show(props.api)
            })
          })
...
function row(api: TuiPluginApi, item: TuiPluginStatus, width: number): DialogSelectOption<string> {
  return {
    title: item.id,
    value: item.id,
    category: item.source === "internal" ? "Дотоод" : "Гадаад",
