    ...
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box gap={1}>
        ...
        <textarea
          ...
          placeholder={props.placeholder ?? "Текст оруулах"}
        ...
        <Show when={props.busy}>
          <Spinner color={theme.textMuted}>{props.busyText ?? "Ажиллаж байна..."}</Spinner>
        </Show>
      </box>
      <box paddingBottom={1} gap={1} flexDirection="row">
        <Show when={!props.busy} fallback={<text fg={theme.textMuted}>боловсруулж байна...</text>}>
          <Show when={submitShortcut()}>
            <text fg={theme.text}>
              {submitShortcut()} <span style={{ fg: theme.textMuted }}>илгээх</span>
            </text>
          </Show>
        </Show>
      </box>
