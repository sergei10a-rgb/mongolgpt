...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@mongolgpt/sdk/v2"
        ...
        if (provider.type === "custom") {
          return {
            ...
            async onSelect() {
              ...
              return dialog.replace(() => <ApiMethod providerID={providerID} title="API түлхүүр" custom />)
            },
          }
        }
...
function AutoMethod(props: AutoMethodProps) {
      ...
      toast.show({
        ...
        message:
          "name" in result.error && result.error.name === "ProviderAuthOauthCallbackFailed"
            ? "OAuth зөвшөөрөл амжилтгүй боллоо. /connect командыг дахин оролдоно уу."
  ...
  return (
    <box paddingLeft={2} paddingRight={2} gap={1} paddingBottom={1}>
      ...
      <text fg={theme.textMuted}>Зөвшөөрөл хүлээж байна...</text>
      <text fg={theme.text}>
        c <span style={{ fg: theme.textMuted }}>хуулах</span>
      </text>
    </box>
  )
}
    ...
    <DialogPrompt
      ...
      description={() => (
        <box gap={1}>
          ...
          <Link href={props.authorization.url} fg={theme.primary} />
          <Show when={error()}>
            <text fg={theme.error}>Код буруу байна</text>
          </Show>
        </box>
      )}
    />
