...
export function DialogSessionList() {
    ...
    const warp = async (selection: WorkspaceSelection) => {
      const workspaceID = await (async () => {
        ...
        try {
          ...
          toast.show({
            title: "Ажлын орчин үүсгэж чадсангүй",
      ...
      <DialogSessionDeleteFailed
        ...
        onDelete={async () => {
          ...
          if (result.error) {
            toast.show({
              variant: "error",
              title: "Ажлын орчин устгаж чадсангүй",
  ...
  const quickSwitchFooterHints = createMemo(() => {
    ...
    return hint && local.session.slots().length > 0 ? [{ title: "солих", label: hint }] : []
  })
  ...
  const options = createMemo(() => {
    ...
    function buildOption(id: string, category: string) {
      ...
      const gutter = isWorking
        ...
        : slot !== undefined
          ...
          : undefined
      return {
        title: isDeleting ? `Баталгаажуулахын тулд ${deleteHint()} дахин дарна уу` : x.title,
    ...
    const remaining = displayOrder
      ...
      .map((id) => {
        ...
        return buildOption(id, label === today ? "Өнөөдөр" : label)
      })
      .filter((x) => x !== undefined)

    return [...pinned.map((id) => buildOption(id, "Хадсан")).filter((x) => x !== undefined), ...remaining]
  })
