...
import type { ProjectDirectories } from "@mongolgpt/sdk/v2"
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>{item.location}-г устгаж байна</span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>Баталгаажуулахын тулд {deleteHint()} дахин дарна уу</span>
        ) : suffix ? (
          <>
        ...
        category: item.root.directory === current ? "Одоогийн" : "Бусад",
        titleWidth,
