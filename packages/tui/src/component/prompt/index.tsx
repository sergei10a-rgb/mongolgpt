...
import type { AssistantMessage, FilePart, UserMessage } from "@mongolgpt/sdk/v2"
...
import { MONGOLGPT_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useMongolGPTKeymap } from "../../keymap"
...
export function Prompt(props: PromptProps) {
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Сешн",
        enabled: Flag.MONGOLGPT_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  async function submitInner() {
    ...
    if (sessionID == null) {
      ...
      if (res.error) {
        ...
        console.log("Сешн үүсгэж чадсангүй:", res.error)

        toast.show({
          message: "Сешн үүсгэж чадсангүй. Дэлгэрэнгүйг консолоос харна уу.",
          variant: "error",
        })
  ...
  async function pasteInputText(text: string) {
    ...
    if (
      ...
      pasteText(pastedContent, `[Наасан ~${lineCount} мөр]`)
      return
    }
  ...
  async function pasteAttachment(file: { filename?: string; filepath?: string; content: string; mime: string }) {
    ...
    const virtualText = pdf ? `[PDF ${count + 1}]` : `[Зураг ${count + 1}]`
  ...
  const placeholderText = createMemo(() => {
    ...
    if (store.mode === "shell") {
      ...
      return `Команд ажиллуулах... "${example}"`
    }
    if (!list().length) return undefined
    return `Юу ч асууж болно... "${list()[store.placeholder % list().length]}"`
  })
                  ...
                  {(agent) => (
                    <>
                      <text fg={fadeColor(highlight(), agentMetaAlpha())}>
                        {store.mode === "shell" ? "Терминал" : Locale.titlecase(agent().name)}
                      </text>
              ...
              >
                    ...
                    {(() => {
                      ...
                      const message = createMemo(() => {
                        ...
                        if (r.message.includes("exceeded your current quota") && r.message.includes("gemini"))
                          return "gemini яг одоо хэт ачаалалтай байна"
                      ...
                      const handleMessageClick = () => {
                        ...
                        if (!r) return
                        if (isTruncated()) {
                          void DialogAlert.show(dialog, "Дахин оролдох алдаа", r.message)
                        }
                      }
                      ...
                      const retryText = () => {
                        ...
                        const truncatedHint = isTruncated() ? " (задлахын тулд дарна уу)" : ""
                        const duration = formatDuration(seconds())
                        const retryInfo = ` [${duration ? `${duration}-ийн дараа ` : ""}дахин оролдоно, оролдлого #${r.attempt}]`
                ...
                <text fg={store.interrupt > 0 ? theme.primary : theme.text}>
                  ...
                  <span style={{ fg: store.interrupt > 0 ? theme.primary : theme.textMuted }}>
                    {store.interrupt > 0 ? "дахин дарж таслах" : "таслах"}
                  </span>
                </text>
              </box>
            </Match>
                    ...
                    {(() => {
                      ...
                      if (item.type === "new") {
                        if (workspace.creating())
                          return `${item.workspaceType} үүсгэж байна${".".repeat(workspace.creatingDots())}`
                        return (
                          <>
                            Ажлын орчин <span style={{ fg: theme.textMuted }}>(шинэ {item.workspaceType})</span>
                          </>
                        )
                      }
                      return (
                        <>
                          Ажлын орчин <span style={{ fg: theme.textMuted }}>{item.workspaceName}</span>
                        </>
                      )
                    })()}
                  </text>
              ...
              <Switch>
                <Match when={store.mode === "normal"}>
                  <Switch>
                    ...
                    <Match when={true}>
                      <text fg={theme.text}>
                        {agentShortcut()} <span style={{ fg: theme.textMuted }}>агентууд</span>
                      </text>
                    </Match>
                  </Switch>
                  <text fg={theme.text}>
                    {paletteShortcut()} <span style={{ fg: theme.textMuted }}>командууд</span>
                  </text>
                </Match>
