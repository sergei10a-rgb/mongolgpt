...
export function usePromptMove(input: { projectID: () => string | undefined; sessionID: () => string | undefined }) {
  ...
  async function create(context?: string) {
    ...
    if (!projectID) return
    setCreating(true)
    setProgress("Хуулбар үүсгэж байна")
    try {
      ...
      setProgress("Сешн үүсгэж байна")
      return directory
    } catch (err) {
      homeDestination?.clear()
      setProgress(undefined)
      setCreating(false)
      toast.show({ title: "Ажлын орчин үүсгэж чадсангүй", message: errorMessage(err), variant: "error" })
      return
    }
  }
  ...
  function startSubmit() {
    if (progress()) setProgress("Prompt илгээж байна")
  }
