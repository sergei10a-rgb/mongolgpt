...
} from "@mongolgpt/plugin/tui"
import { AttentionSoundName, type TuiConfig } from "./config"
import { Schema } from "effect"
import stripAnsi from "strip-ansi"
import * as TuiAudio from "./audio"
import defaultSoundPath from "@mongolgpt/ui/audio/bip-bop-01.mp3" with { type: "file" }
import questionSoundPath from "@mongolgpt/ui/audio/bip-bop-03.mp3" with { type: "file" }
import permissionSoundPath from "@mongolgpt/ui/audio/staplebops-06.mp3" with { type: "file" }
import errorSoundPath from "@mongolgpt/ui/audio/nope-03.mp3" with { type: "file" }
import doneSoundPath from "@mongolgpt/ui/audio/bip-bop-01.mp3" with { type: "file" }
import subagentDoneSoundPath from "@mongolgpt/ui/audio/yup-01.mp3" with { type: "file" }
...
const DEFAULT_TITLE = "MongolGPT"
const DEFAULT_PACK_ID = "mongolgpt.default"
const KV_SOUND_PACK = "attention_sound_pack"
const TITLE_LIMIT = 80
const MESSAGE_LIMIT = 240
const BUILTIN_PACK: RegisteredSoundPack = {
  id: DEFAULT_PACK_ID,
  name: "MongolGPT үндсэн",
  builtin: true,
  sounds: {
    ...
    try {
      for (const file of soundCandidates(name)) {
        const current = await audio.loadSoundFile(file).catch((error) => {
          console.debug("Анхааруулах дууг ачаалж чадсангүй", { file, error })
          return null
        })
      ...
      console.debug("Анхааруулах дууг тоглуулж чадсангүй", { error })
      return false
    }
  }
  ...
  return {
    async notify(request) {
      try {
        ...
        const notification = shouldNotify
          ? (() => {
              try {
                ...
                console.debug("Анхааруулах мэдэгдлийг ажиллуулж чадсангүй", { error })
                return false
              }
            })()
          : false
        ...
        console.debug("Анхааруулах мэдэгдлийг боловсруулж чадсангүй", { error })
        return {
          ok: false,
          notification: false,
          sound: false,
