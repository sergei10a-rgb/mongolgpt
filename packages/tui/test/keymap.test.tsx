...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerMongolGPTKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when mongolgpt mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getMongolGPTModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getMongolGPTModeStack(keymap).push("autocomplete")
