...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-mongolgpt-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-mongolgpt-directory"] ? decode(request.headers["x-mongolgpt-directory"]) : process.cwd())
