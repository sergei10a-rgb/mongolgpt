...
const reg = process.env.REGISTRY ?? "ghcr.io/sergei10a-rgb"
...
const setup = async () => {
  ...
  if (list.includes("mongolgpt")) {
    await $`docker buildx use mongolgpt`
    return
  }
  await $`docker buildx create --name mongolgpt --use`
}
