...
- Author package entrypoint is `@mongolgpt/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/mongolgpt-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@mongolgpt/plugin/tui`.
- `@mongolgpt/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@mongolgpt/plugin/tui"
...
- The value is a semver range checked against the running MongolGPT version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.mongolgpt` is absent, no check is performed (backward compatible).
...
- `mongolgpt plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `mongolgpt plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.mongolgpt` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.mongolgpt`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.mongolgpt`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.mongolgpt` dir or `MONGOLGPT_CONFIG_DIR`, MongolGPT installs `@mongolgpt/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@mongolgpt/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus MongolGPT's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
MongolGPT registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, MongolGPT pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@mongolgpt/plugin/tui`.
...
- `message` is required; `title` defaults to `"mongolgpt"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `mongolgpt.default`.
...
- Local plugins persist installed themes under the local `.mongolgpt/themes` area near the plugin config source.
...
- `--pure` / `MONGOLGPT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.mongolgpt/plugins/tui-smoke.tsx`
- Local vim plugin: `.mongolgpt/plugins/tui-vim.tsx`
- Local smoke config: `.mongolgpt/tui.json`
- Local smoke theme: `.mongolgpt/plugins/smoke-theme.json`
