...
Practical reference for a future `packages/server` split after the mongolgpt
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/mongolgpt`.
...
Until enough shared service code lives outside `packages/mongolgpt`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/mongolgpt`

It should not import `packages/mongolgpt` services while `packages/mongolgpt`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on mongolgpt-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/mongolgpt` implementation details.
