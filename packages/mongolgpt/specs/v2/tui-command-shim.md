...
Delete `packages/mongolgpt/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/mongolgpt/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/mongolgpt/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/mongolgpt`
- TUI plugin loader tests in `packages/mongolgpt` if runtime plugin API wiring changed
