...
await Bun.file(`./dist/${pkg.name}/bin/${pkg.name}.exe`).write(
  [
    `echo "Алдаа: ${pkg.name} багцын суулгасны дараах скрипт ажиллаагүй байна." >&2`,
    'echo "" >&2',
    'echo "--ignore-scripts сонголтоор суулгасан эсвэл pnpm зэрэг багцын менежер" >&2',
    'echo "postinstall скриптийг ажиллуулаагүй үед энэ алдаа гардаг." >&2',
    'echo "" >&2',
    'echo "Засахын тулд postinstall скриптийг гараар ажиллуулна уу:" >&2',
    `echo "  cd node_modules/${pkg.name} && node postinstall.mjs" >&2`,
    'echo "" >&2',
    `echo "Эсвэл ${pkg.name}-ийг --ignore-scripts сонголтгүй дахин суулгана уу." >&2`,
    "exit 1",
    "",
  ].join("\n"),
)
...
if (!Script.preview && !dryRun && !npmOnly) {
  ...
  const binaryPkgbuild = [
    ...
    "options=('!debug' '!strip')",
    "pkgrel=1",
    "pkgdesc='Монгол хэрэглэгчдэд зориулсан терминалын AI кодын агент.'",
  ...
  const homebrewFormula = [
    ...
    `  desc "Монгол хэрэглэгчдэд зориулсан терминалын AI кодын агент"`,
