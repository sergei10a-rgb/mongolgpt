...
describe("mongolgpt read-only commands (smoke)", () => {
  ...
  // MONGOLGPT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const r = yield* mongolgpt.spawn(["mcp", "list"])
        mongolgpt.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const r = yield* mongolgpt.spawn(["models"])
        mongolgpt.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const r = yield* mongolgpt.spawn(["agent", "list"])
        mongolgpt.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh MONGOLGPT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const r = yield* mongolgpt.spawn(["session", "list"])
        mongolgpt.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const r = yield* mongolgpt.spawn(["stats"])
        mongolgpt.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under MONGOLGPT_TEST_HOME / XDG_DATA_HOME.
