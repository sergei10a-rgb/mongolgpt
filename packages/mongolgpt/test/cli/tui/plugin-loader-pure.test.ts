...
`,
  ...
  const pure = process.env.MONGOLGPT_PURE
  const meta = process.env.MONGOLGPT_PLUGIN_META_FILE
  process.env.MONGOLGPT_PURE = "1"
  process.env.MONGOLGPT_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.MONGOLGPT_PURE
    } else {
      process.env.MONGOLGPT_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.MONGOLGPT_PLUGIN_META_FILE
    } else {
      process.env.MONGOLGPT_PLUGIN_META_FILE = meta
    }
  }
})
