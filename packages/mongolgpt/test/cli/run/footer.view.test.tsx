...
import type { QuestionRequest } from "@mongolgpt/sdk/v2"
import { MongolGPTKeymapProvider, registerMongolGPTKeymap } from "@mongolgpt/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerMongolGPTKeymap(keymap, renderer, config)
...
test.skip("direct footer recreates the frame across command panel transitions", async () => {
  ...
  try {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      expect(app.captureCharFrame()).toContain("Командууд")
      expect(footerComposerFrame(app.renderer.root)).not.toBe(composerFrame)
      app.mockInput.pressKey("c", { ctrl: true })
      await app.renderOnce()
      expect(app.captureCharFrame()).not.toContain("Командууд")
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerMongolGPTKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "mongolgpt", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerMongolGPTKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "mongolgpt", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Загвар сонгох")
    expect(frame).toContain("Хайх")
    expect(frame).toContain("mongolgpt")
    expect(frame).toContain("GPT-5")
    expect(frame).toContain("одоогийн")
