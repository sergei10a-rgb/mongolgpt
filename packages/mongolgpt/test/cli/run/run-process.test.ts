// Subprocess integration tests for `mongolgpt run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `mongolgpt.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `MONGOLGPT_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("mongolgpt run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mongolgpt.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mongolgpt.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mongolgpt.run("say hi", { format: "json" })
        mongolgpt.expectExit(result, 0)

        const events = mongolgpt.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const result = yield* mongolgpt.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mongolgpt.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mongolgpt.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const allowed = yield* mongolgpt.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        ...
        const explicitlyDenied = yield* mongolgpt.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        mongolgpt.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, mongolgpt }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mongolgpt.run("read the attachment", {
  ...
  cliIt.live(
    "attach mode requires the remote server account when Free Auto is the configured default",
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        const server = yield* mongolgpt.serve({
          env: {
        ...
        const result = yield* mongolgpt.spawn(["run", "--attach", server.url, "say hi"], {
          timeoutMs: 30_000,
        })
  ...
  cliIt.concurrent(
    ...
    ({ home, mongolgpt }) =>
      Effect.gen(function* () {
        const result = yield* mongolgpt.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, mongolgpt }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* mongolgpt.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
