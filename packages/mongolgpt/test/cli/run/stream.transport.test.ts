...
import { MongolGPTClient, type GlobalEvent } from "@mongolgpt/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<MongolGPTClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<MongolGPTClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<MongolGPTClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<MongolGPTClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<MongolGPTClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://mongolgpt.test"),
...
function sdk(
  input: {
    ...
    subscribe?: MongolGPTClient["event"]["subscribe"]
    globalEvent?: MongolGPTClient["global"]["event"]
    promptAsync?: MongolGPTClient["session"]["promptAsync"]
    status?: MongolGPTClient["session"]["status"]
    messages?: MongolGPTClient["session"]["messages"]
    children?: MongolGPTClient["session"]["children"]
    permissions?: MongolGPTClient["permission"]["list"]
    questions?: MongolGPTClient["question"]["list"]
  } = {},
) {
  const client = new MongolGPTClient()

  const subscribe: MongolGPTClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: MongolGPTClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: MongolGPTClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: MongolGPTClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: MongolGPTClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: MongolGPTClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: MongolGPTClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: MongolGPTClient["question"]["list"] = input.questions ?? (() => ok([]))
    ...
    try {
      ...
      expect(boot.tabs).toEqual([
        expect.objectContaining({
          sessionID: "child-1",
          label: "Explore",
          description: "Хүлээгдэж буй зөвшөөрөл",
          status: "running",
        }),
      ])
