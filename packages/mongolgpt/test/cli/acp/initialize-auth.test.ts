...
describe("mongolgpt acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ mongolgpt }))
        ...
        expect(initialized.agentInfo?.name).toBe("MongolGPT")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.authMethods?.[0]?.id).toBe("mongolgpt-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "mongolgpt-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.MONGOLGPT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mongolgpt }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("mongolgpt-login")
