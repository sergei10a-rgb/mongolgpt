...
import { createTuiAttention } from "@mongolgpt/tui/attention"
...
describe("createTuiAttention", () => {
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "mongolgpt", message: "hello", sound: false })).toEqual({
      ok: true,
      notification: true,
      sound: false,
    })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("mongolgpt.default")
  })
