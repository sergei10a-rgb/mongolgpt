...
it.instance("plan agent denies edits except .mongolgpt/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".mongolgpt/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".mongolgpt", "skill", "perm-skill")
...
`,
      ...
      process.env.MONGOLGPT_TEST_HOME = test.directory
