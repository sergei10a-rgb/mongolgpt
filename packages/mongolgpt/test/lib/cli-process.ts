// Subprocess test harness for the mongolgpt CLI. Spawns the real binary against
...
//
// Configuration flows through mongolgpt's built-in test affordances:
//   - MONGOLGPT_CONFIG_CONTENT      : provider config inline, no files to find
//   - MONGOLGPT_TEST_HOME           : pins os.homedir() → tmpdir
//   - MONGOLGPT_DISABLE_PROJECT_CONFIG : skip walking up for mongolgpt.json
//   - MONGOLGPT_PURE                : skip external plugin discovery + install
//   - MONGOLGPT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `mongolgpt.run` is fully wired. The shape supports adding more
// builders (`mongolgpt.serve(opts)`, `mongolgpt.acp(opts)`, `mongolgpt.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of MongolGPTCli.
...
// Typed equivalent of constructing argv for `mongolgpt run`. New flags should
...
// `mongolgpt serve` is a long-lived process — it never exits on its own.
...
// `mongolgpt acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type MongolGPTCli = {
  ...
  // Spawn `mongolgpt acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly mongolgpt: MongolGPTCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const startRun = Effect.fn("mongolgpt.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("mongolgpt.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
    ...
    const acp = Effect.fn("mongolgpt.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    // and hit endpoints on `mongolgpt.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
// every call site. Use this for any test that needs the mongolgpt CLI fixture.
//
...
// resources (e.g. `mongolgpt.serve`) without an extra `Effect.scoped` wrapper —
