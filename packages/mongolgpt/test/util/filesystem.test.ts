...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "mongolgpt.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "mongolgpt.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mongolgpt.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mongolgpt.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["mongolgpt.json", "mongolgpt.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "mongolgpt.json"),
        path.join(project, "mongolgpt.jsonc"),
      ])
    })
  })
