...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MONGOLGPT_TEST ?? "")'), {
      env: {
        MONGOLGPT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MONGOLGPT_TEST_SHELL"], {
      shell: true,
      env: {
        MONGOLGPT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MONGOLGPT_TEST_SHELL=ok")
  })
