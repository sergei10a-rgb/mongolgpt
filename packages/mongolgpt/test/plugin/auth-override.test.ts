            ...
            get: () =>
              Effect.succeed({
                ...
                plugin_origins: plugins.map((plugin) => ({
                  spec: plugin,
                  source: path.join(directory, "mongolgpt.json"),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const pluginDir = path.join(tmp.directory, ".mongolgpt", "plugin")
...
describe("plugin.config-hook-error-isolation", () => {
  test("config hooks are individually error-isolated in the layer factory", async () => {
    ...
    expect(src).toContain("Нэмэлтийн тохиргооны функц амжилтгүй боллоо")

    const pattern =
      /for\s*\(const hook of hooks\)\s*\{[\s\S]*?Effect\.tryPromise[\s\S]*?\.config\?\.\([\s\S]*?Нэмэлтийн тохиргооны функц амжилтгүй боллоо[\s\S]*?Effect\.ignore/
