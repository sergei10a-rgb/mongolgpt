...
import { LLMEvent, LLMResponse } from "@mongolgpt/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@mongolgpt/llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.MONGOLGPT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["mongolgpt", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MONGOLGPT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("MONGOLGPT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["mongolgpt", "native", "oauth", "tool-loop"],
  ...
  {
    id: "mongolgpt-proxy",
    name: "MongolGPT proxy",
    providerID: ProviderV2.ID.mongolgpt,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["mongolgpt", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.MONGOLGPT_RECORD_CONSOLE_TOKEN && process.env.MONGOLGPT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.mongolgpt,
        name: "MongolGPT Zen",
        ...
        api: zenURL(process.env.MONGOLGPT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.MONGOLGPT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.MONGOLGPT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["mongolgpt", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MONGOLGPT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("MONGOLGPT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("MONGOLGPT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set MONGOLGPT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
