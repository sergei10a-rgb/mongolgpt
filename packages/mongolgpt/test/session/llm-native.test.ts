...
import { LLMEvent, ToolFailure } from "@mongolgpt/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@mongolgpt/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("mongolgpt"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("mongolgpt") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("mongolgpt") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
  ...
  test("prefers console provider api key over stored mongolgpt auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("mongolgpt") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("mongolgpt"),
