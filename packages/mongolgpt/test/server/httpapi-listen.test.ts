...
const original = {
  ...
  envPassword: process.env.MONGOLGPT_SERVER_PASSWORD,
  envUsername: process.env.MONGOLGPT_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.MONGOLGPT_SERVER_PASSWORD
  else process.env.MONGOLGPT_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.MONGOLGPT_SERVER_USERNAME
  else process.env.MONGOLGPT_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.MONGOLGPT_SERVER_PASSWORD = auth.password
  Flag.MONGOLGPT_SERVER_USERNAME = auth.username
  process.env.MONGOLGPT_SERVER_PASSWORD = auth.password
  process.env.MONGOLGPT_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-mongolgpt-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-mongolgpt-ticket": "1" }),
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-mongolgpt-ticket": "1" },
      })
