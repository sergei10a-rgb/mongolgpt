...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      // This starts a second HTTP server that stands in for the mongolgpt server
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          "x-mongolgpt-directory": "/secret/path",
      ...
      expect(forwarded?.headers["x-mongolgpt-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-mongolgpt-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("returns 503 when a remote workspace is not actively syncing", () =>
    Effect.gen(function* () {
      ...
      expect(yield* response.text).toBe(`Workspace-ийн sync холболт тасарсан байна: ${workspaceID}`)
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-mongolgpt-directory", headerDir),
        HttpClient.execute,
      )
