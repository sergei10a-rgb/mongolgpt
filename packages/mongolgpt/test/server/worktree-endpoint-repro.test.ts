...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      MONGOLGPT_EXPERIMENTAL_WORKSPACES: Flag.MONGOLGPT_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.MONGOLGPT_EXPERIMENTAL_WORKSPACES = original.MONGOLGPT_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
