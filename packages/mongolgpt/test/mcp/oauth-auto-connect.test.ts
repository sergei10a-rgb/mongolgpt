...
const { FSUtil } = await import("@mongolgpt/core/fs-util")
const { CrossSpawnSpawner } = await import("@mongolgpt/core/cross-spawn-spawner")
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(yield* mcp.finishAuth(name, "invalid-code")).toEqual({
        status: "failed",
        error: "OAuth дуусгаж чадсангүй: Token exchange failed",
      })
