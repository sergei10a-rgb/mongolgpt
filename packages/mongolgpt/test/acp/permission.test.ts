...
import type { Event, MongolGPTClient } from "@mongolgpt/sdk/v2"
...
type PermissionReplyParams = Parameters<MongolGPTClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "mongolgpt-acp-permission-"))
