  ...
  describe("latest", () => {
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(npmCalls).toContain(`https://registry.npmjs.org/mongolgpt/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(bunCalls).toContain(`https://registry.npmjs.org/mongolgpt/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(pnpmCalls).toContain(`https://registry.npmjs.org/mongolgpt/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("sergei10a-rgb/tap/mongolgpt")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("mongolgpt")) return "mongolgpt"
          return ""
        },
      ),
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("sergei10a-rgb/tap/mongolgpt") && args.includes("--formula")) return "mongolgpt"
  ...
  describe("upgrade", () => {
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(error.stderr).toBe("npm аргаар шинэчилж чадсангүй (гаралтын код: 1).")
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(error).toBeInstanceOf(Installation.UpgradeFailedError)
        expect(error.stderr).toBe("curl аргаар шинэчилж чадсангүй (гаралтын код: 1).")
        expect(error.message).toBe(error.stderr)
        expect(error.stderr).not.toContain("secret")
