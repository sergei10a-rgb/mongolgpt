...
import type { HostPluginApi } from "@mongolgpt/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createMongolGPTClient({
  ...
  let selected = opts.theme?.selected ?? "mongolgpt"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "mongolgpt.default",
