...
import { LayerNodeTree } from "@mongolgpt/core/effect/layer-node"
...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".mongolgpt")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .mongolgpt/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const mongolgpt = path.join(test.directory, ".mongolgpt")
      const tool = path.join(mongolgpt, "tool")
  ...
  it.instance("ignores non-tool exports in .mongolgpt/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".mongolgpt", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-mongolgpt` was registering a tool
  ...
  it.instance("loads tools from .mongolgpt/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const mongolgpt = path.join(test.directory, ".mongolgpt")
      const tools = path.join(mongolgpt, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".mongolgpt", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const mongolgpt = path.join(test.directory, ".mongolgpt")
        const customTools = path.join(mongolgpt, "tools")
        const plugin = path.join(mongolgpt, "node_modules", "@mongolgpt", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(mongolgpt, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@mongolgpt/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".mongolgpt", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(mongolgpt, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@mongolgpt/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(mongolgpt, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(mongolgpt, "node_modules", "cowsay")
