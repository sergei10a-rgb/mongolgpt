...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.MONGOLGPT_WEBSEARCH_PROVIDER

    try {
      process.env.MONGOLGPT_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.MONGOLGPT_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.MONGOLGPT_WEBSEARCH_PROVIDER
      else process.env.MONGOLGPT_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for mongolgpt or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.mongolgpt, { exa: false, parallel: false })).toBe(true)
  ...
  test("uses branded labels", () => {
    ...
    expect(webSearchProviderLabel(undefined)).toBe("Вэб хайлт")
  })
