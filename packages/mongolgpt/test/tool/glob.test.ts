...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.MONGOLGPT_REPO_CLONE_GITHUB_BASE_URL
      process.env.MONGOLGPT_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.MONGOLGPT_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.MONGOLGPT_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
  ...
  it.instance("rejects exact file paths", () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(err instanceof Error ? err.message : String(err)).toContain("glob path нь directory байх ёстой")
      }
    }),
  )
})
