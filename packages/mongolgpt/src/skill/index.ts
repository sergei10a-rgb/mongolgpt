...
import { SkillPlugin } from "@mongolgpt/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@mongolgpt/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@mongolgpt/core/v1/config/error"
...
const MONGOLGPT_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with MongolGPT. The model's intuition for what a
// mongolgpt.json should look like is often wrong, and MongolGPT hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch MongolGPT's own config files gives it the
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, MONGOLGPT_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_MONGOLGPT_SKILL_NAME] = {
          name: CUSTOMIZE_MONGOLGPT_SKILL_NAME,
          description: CUSTOMIZE_MONGOLGPT_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_MONGOLGPT_SKILL_BODY,
        }
