import type { Session as SDKSession, Message, Part } from "@mongolgpt/sdk/v2"
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    ...
    exportData = (yield* fs.readJson(file).pipe(Effect.orElseSucceed(() => undefined))) as
      ...
      | undefined
    if (!exportData) {
      process.stdout.write(`Файл олдсонгүй: ${file}`)
      process.stdout.write(EOL)
      return
    }
  }
  ...
  if (!exportData) {
    process.stdout.write(`Сешний өгөгдөл уншиж чадсангүй`)
    process.stdout.write(EOL)
    return
  }
