...
const run = Effect.fn("Cli.debug.agent.body")(function* (
  ...
  if (!agent) {
    process.stderr.write(
      `Agent ${agentName} олдсонгүй, agent жагсаалт авахын тулд '${basename(process.execPath)} agent list' ажиллуулна уу` +
        EOL,
    )
    return yield* fail("", 1)
  }
...
function parseToolParams(input?: string) {
  ...
  const parsed = iife(() => {
    try {
      ...
      try {
        ...
        throw new Error(
          `--params parse хийж чадсангүй. JSON эсвэл JS object literal ашиглана уу. JSON алдаа: ${jsonError}. Eval алдаа: ${evalError}.`,
          { cause: evalError },
        )
      }
    }
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Tool-ийн params object байх ёстой.")
  }
...
const createToolContext = Effect.fn("Cli.debug.agent.createToolContext")(function* (
  ...
  const session = yield* sessionSvc.create({ title: `Debug tool ажиллуулах (${agent.name})` })
