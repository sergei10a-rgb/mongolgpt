...
import type { QuestionInfo, QuestionRequest } from "@mongolgpt/sdk/v2"
...
export function questionHint(request: QuestionRequest, state: QuestionBodyState): string {
  if (state.submitting) {
    return "Асуултын event хүлээж байна..."
  }

  if (questionConfirm(request, state)) {
    return "enter илгээх   esc хаах"
  }

  if (state.editing) {
    return "enter хадгалах   esc цуцлах"
  }

  const info = questionInfo(request, state)
  if (questionSingle(request)) {
    return `↑↓ сонгох   enter ${info?.multiple ? "асаах/унтраах" : "илгээх"}   esc хаах`
  }

  return `⇆ tab   ↑↓ сонгох   enter ${info?.multiple ? "асаах/унтраах" : "батлах"}   esc хаах`
}
