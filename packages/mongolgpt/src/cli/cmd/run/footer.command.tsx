...
function categoryRank(category: string) {
  if (category === "Төслийн командууд") {
    return 0
  }

  if (category === "MCP командууд") {
    return 1
  }
...
export function RunCommandMenuBody(props: {
  ...
  const entries = createMemo<CommandEntry[]>(() => {
    ...
    const commands = (props.commands() ?? [])
      ...
      .map(
        (item) =>
          ({
            action: "slash",
            category: item.source === "mcp" ? "MCP командууд" : "Төслийн командууд",
            name: item.name,
            display: item.name,
            footer: `/${item.name}`,
            keywords:
    ...
    return [
      ...
      ...agent,
      ...commands,
      { action: "exit", category: "Систем", display: "Гарах", footer: "/exit", keywords: "/exit exit" },
    ]
  })
...
export function RunQueuedPromptSelectBody(props: {
  ...
  const entries = createMemo<QueuedEntry[]>(() =>
    props.prompts().map((prompt) => ({
      ...
      footer: "дараалалд · ctrl+e засах · ctrl+d устгах",
  ...
  const entries = createMemo<VariantEntry[]>(() => [
    {
      category: "",
      display: "Анхдагч",
      description: props.current() === undefined ? "одоогийн" : undefined,
      keywords: "default",
      variant: undefined,
    ...
    ...props.variants().map((variant) => ({
      category: "",
      display: variant,
      description: props.current() === variant ? "одоогийн" : undefined,
      keywords: variant,
      variant,
...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "одоогийн"
              : model.cost?.input === 0 && provider.id === "mongolgpt"
                ? "Үнэгүй"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider = Number(a.providerID !== "mongolgpt") - Number(b.providerID !== "mongolgpt")
        if (provider !== 0) {
          return provider
        }
  ...
  return (
    ...
    >
      <RunFooterMenu
        ...
        rows={() => PANEL_LIST_ROWS}
        limit={PANEL_LIST_ROWS}
        empty={props.providers() ? "Илэрц олдсонгүй" : "Загварууд ачаалж байна"}
        border={false}
        paddingLeft={PANEL_PAD}
        paddingRight={PANEL_PAD}
        grouped={!query().trim()}
