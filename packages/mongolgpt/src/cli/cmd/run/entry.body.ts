...
function reasoningBody(raw: string): RunEntryBody {
  ...
  if (body.startsWith(mark)) {
    return codeBody(`${lead}_Бодол:_ ${body.slice(mark.length).trimStart()}`, "markdown")
  }
...
export function entryBody(commit: StreamCommit): RunEntryBody {
  ...
  if (commit.kind === "assistant") {
    ...
    if (commit.phase === "final") {
      return commit.interrupted ? textBody("туслах таслагдлаа") : RUN_ENTRY_NONE
    }
  ...
  if (commit.kind === "reasoning") {
    ...
    if (commit.phase === "final") {
      return commit.interrupted ? textBody("бодолт таслагдлаа") : RUN_ENTRY_NONE
    }
