// Shared type vocabulary for the direct interactive mode (`mongolgpt --mini`).
//
...
import type { MongolGPTClient, PermissionRequest, QuestionRequest, ToolPart } from "@mongolgpt/sdk/v2"
import type { TuiConfig } from "@mongolgpt/tui/config"
...
type PromptModel = Parameters<MongolGPTClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<MongolGPTClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<MongolGPTClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<MongolGPTClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<MongolGPTClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<MongolGPTClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<MongolGPTClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<MongolGPTClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<MongolGPTClient["question"]["reject"]>[0]
