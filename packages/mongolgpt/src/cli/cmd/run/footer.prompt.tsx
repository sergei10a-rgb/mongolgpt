...
import { normalizePromptContent } from "@mongolgpt/tui/editor"
...
import { MONGOLGPT_BASE_MODE, useBindings } from "@mongolgpt/tui/keymap"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const placeholder = createMemo(() => {
    if (shell()) {
      return new StyledText([fg(input.theme().muted)('Команд ажиллуулах... "git status"')])
    }
    ...
    return new StyledText([fg(input.theme().muted)('Юу ч асуугаарай... "Кодын TODO-г зас"')])
  })
  ...
  const submitPrompt = (next: RunPrompt) => {
    ...
    if (parsed?.type === "pending") {
      input.onStatus("командууд ачаалж байна")
      return
    }
