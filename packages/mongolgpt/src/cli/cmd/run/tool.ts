// Per-tool display rules shared across `mongolgpt run` output paths.
//
...
import type { ToolPart } from "@mongolgpt/sdk/v2"
...
function fallbackInline(ctx: ToolFrame): ToolInline {
  const title = text(ctx.state.title) || (Object.keys(ctx.input).length > 0 ? JSON.stringify(ctx.input) : "Тодорхойгүй")
...
function runGlob(p: ToolProps<typeof GlobTool>): ToolInline {
  ...
  const title = `Файл хайлт "${p.input.pattern ?? ""}"`
  const suffix = root ? `${toolPath(root)} дотор` : ""
  const matches = p.metadata.count
  const description = matches === undefined ? suffix : `${suffix}${suffix ? " · " : ""}${count(matches, "тохирол")}`
  return {
    icon: "✱",
    title,
...
function runGrep(p: ToolProps<typeof GrepTool>): ToolInline {
  const root = p.input.path ?? ""
  const title = `Агуулга хайлт "${p.input.pattern ?? ""}"`
  const suffix = root ? `${toolPath(root)} дотор` : ""
...
function runList(p: ToolProps): ToolInline {
  ...
  return {
    icon: "→",
    title: dir ? `Жагсаах ${toolPath(dir)}` : "Жагсаах",
  }
}
...
function runWrite(p: ToolProps<typeof WriteTool>): ToolInline {
  return {
    icon: "←",
    title: `Бичих ${toolPath(p.input.filePath)}`,
    mode: "block",
...
function runEdit(p: ToolProps<typeof EditTool>): ToolInline {
  return {
    icon: "←",
    title: `Засах ${toolPath(p.input.filePath)}`,
    mode: "block",
...
function runTask(p: ToolProps<typeof TaskTool>): ToolInline {
  const kind = Locale.titlecase(p.input.subagent_type || "тодорхойгүй")
  const desc = p.input.description
  const icon = p.frame.status === "error" ? "✗" : p.frame.status === "running" ? "•" : "✓"
  return {
    icon,
    title: desc || `${kind} даалгавар`,
    description: desc ? `${kind} агент` : undefined,
  }
}
...
function runInvalid(p: ToolProps<typeof InvalidTool>): ToolInline {
  return {
    icon: "✗",
    title: text(p.frame.state.title) || "Хүчингүй хэрэгсэл",
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
...
function runBatch(p: ToolProps): ToolInline {
  ...
  return {
    icon: "#",
    title: text(p.frame.state.title) || (calls > 0 ? `Багц ${calls} хэрэгсэл` : "Багц"),
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
...
function patchTitle(file: PatchFile): string {
  ...
  if (file.type === "add") {
    return `# Үүсгэсэн ${rel || toolPath(from)}`
  }
  if (file.type === "delete") {
    return `# Устгасан ${rel || toolPath(from)}`
  }
  if (file.type === "move") {
    return `# Зөөсөн ${toolPath(from)} -> ${rel || toolPath(file.movePath)}`
  }

  return `# Нөхсөн ${rel || toolPath(from)}`
}
...
function snapTask(p: ToolProps<typeof TaskTool>): ToolSnapshot {
  const kind = Locale.titlecase(p.input.subagent_type || "ерөнхий")
  const desc = p.input.description
...
function snapQuestion(p: ToolProps<typeof QuestionTool>): ToolSnapshot {
  ...
  const items = list<{ question?: string }>(p.frame.input.questions).map((item, i) => {
    ...
    return {
      question: item.question || `Асуулт ${i + 1}`,
      answer: answer.length > 0 ? answer.join(", ") : "(хариулт алга)",
    }
  })
...
function scrollBashStart(p: ToolProps<typeof BashTool>): string {
  ...
  if (!cmd) {
    return dir ? `# ${dir} дотор ажиллаж байна` : ""
  }

  return `# ${dir} дотор ажиллаж байна\n$ ${cmd}`
}
...
function scrollPatchFinal(p: ToolProps<typeof ApplyPatchTool>): string {
  ...
  if (shown.length > 6) {
    rows.push(`... мөн ${shown.length - 6} нэмэлт`)
  }
...
function scrollTodoFinal(p: ToolProps<typeof TodoWriteTool>): string {
  ...
  const tail = [`нийт ${items.length}`]
  if (doneN > 0) {
    tail.push(`${doneN} дууссан`)
  }
  if (runN > 0) {
    tail.push(`${runN} идэвхтэй`)
  }
  if (left > 0) {
    tail.push(`${left} хүлээгдэж байна`)
  }
...
function scrollQuestionFinal(p: ToolProps<typeof QuestionTool>): string {
  ...
  for (const [i, item] of q.slice(0, 4).entries()) {
    ...
    const reply = a[i] ?? []
    rows.push(`? ${prompt || `Асуулт ${i + 1}`}`)
    rows.push(`  ${reply.length > 0 ? reply.join(", ") : "(хариулт алга)"}`)
  }

  if (q.length > 4) {
    rows.push(`... мөн ${q.length - 4} нэмэлт`)
  }
...
function scrollGlobStart(p: ToolProps<typeof GlobTool>): string {
  ...
  const head = pattern ? `✱ Файл хайлт "${pattern}"` : "✱ Файл хайлт"
  const dir = p.input.path ?? ""
  if (!dir) {
    return head
  }

  return `${head} ${toolPath(dir)} дотор`
}
...
function scrollGrepStart(p: ToolProps<typeof GrepTool>): string {
  const pattern = p.input.pattern ?? ""
  const head = pattern ? `✱ Агуулга хайлт "${pattern}"` : "✱ Агуулга хайлт"
  const dir = p.input.path ?? ""
  if (!dir) {
    return head
  }
...
function permEdit(p: ToolPermissionProps<typeof EditTool>): ToolPermissionInfo {
  ...
  return {
    icon: "→",
    title: `Засах ${toolPath(file, { home: true })}`,
    lines: [],
...
function permRead(p: ToolPermissionProps<typeof ReadTool>): ToolPermissionInfo {
  ...
  return {
    icon: "→",
    title: `Унших ${toolPath(file, { home: true })}`,
    lines: file ? [`Зам: ${toolPath(file, { home: true })}`] : [],
  }
}
...
function permList(p: ToolPermissionProps): ToolPermissionInfo {
  ...
  return {
    icon: "→",
    title: `Жагсаах ${toolPath(dir, { home: true })}`,
    lines: dir ? [`Зам: ${toolPath(dir, { home: true })}`] : [],
  }
}
...
function permTask(p: ToolPermissionProps<typeof TaskTool>): ToolPermissionInfo {
  const type = p.input.subagent_type || "ерөнхий"
  const desc = p.input.description
  return {
    icon: "#",
...
function permLsp(p: ToolPermissionProps<typeof LspTool>): ToolPermissionInfo {
  ...
  return {
    ...
    lines: [
      ...(p.input.operation ? [`Үйлдэл: ${p.input.operation}`] : []),
      ...(file ? [`Зам: ${toolPath(file, { home: true })}`] : []),
      ...(pos ? [`Байрлал: ${pos}`] : []),
    ],
  }
}
