...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (!ctx) return yield* fail("Instance context ачаалж чадсангүй")
    if (ctx.project.vcs !== "git") {
      return yield* fail("Git repository олдсонгүй. Энэ командыг git repository дотроос ажиллуулна уу.")
    }
    ...
    if (checkout.code !== 0) {
      return yield* fail(`PR #${prNumber} checkout хийж чадсангүй. gh CLI суусан, нэвтэрсэн эсэхээ шалгана уу.`)
    }
    ...
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["mongolgpt", ...mongolgptArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
