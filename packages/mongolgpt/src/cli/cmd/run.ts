import type { PermissionV1 } from "@mongolgpt/core/v1/permission"
import { FSUtil } from "@mongolgpt/core/fs-util"
// CLI entry point for `mongolgpt run` and `mongolgpt --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`mongolgpt --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`mongolgpt --mini --attach`): connects to a running
//      MongolGPT server and runs interactive mode against it.
//
...
type ModelInput = Parameters<MongolGPTClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "мессежтэйгээр MongolGPT ажиллуулах",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("file", {
        ...
        type: "string",
        array: true,
        describe: "мессежид хавсаргах файл(ууд)",
      })
      .option("title", {
        type: "string",
      ...
      .option("attach", {
        type: "string",
        describe: "ажиллаж буй MongolGPT серверт холбогдох (жишээ: http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("demo", {
        ...
        hidden: true,
        describe:
          "шууд интерактив demo slash командуудыг идэвхжүүлэх; даруй ажиллуулах бол нэгийг нь message болгон өгнө",
      }),
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (interactive && args.command) {
        die("--mini-ийг --command-тэй хамт ашиглах боломжгүй")
      }

      if (interactive && args._?.[0] !== "mini") {
        die("--mini-ийг run subcommand-гүй ашиглах ёстой")
      }

      if (args.demo && !interactive) {
        die("--demo нь --mini шаарддаг")
      }

      if (interactive && args.format === "json") {
        die("--mini-ийг --format json-тэй хамт ашиглах боломжгүй")
      }

      if (args["replay-limit"] !== undefined && !interactive) {
        die("--replay-limit нь --mini шаарддаг")
      }
      ...
      if (
        ...
        die("--replay-limit эерэг бүхэл тоо байх ёстой")
      }

      if (interactive && !process.stdout.isTTY) {
        die("--mini нь TTY stdout шаарддаг")
      }
      ...
      const directory = (() => {
        ...
        try {
          ...
          UI.error("Хавтсыг солиж чадсангүй: " + args.dir)
          process.exit(1)
        }
      })()
      ...
      async function session(sdk: MongolGPTClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: MongolGPTClient, sessionID: string) {
      ...
      async function current(sdk: MongolGPTClient): Promise<string> {
        if (!args.attach) {
        ...
        UI.error("Алсын хавтсыг тодорхойлж чадсангүй")
        process.exit(1)
      }
      ...
      async function attachAgent(sdk: MongolGPTClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: MongolGPTClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: MongolGPTClient) {
        ...
        async function loop(client: MongolGPTClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createMongolGPTClient({
        baseUrl: "http://mongolgpt.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
