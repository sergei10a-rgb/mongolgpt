...
import type { LLMClientShape } from "@mongolgpt/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @mongolgpt/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains MongolGPT-owned. The native runtime only adapts
      // the @mongolgpt/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
