...
export interface Interface {
  ...
  /**
   * Resolved truncation limits: values from `tool_output` in MongolGPT config, or MAX_LINES / MAX_BYTES if unset.
   */
  readonly limits: () => Effect.Effect<{ maxLines: number; maxBytes: number }>
}

export class Service extends Context.Service<Service, Interface>()("@mongolgpt/Truncate") {}
  ...
  Effect.gen(function* () {
    ...
    const cleanup = Effect.fn("Truncate.cleanup")(function* () {
      ...
      for (const entry of entries) {
        if (Identifier.timestampAtOrBefore(entry, now) >= cutoff) continue
    ...
    const output = Effect.fn("Truncate.output")(function* (text: string, options: Options = {}, agent?: Agent.Info) {
      ...
      const hint = hasTaskTool(agent)
        ? `Хэрэгслийн дуудлага амжилттай боловч гаралтыг товчиллоо. Бүтэн гаралтыг энд хадгаллаа: ${file}\nБүтэн файлыг өөрөө уншихгүй, Task хэрэгслээр explore агентад өгч Grep болон Read (offset/limit-тэй) ашиглан боловсруулна уу.`
        : `Хэрэгслийн дуудлага амжилттай боловч гаралтыг товчиллоо. Бүтэн гаралтыг энд хадгаллаа: ${file}\nБүтэн агуулгаас хайхдаа Grep, тодорхой хэсэг уншихдаа offset/limit-тэй Read ашиглана уу.`
