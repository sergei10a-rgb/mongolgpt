...
import { IdeEvent } from "@mongolgpt/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["MONGOLGPT_CALLER"] === "vscode" || process.env["MONGOLGPT_CALLER"] === "vscode-insiders"
}

export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  const cmd = SUPPORTED_IDES.find((i) => i.name === ide)?.cmd
  if (!cmd) throw new Error(`Танигдаагүй IDE: ${ide}`)

  const p = await Process.run([cmd, "--install-extension", "mongolgpt.mongolgpt"], {
    nothrow: true,
  })
