...
import type { ConsoleState } from "@mongolgpt/core/v1/config/console-state"
...
import { RemoteAuthError } from "@mongolgpt/core/v1/config/error"
import { ConfigPermissionV1 } from "@mongolgpt/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["mongolgpt.jsonc", "mongolgpt.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.MONGOLGPT_CONFIG && !Flag.MONGOLGPT_CONFIG_DIR && !Flag.MONGOLGPT_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mongolgpt.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mongolgpt.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "MONGOLGPT_CONFIG_CONTENT") return "local"
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.mongolgpt/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (Flag.MONGOLGPT_CONFIG_CONTENT) {
          const source = "MONGOLGPT_CONFIG_CONTENT"
          const next = yield* loadConfig(Flag.MONGOLGPT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["MONGOLGPT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("MONGOLGPT_CONSOLE_TOKEN", tokenOpt.value)
            }
