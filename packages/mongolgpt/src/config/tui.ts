...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old mongolgpt.json shape) still get their settings applied.
  ...
  const projectFiles = Flag.MONGOLGPT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit MONGOLGPT_TUI_CONFIG override, if set.
