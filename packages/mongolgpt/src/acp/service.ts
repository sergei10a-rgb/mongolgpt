...
import type { AssistantMessage, Message, MongolGPTClient, SessionMessageResponse } from "@mongolgpt/sdk/v2"
...
export const AuthMethodID = "mongolgpt-login"
...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/ACP/Service") {}
...
function makeDirectoryService(sdk: MongolGPTClient) {
...
function makeUsageService(sdk: MongolGPTClient) {
...
async function loadDirectorySnapshot(sdk: MongolGPTClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, MongolGPT provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const mongolgptProvider = providers[ProviderV2.ID.make("mongolgpt")]
  const mongolgptModel = mongolgptProvider ? Provider.sort(Object.values(mongolgptProvider.models))[0] : undefined
  if (mongolgptProvider && mongolgptModel) return { providerID: mongolgptProvider.id, modelID: mongolgptModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "MongolGPT prompt амжилтгүй боллоо"
}
