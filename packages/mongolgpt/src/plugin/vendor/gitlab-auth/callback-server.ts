    ...
    constructor(options = {}) {
        ...
        this.server.get('/callback', async (request, reply) => {
            ...
            if (error) {
                ...
                this.rejectCallback?.(new Error(`OAuth алдаа: ${errorMsg}`));
                await reply.type('text/html').send(`
          <!DOCTYPE html>
          <html>
            <head><title>Нэвтрэлт амжилтгүй</title></head>
            <body>
              <h1>Нэвтрэлт амжилтгүй</h1>
              <p>${errorMsg}</p>
              <p>Энэ цонхыг хааж болно.</p>
            </body>
          </html>
        `);
                this.cleanup();
            ...
            if (!code || !state) {
                this.rejectCallback?.(new Error('code эсвэл state параметр дутуу байна'));
                await reply.type('text/html').send(`
          <!DOCTYPE html>
          <html>
            <head><title>Нэвтрэлт амжилтгүй</title></head>
            <body>
              <h1>Нэвтрэлт амжилтгүй</h1>
              <p>Шаардлагатай параметр дутуу байна.</p>
              <p>Энэ цонхыг хааж болно.</p>
            </body>
          </html>
        `);
        ...
        <!DOCTYPE html>
        <html>
          <head><title>Нэвтрэлт амжилттай</title></head>
          <body>
            <h1>Нэвтрэлт амжилттай</h1>
            <p>Энэ цонхыг хаагаад MongolGPT рүү буцаж болно.</p>
          </body>
        </html>
      `);
            this.cleanup();
