...
import { generateSecret, generateCodeChallengeFromVerifier } from './pkce';
/**
...
export class GitLabOAuthFlow {
    ...
    async authorizeWithCallback(codeChallenge, state, codeVerifier) {
        ...
        try {
            ...
            if (result.state !== state) {
                throw new Error('State тохирохгүй байна - CSRF эрсдэл байж магадгүй');
            }
            return {
                code: result.code,
                state: result.state,
    ...
    authorizeWithManualCode(codeChallenge, state, _codeVerifier) {
        ...
        throw new Error('Гараар code оруулах боломж одоогоор хийгдээгүй байна - auto аргыг ашиглана уу');
    }
    /**
