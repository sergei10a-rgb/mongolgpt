...
import { errorData, errorMessage } from "@mongolgpt/tui/util/error"
import { isRecord } from "@mongolgpt/tui/util/record"
...
import { hasTheme, upsertTheme } from "@mongolgpt/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@mongolgpt/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@mongolgpt/core/v1/config/plugin"
import { createCommandShim } from "@mongolgpt/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@mongolgpt/tui/plugin/runtime"
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".mongolgpt", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.MONGOLGPT_PURE ? [] : pluginOrigins
    if (Flag.MONGOLGPT_PURE && pluginOrigins.length) {
    }
