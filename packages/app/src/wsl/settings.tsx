...
import { Dialog, DialogBody, DialogHeader, DialogTitle } from "@mongolgpt/ui/v2/dialog-v2"
import { DividerV2 } from "@mongolgpt/ui/v2/divider-v2"
import { Icon as IconV2 } from "@mongolgpt/ui/v2/icon"
import { IconButtonV2 } from "@mongolgpt/ui/v2/icon-button-v2"
import { MenuV2 } from "@mongolgpt/ui/v2/menu-v2"
...
import { wslMongolGPTAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.mongolgptChecks[item.config.distro]
          const mongolgptAction = () => wslMongolGPTAction(check())
          const busy = () => wsl.data?.job?.kind === "install-mongolgpt" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={mongolgptAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installMongolGPT(item.config.distro))}
                    >
