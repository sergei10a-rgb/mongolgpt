...
import { Spinner } from "@mongolgpt/ui/spinner"
import { showToast } from "@mongolgpt/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslMongolGPTStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "mongolgpt"

const STEPS: WslServerStep[] = ["wsl", "distro", "mongolgpt"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const mongolgptCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.mongolgptChecks[distro] ?? null
  })
  ...
  const mongolgptReady = createMemo(() => {
    const check = mongolgptCheck()
  ...
  const installingMongolGPT = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-mongolgpt" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && mongolgptReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-mongolgpt"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.mongolgptChecks[distro]) {
      return { key: `probe-mongolgpt:${distro}`, run: () => api.probeMongolGPT(distro) }
    }
    return null
  })
  ...
  const mongolgptMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingMongolGPT")
    const distro = selectedDistro()
    if (state.job?.kind === "install-mongolgpt") {
      return distro
        ? language.t("wsl.onboarding.updatingMongolGPTIn", { distro })
        : language.t("wsl.onboarding.updatingMongolGPT")
    }
    if (state.job?.kind === "probe-mongolgpt") {
      return distro
        ? language.t("wsl.onboarding.checkingMongolGPTIn", { distro })
        : language.t("wsl.onboarding.checkingMongolGPT")
    }
    if (mongolgptCheck()?.error) return mongolgptCheck()!.error
    if (mongolgptCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateMongolGPTIn", { distro })
        : language.t("wsl.onboarding.updateMongolGPT")
    }
    if (mongolgptReady()) {
      return distro
        ? language.t("wsl.onboarding.mongolgptReadyIn", { distro })
        : language.t("wsl.onboarding.mongolgptReady")
    }
    return distro
      ? language.t("wsl.onboarding.installMongolGPTIn", { distro })
  ...
  const openMongolGPTStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslMongolGPTStep(distro, api.probeMongolGPT, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.mongolgpt"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : mongolgptCheck()?.matchesDesktop === false
                  ? "warning"
                  : mongolgptReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "mongolgpt"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.mongolgpt")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeMongolGPT(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!mongolgptReady() || mongolgptCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installMongolGPT(distro))}
                      >
                        <Show when={installingMongolGPT()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {mongolgptCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateMongolGPT")
                          : language.t("wsl.onboarding.installMongolGPT")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{mongolgptMessage()}</div>
                <Show when={mongolgptCheck()?.matchesDesktop === false ? mongolgptCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "mongolgpt" && allReady() && selectedDistro()}>
