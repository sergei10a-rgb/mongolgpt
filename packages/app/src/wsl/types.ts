...
export type WslJob =
  ...
  | { kind: "probe-mongolgpt"; distro: string; startedAt: number }
  | { kind: "install-mongolgpt"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeMongolGPT(name: string): Promise<void>
  installMongolGPT(name: string): Promise<void>
