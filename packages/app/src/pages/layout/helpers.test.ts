...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("mongolgpt://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("mongolgpt://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("mongolgpt://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("mongolgpt://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("mongolgpt://open-project")).toBeUndefined()
    expect(parseDeepLink("mongolgpt://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "mongolgpt://open-project?directory=/a",
      "mongolgpt://other?directory=/b",
      "mongolgpt://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("mongolgpt://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("mongolgpt://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("mongolgpt://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("mongolgpt://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "mongolgpt://new-session?directory=/a",
      "mongolgpt://open-project?directory=/b",
      "mongolgpt://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MONGOLGPT__: {
        deepLinks: ["mongolgpt://open-project?directory=/a"],
      },
    } as unknown as Window & { __MONGOLGPT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["mongolgpt://open-project?directory=/a"])
