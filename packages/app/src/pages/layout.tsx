...
import { ResizeHandle } from "@mongolgpt/ui/resize-handle"
...
import { Session } from "@mongolgpt/sdk/v2/client"
...
import { retry } from "@mongolgpt/core/util/retry"
...
import { useTheme, type ColorScheme } from "@mongolgpt/ui/theme/context"
...
export default function LegacyLayout(props: ParentProps) {
  ...
  function connectMongolGPTProvider() {
    if (!providers.all().has("mongolgpt")) {
      connectProvider()
      return
    }
    ...
    void import("@/components/dialog-connect-provider").then((x) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(() => <x.DialogConnectProvider provider="mongolgpt" />)
    })
  }
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openLink("https://mongolgpt.duckdns.org/desktop-feedback")}
      renderPanel={() =>
