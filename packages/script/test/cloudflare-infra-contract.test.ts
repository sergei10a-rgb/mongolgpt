...
describe("Cloudflare hosted infrastructure contract", () => {
  ...
  test("deploys only the isolated dev admin with exact confirmation and bounded secrets", async () => {
    ...
    expect(diff?.run).toContain('bun script/sst-error-diagnostics.ts .sst/pulumi "$stderr_file"')
    ...
    expect(deploy?.run).toContain('bun script/sst-error-diagnostics.ts .sst/pulumi "$stderr_file"')
