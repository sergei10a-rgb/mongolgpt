...
describe("Cloudflare hosted infrastructure contract", () => {
  ...
  test("deploys only the isolated dev admin with exact confirmation and bounded secrets", async () => {
    ...
    expect(browserSmoke).toContain("isCloudflareAccessDataImageCspNoise")
    expect(browserSmoke).toContain('hostname.endsWith(".cloudflareaccess.com")')
    expect(browserSmoke).toContain("Loading the image 'data:image/svg+xml")
  ...
  test("archives payment dead letters outside D1 and keeps a bounded last-resort queue", async () => {
    ...
    expect(source).toContain("new cloudflare.R2BucketLifecycle(")
    expect(source).toContain('"PaymentRecoveryArchiveRetention"')
    expect(source).toContain('const PAYMENT_RECOVERY_ARCHIVE_PREFIX = "payment-recovery/v1/"')
    expect(source).toContain("conditions: { prefix: PAYMENT_RECOVERY_ARCHIVE_PREFIX }")
    expect(source).toContain('id: "expire-payment-recovery-archive-after-14-days"')
    expect(source).toContain("maxAge: PAYMENT_QUEUE_RETENTION_SECONDS")
  ...
  test("creates a fail-closed Cloudflare Access admin application through IaC", async () => {
    ...
    expect(deploymentSource).not.toContain("customDenyMessage")
