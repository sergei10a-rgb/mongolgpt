...
import { npmPublishCliPackages, verifyNpmPublishAccess } from "../src/npm-publish-access"
...
function runner(input: {
  ...
  return {
    ...
    run: async (args: readonly string[]): Promise<Result> => {
      ...
      if (args[0] === "access") return { status: 0, stdout: "{}", stderr: "" }
...
describe("npm publish access preflight", () => {
  test("verifies every existing CLI package before the scoped organization", async () => {
    ...
    await expect(verifyNpmPublishAccess({ token: "secret", run: mock.run })).resolves.toEqual({
      owner: "sergei9a",
      role: "owner",
  ...
  test("fails before publication when the npm account is not a scoped organization member", async () => {
    const mock = runner({})
    await expect(verifyNpmPublishAccess({ token: "secret", run: mock.run })).rejects.toThrow(
  ...
  test("requires the automation token before contacting npm", async () => {
    const mock = runner({ role: "owner" })
    await expect(verifyNpmPublishAccess({ token: " ", run: mock.run })).rejects.toThrow("NPM_TOKEN")
