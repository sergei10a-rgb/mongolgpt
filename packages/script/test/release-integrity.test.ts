...
describe("release integrity contract", () => {
  ...
  test("uses one reusable packaged Windows desktop smoke gate", () => {
    ...
    expect(smoke).toContain("Assert-DesktopSmokeScreenshot")
    expect(smoke).toContain("PNG файл биш")
  ...
  test("uses reusable packaged and installed Windows desktop smoke gates before desktop publication", () => {
    ...
    expect(installedSmoke).toContain("Assert-DesktopSmokeScreenshot -Path $screenshotPath -Metadata $result.screenshot")
    expect(installedSmoke).toContain("$result.functional.capable -ne $true")
    expect(installedSmoke).toContain("$functionalHttp.Count -ne 15")
    ...
    expect(packagedSmoke).toContain("Assert-DesktopSmokeScreenshot -Path $screenshotPath -Metadata $result.screenshot")
    expect(packagedSmoke).toContain("$result.functional.capable -ne $true")
    expect(packagedSmoke).toContain("$functionalHttp.Count -ne 15")
    ...
    expect(packagedSmoke).toContain("$result.functional.summary.fixture.localModelInference -ne $true")
  ...
  test("builds a guarded, checksummed Windows dev preview without publishing", () => {
    ...
    expect(workflow).toContain('"mongolgpt-desktop-packaged-smoke.png"')
    expect(workflow).toContain('"mongolgpt-desktop-installed-smoke.png"')
