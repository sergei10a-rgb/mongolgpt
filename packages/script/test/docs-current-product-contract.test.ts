...
describe("documentation product contract", () => {
  ...
  test("publishes dedicated Mongolian service, privacy, billing, and admin guidance", async () => {
    ...
    expect(deployment).toContain("Бичих (`Write`; хуучин UI-д `Edit`)")
    expect(deployment).toContain("Settings → Environments → dev")
    expect(deployment).toContain("Account Resources")
