...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("mongolgpt-find")
  api.delete("mongolgpt-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("mongolgpt-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("mongolgpt-find", new Highlight(...rest))
    return true
  }
