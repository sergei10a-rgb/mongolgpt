...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      MONGOLGPT_CLI_NAME: `'${binary}'`,
      MONGOLGPT_MODELS_DEV: modelsData,
      MONGOLGPT_CHANNEL: `'${Script.channel}'`,
      MONGOLGPT_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/sergei10a-rgb/mongolgpt.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
