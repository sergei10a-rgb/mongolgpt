...
export default Runtime.handler(
  ...
  Effect.fn("cli.api")(function* (input) {
    ...
    for (const header of input.header) {
      ...
      if (index < 1) return yield* Effect.fail(new Error(`Header буруу байна, name:value хэлбэр хүлээгдсэн: ${header}`))
...
export function resolveOperation(spec: OpenApi, operationID: string, params: Record<string, string>) {
  ...
  throw new Error(`Ажиллагаа олдсонгүй: ${operationID}`)
}
...
function resolveRequest(
  ...
  if (input.length !== 1) return Effect.fail(new Error("Ажиллагааны нэр эсвэл HTTP арга ба path хүлээгдсэн"))
  return Effect.tryPromise(async () => {
    const response = await fetch(new URL("/openapi.json", transport.url), { headers: transport.headers })
    if (!response.ok) throw new Error(`OpenAPI баримтыг ачаалж чадсангүй: HTTP ${response.status}`)
...
function interpolate(path: string, params: Record<string, string>) {
  ...
  const pathname = path.replaceAll(/\{([^}]+)\}/g, (_, name: string) => {
    ...
    if (value === undefined) throw new Error(`Path параметр дутуу байна: ${name}`)
    used.add(name)
