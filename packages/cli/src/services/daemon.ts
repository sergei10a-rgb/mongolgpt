...
import { ServerAuth } from "@mongolgpt/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createMongolGPTClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createMongolGPTClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const healthy = Effect.fnUntraced(function* () {
      ...
      return yield* Effect.fail(new Error("Бүртгэгдсэн сервер эрүүл төлөвтэй биш"))
    })

    const compatible = Effect.fnUntraced(function* () {
      const info = yield* healthy()
      if (info.version === InstallationVersion) return info
      return yield* Effect.fail(new Error("Бүртгэгдсэн серверийн хувилбар client-тэй таарахгүй байна"))
    })
    ...
    const awaitStopped = Effect.fnUntraced(function* (pid: number) {
      ...
      return yield* Effect.fail(new Error(`${pid} серверийн process одоо ч ажиллаж байна`))
    })
    ...
    const start = Effect.fn("cli.daemon.start")(function* () {
      ...
      if (!compiled && entrypoint === undefined)
        return yield* Effect.fail(new Error("CLI entrypoint тодорхойлж чадсангүй"))
      yield* Effect.try({
        try: () => {
        ...
        catch: (cause) => new Error("Сервер эхлүүлж чадсангүй", { cause }),
      })

      return yield* compatible().pipe(
        Effect.retry(Schedule.spaced("50 millis").pipe(Schedule.both(Schedule.recurs(100)))),
        Effect.map((info) => info.url),
        Effect.mapError(() => new Error("Сервер эхлүүлж чадсангүй")),
      )
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createMongolGPTClient({ baseUrl: connection.url, headers: connection.headers })
    })
