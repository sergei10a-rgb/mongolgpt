...
import {
  AdminPlanMutationInput,
  type AdminPlansDependencies,
  mutateAdminPlans,
} from "../src/lib/admin-plans"
...
const limits = {
  ...
  plans: {
    basic: { weeklyCostLimit: 1, weeklyTokenLimit: 100, weeklyRequestLimit: 10, monthlyCostLimit: 4, monthlyTokenLimit: 400, monthlyRequestLimit: 40, rollingCostLimit: 1, rollingWindow: 5 },
    pro: { weeklyCostLimit: 2, weeklyTokenLimit: 200, weeklyRequestLimit: 20, monthlyCostLimit: 8, monthlyTokenLimit: 800, monthlyRequestLimit: 80, rollingCostLimit: 2, rollingWindow: 8 },
    max: { weeklyCostLimit: 3, weeklyTokenLimit: 300, weeklyRequestLimit: 30, monthlyCostLimit: 12, monthlyTokenLimit: 1200, monthlyRequestLimit: 120, rollingCostLimit: 3, rollingWindow: 12 },
  },
}

const context: PlatformAdminContext = {
  id: "adm_01ARZ3NDEKTSV4RRFFQ69G5FAV", email: "owner@mgpt.mn", subject: "access-owner", role: "owner", permissions: ["plans.manage"], requestID: "req_owner", bootstrapped: false,
}

async function source(path: string) { return Bun.file(resolve(import.meta.dir, "..", path)).text() }
...
function updateRequest() {
  return { operation: "update", expectedRevision: "0", expectedActiveStateRevision: "none", note: "Үнийн багцын хязгаарыг шинэчиллээ.", ...limits }
}
...
function dependencies(events: string[], overrides: Partial<AdminPlansDependencies> = {}): AdminPlansDependencies {
  ...
  return {
    ...
    createVersionWithDb: (async () => { events.push("create"); return { id: "01ARZ3NDEKTSV4RRFFQ69G5FAV", revision: 1 } }) as unknown as AdminPlansDependencies["createVersionWithDb"],
    cloneVersionForRollbackWithDb: (async () => { events.push("clone"); return { id: "01ARZ3NDEKTSV4RRFFQ69G5FAV", revision: 1 } }) as unknown as AdminPlansDependencies["cloneVersionForRollbackWithDb"],
    activateVersionWithDb: (async () => { events.push("activate"); return { activeVersionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV", revision: 1 } }) as AdminPlansDependencies["activateVersionWithDb"],
    writeAdminAuditWithDb: (async () => { events.push("success-audit") }) as AdminPlansDependencies["writeAdminAuditWithDb"],
    writeAdminAudit: (async () => { events.push("failure-audit") }) as AdminPlansDependencies["writeAdminAudit"],
    ...overrides,
  }
}
...
describe("admin plan management", () => {
  test("accepts a complete bounded Mongolian update and rejects invalid numeric invariants", () => {
    ...
    expect(AdminPlanMutationInput.parse(valid)).toMatchObject({ operation: "update", expectedRevision: 0, expectedActiveStateRevision: null })
    expect(AdminPlanMutationInput.safeParse({ ...valid, note: "english only" }).success).toBe(false)
    expect(AdminPlanMutationInput.safeParse({ ...valid, lite: { ...limits.lite, monthlyLimit: 4 } }).success).toBe(false)
    expect(AdminPlanMutationInput.safeParse({ ...valid, plans: { ...limits.plans, basic: { ...limits.plans.basic, monthlyCostLimit: 0 } } }).success).toBe(false)
    expect(AdminPlanMutationInput.safeParse({ ...valid, plans: { ...limits.plans, pro: { ...limits.plans.pro, weeklyCostLimit: 0 } } }).success).toBe(false)
    expect(AdminPlanMutationInput.safeParse({ ...valid, plans: { ...limits.plans, basic: { ...limits.plans.basic, rollingCostLimit: 2 } } }).success).toBe(false)
    expect(AdminPlanMutationInput.safeParse({ ...valid, plans: { ...limits.plans, max: { ...limits.plans.max, rollingWindow: 169 } } }).success).toBe(false)
    expect(AdminPlanMutationInput.safeParse({ ...valid, plans: { ...limits.plans, pro: { ...limits.plans.pro, weeklyTokenLimit: 50 } } }).success).toBe(false)
  })

  test("requires a matching confirmation and Mongolian note for immutable rollback", () => {
    const valid = { operation: "rollback", sourceVersionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV", confirmation: "БУЦААХ", expectedRevision: "4", expectedActiveStateRevision: "2", note: "Өмнөх тогтвортой хувилбар руу буцаалаа." }
  ...
  test("requires plans.manage", () => {
    ...
    expect(() => requirePlatformAdminPermission({ ...context, permissions: [] }, "plans.manage")).toThrow("эрх хүрэлцэхгүй")
  })

  test("runs update in create, activate, success-audit order inside one transaction", async () => {
    const events: string[] = []
    const result = await mutateAdminPlans(context, request(), updateRequest(), dependencies(events))
  ...
  test("runs rollback in clone, activate, success-audit order inside one transaction", async () => {
    const events: string[] = []
    const result = await mutateAdminPlans(context, request(), {
      operation: "rollback", sourceVersionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV", confirmation: "БУЦААХ", expectedRevision: "0", expectedActiveStateRevision: "none", note: "Өмнөх тогтвортой хувилбар руу буцаалаа.",
    }, dependencies(events))
    expect(result).toMatchObject({ ok: true, versionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV", revision: 1 })
  ...
  test("does not report success when the transaction audit fails and records an external failure audit", async () => {
    const events: string[] = []
    const result = await mutateAdminPlans(context, request(), updateRequest(), dependencies(events, {
      writeAdminAuditWithDb: (async () => { events.push("success-audit"); throw new Error("audit unavailable") }) as AdminPlansDependencies["writeAdminAuditWithDb"],
    }))
  ...
  test("does not call plan primitives when origin, permission, or input validation is denied", async () => {
    ...
    await mutateAdminPlans(context, request({ origin: "https://attacker.example" }), updateRequest(), dependencies(originEvents))
  ...
  test("keeps mutation ordering, bounded history, and secret-free contracts", async () => {
    ...
    expect(route).toContain("Хуучин Lite нийцтэй тохиргоо")
