...
describe("admin user management contract", () => {
  ...
  test("keeps permission, CSRF, state change, and audit in the server contract", async () => {
    ...
    expect(users).not.toContain("Database.transaction")
    expect(users).toContain("AccountAccess.transition(")
    expect(users).toContain("dependencies.batch ?? Database.batch")
    expect(users).toContain("actor: { email: admin.email, subject: admin.subject }")
    expect(users).toContain("adminAuditQuery(tx")
    expect(users).toContain("'revoked_api_keys', ${transition.revokedApiKeys}")
