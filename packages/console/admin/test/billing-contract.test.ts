...
describe("admin billing contract", () => {
  ...
  test("adds a server-derived, audited QPay cancellation action without client payment scope", async () => {
    ...
    expect(view).toContain("return adminResponse(async () =>")
    expect(view).toContain("error={cancellationSubmission.error}")
    expect(view).not.toContain("revalidate:")
  ...
  test("adds a first-class payment recovery list and a fail-closed retry detail flow", async () => {
    ...
    expect(service).toContain("retryPaymentRecovery")
    expect(service).toContain("adminAuditQuery")
    expect(service).not.toContain("Database.transaction")
