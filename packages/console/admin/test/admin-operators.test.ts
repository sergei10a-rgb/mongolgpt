...
describe("platform admin operator management", () => {
  ...
  test("keeps mutation security, native D1 guards and atomic audit in the server contract", async () => {
    ...
    expect(operators).not.toContain("Database.transaction")
    expect(operators).toContain("dependencies.batch ?? Database.batch")
    expect(operators).toContain("paymentBatchGuard")
    expect(operators).toContain("adminAuditQuery(tx")
    expect(operators).toContain("eq(PlatformAdminTable.access_subject, admin.subject)")
    expect(operators).toContain("eq(PlatformAdminTable.timeUpdated, target.timeUpdated)")
    expect(operators).toContain("changes() = 1")
    expect(operators).not.toContain("Ямар ч өөрчлөлт хийгдээгүй")
