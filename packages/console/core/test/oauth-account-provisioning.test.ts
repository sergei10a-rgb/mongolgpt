...
import { OAuthIdentityConflictError, provisionOAuthAccountIdentity } from "../src/oauth-account-provisioning"
...
async function fixture() {
  ...
  const use = <T>(callback: (tx: CoreDatabase.TxOrDb) => Promise<T>) => callback(db)
  ...
  const batch = (async (callback: (tx: CoreDatabase.TxOrDb) => readonly PromiseLike<unknown>[]) => {
    ...
    batchTail = previous.then(() => current)
    await previous
  ...
  return { sqlite, use, batch }
}

describe("OAuth account provisioning", () => {
  test("keeps first login account and identities in one batch invariant", async () => {
    const { sqlite, use, batch } = await fixture()
    ...
    expect(sqlite.query("select count(*) as count from account").get()).toEqual({ count: 1 })
    expect(sqlite.query("select count(*) as count from auth").get()).toEqual({
      count: 2,
    })
    expect(sqlite.query("select provider, subject, account_id from auth order by provider").all()).toEqual([
      {
        provider: "email",
  ...
  test("is idempotent when duplicate first-login callbacks arrive together", async () => {
    ...
    const [first, second] = await Promise.all([
      provisionOAuthAccountIdentity(input, { use, batch }),
      provisionOAuthAccountIdentity(input, { use, batch }),
    ])
    ...
    expect(
      sqlite.query("select count(*) as count from auth where account_id not in (select id from account)").get(),
    ).toEqual({
      count: 0,
    })
  })
  ...
  test("links a new provider identity to an existing email account", async () => {
    ...
    sqlite
      .query("insert into auth (id, provider, subject, account_id) values (?, 'email', ?, ?)")
    ...
    expect(sqlite.query("select count(*) as count from account").get()).toEqual({ count: 1 })
    expect(sqlite.query("select provider, account_id, time_deleted from auth order by provider").all()).toEqual([
  ...
  test("reactivates soft-deleted identities without creating an orphan account", async () => {
    const { sqlite, use, batch } = await fixture()
    sqlite
      .query("insert into auth (id, provider, subject, account_id, time_deleted) values (?, 'github', ?, ?, ?)")
      .run("auth_deleted_github", "1001", "acc_deleted", 1)
    sqlite
      .query("insert into auth (id, provider, subject, account_id, time_deleted) values (?, 'email', ?, ?, ?)")
    ...
    expect(sqlite.query("select count(*) as count from auth where time_deleted is not null").get()).toEqual({
      count: 0,
    })
  })
  ...
  test("fails closed when active provider and email identities point at different accounts", async () => {
    ...
    sqlite
      .query("insert into auth (id, provider, subject, account_id) values (?, 'github', ?, ?)")
    ...
    expect(sqlite.query("select count(*) as count from account").get()).toEqual({ count: 2 })
    expect(sqlite.query("select count(*) as count from auth").get()).toEqual({
      count: 2,
    })
  })
