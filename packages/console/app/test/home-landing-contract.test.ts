...
describe("home landing contract", () => {
  test("keeps the MongolGPT-first hero and real product screenshot", async () => {
    ...
    expect(view).toContain('data-component="preview-notice"')
    expect(view).toContain("Хөгжүүлэлтийн туршилтын орчин")
    expect(view).toContain("Бодит")
    expect(view).toContain("төлбөр идэвхгүй")
    expect(view).not.toContain("Үнэгүй автомат горимоор шууд эхэлнэ")
  ...
  test("uses the new home layout structure and keyboard-visible focus styling", async () => {
    ...
    expect(styles).toContain('[data-component="preview-notice"]')
