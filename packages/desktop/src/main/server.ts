...
const SIDECAR_SERVICE_NAME = "MongolGPT server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    MONGOLGPT_EXPERIMENTAL_ICON_DISCOVERY: "true",
    MONGOLGPT_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function spawnLocalServer(
  ...
  const child = utilityProcess.fork(sidecar, [], {
    cwd: process.cwd(),
    env: createSidecarEnv(process.env, { packaged: app.isPackaged, platform: process.platform }),
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.postMessage({
      ...
      desktopSmokeProof: options.desktopSmokeProof,
    })
  }).catch((error) => {
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`mongolgpt:${password}`).toString("base64")
