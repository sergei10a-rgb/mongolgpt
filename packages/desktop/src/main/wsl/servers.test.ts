...
let releaseMongolGPTResolve: (() => void) | undefined
...
test("restarts an existing distro server after updating MongolGPT", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.mongolgpt/bin/mongolgpt",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, mongolgptChecks: {} })
})
...
test("ignores stale background MongolGPT checks after removing a WSL server", async () => {
  persistedServers = []
  releaseMongolGPTResolve = undefined
  ...
  await waitFor(() => !!releaseMongolGPTResolve)
  await controller.removeServer("wsl:Debian")
  releaseMongolGPTResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().mongolgptChecks).toEqual({})
})

test("ignores stale startup MongolGPT checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveMongolGPT: async () => {
      await new Promise<void>((resolve) => {
        releaseMongolGPTResolve = resolve
      })
      return "/home/me/.mongolgpt/bin/mongolgpt"
    },
  }
}
