...
import { expectMongolGPTVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveMongolGPT?: typeof resolveWslMongolGPT
...
export function createWslServersController(
  ...
  const setMongolGPTCheck = (distro: string, check: WslMongolGPTCheck) => {
    setState({
      mongolgptChecks: {
        ...state.mongolgptChecks,
        [distro]: check,
      },
    })
  }

  const checkMongolGPT = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveMongolGPT ?? resolveWslMongolGPT)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return mongolgptCheck(distro, resolved, version, appVersion)
  }

  const refreshMongolGPTCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setMongolGPTCheck(distro, await checkMongolGPT(distro, opts))
  }
  ...
  const refreshMongolGPTCheckBackground = (id: string, distro: string) => {
    void checkMongolGPT(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setMongolGPTCheck(distro, check)
      })
      .catch((error) => {
  ...
  const refreshMongolGPTChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkMongolGPT(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setMongolGPTCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
  ...
  return {
    ...
    async probeMongolGPT(name: string) {
      await runJob({ kind: "probe-mongolgpt", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshMongolGPTCheck(name, { signal: abort.signal })
      })
    },

    async installMongolGPT(name: string) {
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.mongolgptChecks, distro) : {}),
      })
    },
...
function mongolgptCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "MongolGPT энэ distro дотор суусангүй",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "MongolGPT суусан боловч ажиллаж чадсангүй",
    }
  }
  return {
    distro,
