...
export async function installWslMongolGPT(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      [
...
export async function resolveWslMongolGPT(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.mongolgpt/bin/mongolgpt" ]; then printf "%s\\n" "$HOME/.mongolgpt/bin/mongolgpt"; fi',
        distro,
        opts,
      )
    ).stdout,
