...
import { type WslCommandLine, resolveWslMongolGPT, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export MONGOLGPT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export MONGOLGPT_CLIENT=desktop",
    "export MONGOLGPT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export MONGOLGPT_CLIENT=desktop",
    `export MONGOLGPT_SERVER_USERNAME=${shellEscape(username)}`,
    `export MONGOLGPT_SERVER_PASSWORD=${shellEscape(password)}`,
    `export MONGOLGPT_SERVER_USERNAME=${shellEscape(username)}`,
    `export MONGOLGPT_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(mongolgpt)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
