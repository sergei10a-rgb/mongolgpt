...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "org.mongolgpt.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `mongolgpt.settings.dat` is special: it maps to the `mongolgpt.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"mongolgpt.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // mongolgpt.settings.dat → the electron settings store ("mongolgpt.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "mongolgpt.global.dat").
  const storeName = filename === "mongolgpt.settings.dat" ? "mongolgpt.settings" : filename
