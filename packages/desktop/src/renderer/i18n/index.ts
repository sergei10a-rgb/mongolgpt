...
import { dict as appMn } from "../../../../app/src/i18n/mn"
...
function migrateLocaleValue(value: unknown) {
  const record = parseRecord(value)
  if (!record) return value
  if (record.locale === "en" && record.source !== "user" && record.defaultLocale !== DEFAULT_LOCALE) {
    return { ...record, locale: DEFAULT_LOCALE, defaultLocale: DEFAULT_LOCALE }
  }
  return value
}
...
function build(locale: Locale): Dictionary {
  ...
  if (locale === "mn") return { ...base, ...i18n.flatten(appMn), ...i18n.flatten(desktopMn) }
...
export function initI18n(): Promise<Locale> {
  ...
  const promise = (async () => {
    const raw = await window.api.storeGet("mongolgpt.global.dat", "language").catch(() => null)
    const value = migrateLocaleValue(parseStored(raw))
