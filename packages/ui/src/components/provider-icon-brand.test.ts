...
describe("MongolGPT provider icon", () => {
  test("keeps the current terminal mark in the source asset and generated sprite", () => {
    ...
    for (const source of [asset, symbol ?? ""]) {
      ...
      expect(source).toContain('d="M8.35 7.88L13.78 12L8.35 16.13"')
      expect(source).toContain('stroke="#26E6F2"')
      expect(source).toContain('fill="#37F28B"')
