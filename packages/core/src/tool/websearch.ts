...
export class ConfigService extends Context.Service<ConfigService, Config>()("@mongolgpt/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () => {
  const provider = env("MONGOLGPT_WEBSEARCH_PROVIDER")
  return ConfigService.of({
    provider: provider === "exa" || provider === "parallel" ? provider : undefined,
    enableExa:
      truthy("MONGOLGPT_EXPERIMENTAL") || truthy("MONGOLGPT_ENABLE_EXA") || truthy("MONGOLGPT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("MONGOLGPT_ENABLE_PARALLEL") || truthy("MONGOLGPT_EXPERIMENTAL_PARALLEL"),
