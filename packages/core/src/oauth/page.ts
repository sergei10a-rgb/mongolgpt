...
// etc.).
//
// The visual language mirrors the MongolGPT app: the design tokens are a curated
...
export function bootstrap(options: BootstrapOptions) {
  return renderDocument({
    title: "Нэвтрэлтийг дуусгаж байна",
    body: renderCard({
      status: "pending",
      headline: "Нэвтрэлтийг дуусгаж байна",
      message: options.provider
        ? `${escapeHtml(options.provider)} зөвшөөрлийг дуусгаж байна.`
        : "Зөвшөөрлийг дуусгаж байна.",
      footnote: "Нэвтрэлт дуусмагц энэ цонхыг хааж болно.",
    }),
...
<html lang="mn">
  <head>
    ...
    <title>${escapeHtml(input.title)} - MongolGPT</title>
    <style>${STYLES}</style>
  </head>
  <body>
