...
import { Entry, Match } from "@mongolgpt/schema/filesystem"
...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/v2/FileSystem/Search") {}
...
export const fffLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    }).pipe(Effect.catch((error) => Effect.logWarning("fff-г эхлүүлж чадсангүй", { error }).pipe(Effect.as(undefined))))
    if (!result?.ok) {
...
const layer = Layer.unwrap(
  Effect.sync(() => (Flag.MONGOLGPT_DISABLE_FFF || !Fff.available() ? ripgrepLayer : fffLayer)),
)
