...
export function fileLogger(file = path.join(Global.Path.log, "mongolgpt.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = env("MONGOLGPT_LOG_LEVEL")?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return env("MONGOLGPT_PRINT_LOGS") === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
