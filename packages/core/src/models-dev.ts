...
declare const MONGOLGPT_MODELS_DEV: Record<string, Provider> | undefined
...
export class Service extends Context.Service<Service, Interface>()("@mongolgpt/ModelsDev") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.MONGOLGPT_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.MONGOLGPT_MODELS_PATH ?? filepath).pipe(
      Effect.catch((error) => {
        if (
          Flag.MONGOLGPT_MODELS_PATH === undefined &&
    ...
    const loadSnapshot = Effect.sync(() =>
      typeof MONGOLGPT_MODELS_DEV === "undefined" ? undefined : MONGOLGPT_MODELS_DEV,
    )
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.MONGOLGPT_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent MongolGPT CLIs can race on this cache file.
    ...
    const refresh = Effect.fn("ModelsDev.refresh")(function* (force = false) {
      ...
      yield* Effect.scoped(
        ...
        Effect.tapCause((cause) => Effect.logError("models.dev-ээс загвар татаж чадсангүй", { cause: cause })),
        Effect.ignore,
      )
    })
