...
describe("MongolGPTPlugin", () => {
  ...
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("mongolgpt")))?.methods).toEqual([
        {
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("mongolgpt"),
            ),
          )
  ...
  it.effect("uses MONGOLGPT_API_KEY as credentials", () =>
    withEnv({ MONGOLGPT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
      ...
      Effect.gen(function* () {
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("mongolgpt"),
            method: { type: "env", names: ["CUSTOM_MONGOLGPT_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("ignores non-mongolgpt providers and models", () =>
    withEnv({ MONGOLGPT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the mongolgpt small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
