...
import { GoogleVertexPlugin } from "@mongolgpt/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.mongolgpt, (provider) => {
          provider.api = {
            ...
            url: "https://example.invalid/gateway/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.mongolgpt))
