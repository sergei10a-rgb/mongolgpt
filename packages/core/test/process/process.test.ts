      ...
      Effect.gen(function* () {
        ...
        if (Exit.isFailure(exit)) {
          ...
          if (reason && reason._tag === "Fail") {
            ...
            expect((reason.error as AppProcess.AppProcessError).message).toContain("Команд амжилтгүй боллоо (гарах код 1)")
          } else {
    ...
    if (process.platform !== "win32") {
      it.live(
        ...
        Effect.acquireUseRelease(
          Effect.promise(() => fs.mkdtemp(path.join(tmpdir(), "mongolgpt-process-timeout-"))),
          (directory) => {
      ...
      it.live(
        ...
        Effect.acquireUseRelease(
          Effect.promise(() => fs.mkdtemp(path.join(tmpdir(), "mongolgpt-process-interrupt-"))),
          (directory) => {
            const ready = path.join(directory, "ready")
            const settled = path.join(directory, "settled")
            const script = `const fs=require('fs');fs.writeFileSync(${JSON.stringify(ready)},String(process.pid));process.on('SIGTERM',()=>{fs.writeFileSync(${JSON.stringify(settled)},'settled');process.exit(0)});setInterval(()=>{},60000)`
