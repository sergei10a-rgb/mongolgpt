...
describe("Ripgrep", () => {
  ...
  it.live.serial("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".mongolgpt")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".mongolgpt", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".mongolgpt/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".mongolgpt/config"))
