...
const mongolgptClient = process.env.MONGOLGPT_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (mongolgptClient === undefined) delete process.env.MONGOLGPT_CLIENT
  else process.env.MONGOLGPT_CLIENT = mongolgptClient
})

describe("resource", () => {
  test("parses and decodes OTEL resource attributes", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "service.namespace=sergei10a-rgb,team=platform%2Cobservability,label=hello%3Dworld,key%2Fname=value%20here"

    expect(resource().attributes).toMatchObject({
      "service.namespace": "sergei10a-rgb",
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES = "service.namespace=sergei10a-rgb,broken"

    expect(resource().attributes["service.namespace"]).toBeUndefined()
    expect(resource().attributes["mongolgpt.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.MONGOLGPT_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "mongolgpt.client=web,service.instance.id=override,service.namespace=sergei10a-rgb"

    expect(resource().attributes).toMatchObject({
      "mongolgpt.client": "cli",
      "service.namespace": "sergei10a-rgb",
    })
    expect(resource().attributes["service.instance.id"]).not.toBe("override")
    expect(resource().attributes["mongolgpt.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mongolgpt-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "mongolgpt.log")
