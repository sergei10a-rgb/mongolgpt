...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@mongolgpt/acme")).toBe("@mongolgpt/acme")
    expect(Npm.sanitize("@mongolgpt/acme@1.0.0")).toBe("@mongolgpt/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/mongolgpt/acme.git"
    const expected = win ? "acme@git+https_//github.com/mongolgpt/acme.git" : spec
