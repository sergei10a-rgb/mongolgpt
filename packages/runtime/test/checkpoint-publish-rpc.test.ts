...
describe("checkpoint file publication RPC", () => {
  ...
  test("maps lease and publisher failures to the existing sanitized errors", async () => {
    for (const [error, status, code] of [
      ...
      await expectFailure(await fixture.handle(json("/v1/publish-files", publication())), status, code)
    }
    await expectFailure(
...
function handler(overrides: Partial<Stores> = {}, tenant = scope) {
  ...
  const stores: Stores = {
    history: { checkpoint: unreadable, epoch: unreadable, fileRevision: unreadable },
    masterKeyJson,
    backups: {
      open: unreadable,
  ...
  return { stores, handle: createCheckpointHandler(stores, tenant), uploads, publications }
}
