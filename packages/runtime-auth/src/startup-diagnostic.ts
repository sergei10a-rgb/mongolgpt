...
export function parseStartupDiagnostic(input: unknown): StartupDiagnostic | undefined {
  try {
    ...
    const fields = ["phase", "code", "overlay", "workspaceMount"]
    ...
    const { phase, code, overlay, workspaceMount } = Object.fromEntries(
    ...
    return { phase: safePhase, code: safeCode, overlay, workspaceMount }
  } catch {
    return undefined
  }
}
