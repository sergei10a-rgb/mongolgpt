---
title: Server
description: Interact with mongolgpt server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `mongolgpt serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an mongolgpt client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `mongolgpt.local` |
...
Set `MONGOLGPT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `mongolgpt`, or set `MONGOLGPT_SERVER_USERNAME` to override it. This applies to both `mongolgpt serve` and `mongolgpt web`.
...
When you run `mongolgpt` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the mongolgpt server to interact with mongolgpt programmatically.
:::

This architecture lets mongolgpt support multiple clients and allows you to interact with mongolgpt programmatically.

You can run `mongolgpt serve` to start a standalone server. If you have the
mongolgpt TUI running, `mongolgpt serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the MongolGPT [IDE](/docs/ide) plugins.
...
The mongolgpt server exposes the following APIs.
