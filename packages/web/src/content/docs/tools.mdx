...
Tools allow the LLM to perform actions in your codebase. MongolGPT comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in MongolGPT.
...
This tool allows the LLM to run terminal commands like
pm install`, `git status`, or any other shell command.
...
:::note
This tool is only available when `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (or `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the MongolGPT provider or when the `MONGOLGPT_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching MongolGPT:
...
For example, this `.ignore` file allows ripgrep to search within
ode_modules/`, `dist/`, and `build/` directories even if they're listed in `.gitignore`.
