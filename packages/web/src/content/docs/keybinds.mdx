...
MongolGPT has a list of keybinds that you can customize through `tui.json`.
...
- `terminal_suspend` is forced to
one` because native Windows terminals do not support POSIX suspend.
  :::
...
MongolGPT uses a `leader` key for many keybinds. This avoids conflicts in your terminal.

By default, `ctrl+x` is the leader key and many actions require you to first press the leader key and then the shortcut. For example, to start a new session you first press `ctrl+x` and then press
`.
...
`leader_timeout` controls how long MongolGPT waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The MongolGPT desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `mongolgpt.json`.
