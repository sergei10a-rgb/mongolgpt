...
Puoi aggiungere strumenti esterni a MongolGPT usando il _Model Context Protocol_, o MCP. MongolGPT supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [MongolGPT Config](https://mongolgpt.duckdns.org/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/mongolgpt`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
MongolGPT gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, MongolGPT:
...
Se il server richiede autenticazione, MongolGPT ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `mongolgpt mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, MongolGPT memorizzera' i token in modo sicuro in `~/.local/share/mongolgpt/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in MongolGPT insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione MongolGPT come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare MongolGPT al tuo account Sentry.
