...
Crea `.mongolgpt/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di MongolGPT o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di MongolGPT:
...
- Globale: `~/.config/mongolgpt/commands/`
- Per progetto: `.mongolgpt/commands/`
...
mongolgpt include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
