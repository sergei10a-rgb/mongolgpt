---
title: Server
description: Interagisci con il server mongolgpt via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `mongolgpt serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client mongolgpt.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `mongolgpt.local` |
...
Imposta `MONGOLGPT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `mongolgpt`, oppure imposta `MONGOLGPT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `mongolgpt serve` sia per `mongolgpt web`.
...
Quando esegui `mongolgpt` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server mongolgpt per interagire con mongolgpt in modo programmatico.
:::

Questa architettura permette a mongolgpt di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `mongolgpt serve` per avviare un server standalone. Se la TUI di mongolgpt e gia in esecuzione, `mongolgpt serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di MongolGPT.
...
Il server mongolgpt espone le seguenti API.
