...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. MongolGPT include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in MongolGPT.
...
Questo strumento permette all'LLM di eseguire comandi da terminale come
pm install`, `git status` o qualunque altro comando di shell.
...
:::note
Questo strumento e' disponibile solo quando `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (oppure `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider MongolGPT o quando la variabile d'ambiente `MONGOLGPT_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di MongolGPT:
...
Per esempio, questo file `.ignore` permette a ripgrep di cercare dentro
ode_modules/`, `dist/` e `build/` anche se sono elencate in `.gitignore`.
