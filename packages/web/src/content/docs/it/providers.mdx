---
title: Provider
description: Usa qualsiasi provider LLM in MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di MongolGPT.
...
MongolGPT Zen e una lista di modelli forniti dal team MongolGPT, testati e
verificati per funzionare bene con MongolGPT. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da MongolGPT Zen.
:::

1. Esegui `/connect` nella TUI, seleziona mongolgpt e vai su [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/auth).
...
Funziona come qualsiasi altro provider in MongolGPT ed e totalmente opzionale.
...
Per usare Amazon Bedrock con MongolGPT:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui mongolgpt:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `mongolgpt.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in MongolGPT non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare mongolgpt per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
-
pm` specifica il pacchetto da utilizzare per questo provider. Qui viene usato `@ai-sdk/openai-compatible` per qualsiasi API compatibile OpenAI.
-
ame` è il nome visualizzato del provider nell'interfaccia.
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché mongolgpt funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di mongolgpt.
...
salvare il token nello storage di auth di mongolgpt.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
MongolGPT usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare MongolGPT
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `mongolgpt.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di MongolGPT GitLab auth adapter.

##### Configurazione

Personalizza tramite `mongolgpt.json`:
...
Per usare il tuo abbonamento GitHub Copilot con mongolgpt:
...
Per usare Google Vertex AI con MongolGPT:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui mongolgpt.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da mongolgpt, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin local Helicone session plugin per loggare automaticamente ogni conversazione di MongolGPT come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di MongolGPT elencata come sessione separata.
...
Puoi configurare mongolgpt per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
-
pm` specifica il pacchetto da usare per questo provider. Qui, `@ai-sdk/openai-compatible` è usato per qualsiasi API OpenAI-compatible.
-
ame` è il nome visualizzato per il provider nella UI.
...
Puoi configurare mongolgpt per usare modelli locali tramite LM Studio.
...
Puoi configurare mongolgpt per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per MongolGPT. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/mongolgpt) per dettagli.
:::
...
:::tip
Se le chiamate agli strumenti non funzionano, prova ad aumentare
um_ctx` in Ollama. Inizia da circa 16k - 32k.
:::
...
Per usare Ollama Cloud con MongolGPT:
...
3. Copia la chiave API da usare in MongolGPT.
...
6. **Importante**: Prima di usare modelli cloud in MongolGPT, devi scaricare le informazioni del modello localmente:
...
MongolGPT Zen è una lista di modelli testati e verificati forniti dal team MongolGPT. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>MongolGPT Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di MongolGPT.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di mongolgpt.
...
5. Puoi anche personalizzarli tramite la configurazione di mongolgpt. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con MongolGPT:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di mongolgpt. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con mongolgpt. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `mongolgpt.json` nella directory del progetto:
...
I campi `limit` permettono a MongolGPT di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `mongolgpt auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di mongolgpt e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config mongolgpt.
