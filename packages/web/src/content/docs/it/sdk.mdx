---
title: SDK
description: Client JS type-safe per il server mongolgpt.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

L'SDK JS/TS di mongolgpt fornisce un client type-safe per interagire con il server.
Usalo per creare integrazioni e controllare mongolgpt in modo programmatico.
...
| `port`     |
umber`      | Porta del server               | `4096`      |
| `signal`   | `AbortSignal` | Segnale di abort per annullare | `undefined` |
| `timeout`  |
umber`      | Timeout in ms per avvio server | `5000`      |
...
Puoi passare un oggetto di configurazione per personalizzare il comportamento. L'istanza legge comunque `mongolgpt.json`, ma puoi sovrascrivere o aggiungere configurazione inline:
...
Se hai gia un'istanza di mongolgpt in esecuzione, puoi creare un client per collegarti:
...
| `retryCount` |
umber`        | Opzionale. Numero di tentativi di validazione (default: 2)            |
