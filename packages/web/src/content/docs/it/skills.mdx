...
Le skill degli agenti permettono a MongolGPT di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
MongolGPT cerca in queste posizioni:

- Config di progetto: `.mongolgpt/skills/<name>/SKILL.md`
- Config globale: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, MongolGPT risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.mongolgpt/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.mongolgpt/skills/git-release/SKILL.md` cosi':
...
MongolGPT elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `mongolgpt.json`:
...
**Per agenti integrati** (in `mongolgpt.json`):
...
2. Controlla che il frontmatter includa
ame` e `description`
