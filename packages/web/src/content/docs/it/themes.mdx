...
Con MongolGPT puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, MongolGPT usa il tema `mongolgpt`.
...
MongolGPT include diversi temi integrati.
...
|
ord`                 | Basato sul tema [Nord](https://github.com/nordtheme/nord)                 |
...
- **Preserva i default del terminale**: usa
one` per testo e sfondo per mantenere l'aspetto nativo del terminale.

Il tema di sistema è pensato per chi:

- Vuole che MongolGPT corrisponda all'aspetto del terminale
...
MongolGPT supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/mongolgpt/themes/*.json` o `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.mongolgpt/themes/*.json`
4. **Directory di lavoro corrente** - in `./.mongolgpt/themes/*.json`
