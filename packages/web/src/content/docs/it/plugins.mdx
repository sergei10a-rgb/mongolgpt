---
title: Plugin
description: Scrivi plugin per estendere MongolGPT.
---

I plugin ti permettono di estendere MongolGPT agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di MongolGPT.
...
- `.mongolgpt/plugins/` - plugin a livello progetto
- `~/.config/mongolgpt/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/mongolgpt/node_modules/`.
...
1. Config globale (`~/.config/mongolgpt/mongolgpt.json`)
2. Config di progetto (`mongolgpt.json`)
3. Directory plugin globale (`~/.config/mongolgpt/plugins/`)
4. Directory plugin di progetto (`.mongolgpt/plugins/`)
...
MongolGPT esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di mongolgpt per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere mongolgpt.
...
:::note
Se usi l'app desktop di MongolGPT, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a mongolgpt di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a mongolgpt:
...
L'helper `tool` crea uno strumento personalizzato che mongolgpt può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in mongolgpt insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://mongolgpt.duckdns.org/docs/sdk) per i dettagli.
