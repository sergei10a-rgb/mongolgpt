...
Per diagnosticare problemi con MongolGPT, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\mongolgpt\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `mongolgpt --log-level DEBUG`.

---

## Archiviazione

mongolgpt salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/mongolgpt/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\mongolgpt`
...
MongolGPT Desktop esegue in background un server locale di MongolGPT (il sidecar `mongolgpt-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `MongolGPT` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/mongolgpt/mongolgpt.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\mongolgpt\mongolgpt.jsonc`
...
MongolGPT puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/mongolgpt/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\mongolgpt\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che MongolGPT possa ricostruirla.

1. Chiudi completamente MongolGPT Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/mongolgpt`
- **Linux**: elimina `~/.cache/mongolgpt` (oppure esegui `rm -rf ~/.cache/mongolgpt`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\mongolgpt`

3. Riavvia MongolGPT Desktop.

---

### Risolvi problemi di connessione al server

MongolGPT Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `mongolgpt.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `MONGOLGPT_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `MONGOLGPT_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, MongolGPT Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di MongolGPT.

---

### Notifiche non visualizzate

MongolGPT Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per MongolGPT nelle impostazioni del sistema operativo, e
...
1. Chiudi MongolGPT Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app MongolGPT Desktop):

- `mongolgpt.settings.dat` (desktop default server URL)
- `mongolgpt.global.dat` e `mongolgpt.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con MongolGPT:
...
Per capire a quali modelli hai accesso, esegui `mongolgpt models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\mongolgpt`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. mongolgpt installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\mongolgpt`

2. Riavvia mongolgpt per reinstallare i pacchetti provider piu recenti

Questo forzera mongolgpt a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
mongolgpt rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
