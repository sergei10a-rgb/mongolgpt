---
title: Providers
description: Using any LLM provider in MongolGPT.
---

import config from "../../../config.mjs"
export const console = config.console

MongolGPT uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your MongolGPT config.
...
in `~/.local/share/mongolgpt/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your MongolGPT
config.
...
MongolGPT Zen is a list of models provided by the MongolGPT team that have been
tested and verified to work well with MongolGPT. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with MongolGPT Zen.
:::

1. Run the `/connect` command in the TUI, select `MongolGPT Zen`, and head to [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
It works like any other provider in MongolGPT and is completely optional to use.

---

## MongolGPT Go

MongolGPT Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the MongolGPT team that have been
tested and verified to work well with MongolGPT.

1. Run the `/connect` command in the TUI, select `MongolGPT Go`, and head to [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
To use Amazon Bedrock with MongolGPT:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running mongolgpt:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `mongolgpt.json`:
...
MongolGPT. Anthropic explicitly prohibits this.

Previous versions of MongolGPT came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in MongolGPT with zero setup:
...
You can configure mongolgpt to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
-
pm` specifies the package to use for this provider. Here, `@ai-sdk/openai-compatible` is used for any OpenAI-compatible API.
-
ame` is the display name for the provider in the UI.
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for mongolgpt to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your mongolgpt config.
...
MongolGPT supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; MongolGPT uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize MongolGPT. Sign in and approve.

   :::note
   MongolGPT requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, MongolGPT surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — MongolGPT forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in MongolGPT is experimental. Features, configuration, and
behavior may change in future releases.
:::

MongolGPT integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in mongolgpt auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
MongolGPT uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock MongolGPT
to only use your own GitLab-hosted instance, add the following to your
`mongolgpt.json` file. It is also recommended to disable session sharing.
...
More documentation on MongolGPT GitLab auth adapter homepage.

##### Configuration

Customize through `mongolgpt.json`:
...
When a `duo-workflow-*` model is selected, MongolGPT will:
...
4. Route tool execution requests through MongolGPT's permission-gated tool system
...
To use your GitHub Copilot subscription with mongolgpt:
...
To use GMI Cloud with MongolGPT:
...
To use Google Vertex AI with MongolGPT:
...
2. Set the required environment variables:
   ...
   Set them while running mongolgpt.
...
In the event you see a feature or model from Helicone that isn't configured automatically through mongolgpt, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the local Helicone session plugin to automatically log each MongolGPT conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each MongolGPT conversation listed as a separate session.
...
You can configure mongolgpt to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure mongolgpt to use local models through LM Studio.
...
You can configure mongolgpt to use local models through Ollama.

:::tip
Ollama can automatically configure itself for MongolGPT. See the [Ollama integration docs](https://docs.ollama.com/integrations/mongolgpt) for details.
:::
...
:::tip
If tool calls aren't working, try increasing
um_ctx` in Ollama. Start around 16k - 32k.
:::
...
To use Ollama Cloud with MongolGPT:
...
3. Copy the API key for use in MongolGPT.
...
6. **Important**: Before using cloud models in MongolGPT, you must pull the model information locally:
...
MongolGPT Zen is a list of tested and verified models provided by the MongolGPT team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>MongolGPT Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Enter your MongolGPT API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your mongolgpt config.
...
5. You can also customize them through your mongolgpt config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with MongolGPT:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=mongolgpt&utm_medium=docs&utm_campaign=cortex-provider).

mongolgpt's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. MongolGPT captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting mongolgpt:
...
The model catalog is provided automatically. A minimal `mongolgpt.json` is all that's needed:
...
You can also customize models through your mongolgpt config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. MongolGPT opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
MongolGPT refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when MongolGPT is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running MongolGPT — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. MongolGPT prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. MongolGPT polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with mongolgpt. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `mongolgpt.json` file in your project directory:
...
The `limit` fields allow MongolGPT to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `mongolgpt auth list` to see if the credentials
...
2. For custom providers, check the mongolgpt config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your mongolgpt config.
