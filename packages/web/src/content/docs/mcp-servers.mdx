...
You can add external tools to MongolGPT using the _Model Context Protocol_, or MCP. MongolGPT supports both local and remote servers.
...
You can define MCP servers in your [MongolGPT Config](https://mongolgpt.duckdns.org/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/mongolgpt` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
MongolGPT automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, MongolGPT will:
...
If the server requires authentication, MongolGPT will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `mongolgpt mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MongolGPT will store the tokens securely in `~/.local/share/mongolgpt/mcp-auth.json`.
...
Your MCPs are available as tools in MongolGPT, alongside built-in tools. So you can manage them through the MongolGPT config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect MongolGPT to your Sentry account.
