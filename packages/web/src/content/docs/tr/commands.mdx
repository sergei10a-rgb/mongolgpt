...
`.mongolgpt/commands/test.md` oluştur:
...
mongolgpt işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

mongolgpt [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/mongolgpt/commands/`
- Proje başına: `.mongolgpt/commands/`
...
mongolgpt, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
