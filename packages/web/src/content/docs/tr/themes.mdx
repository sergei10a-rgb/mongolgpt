...
mongolgpt ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak mongolgpt kendi `mongolgpt` temasını kullanır.
...
mongolgpt birden fazla yerleşik temayla gelir.
...
|
ord`                 | [Nord](https://github.com/nordtheme/nord) temasını temel alır                 |
...
- **Terminal varsayılanlarını korur**: Metin ve arka plan renklerinde
one` kullanarak terminalin yerel görünümünü korur

Sistem teması şu kullanıcılar için idealdir:

- mongolgpt'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
mongolgpt, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/mongolgpt/themes/*.json` veya `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.mongolgpt/themes/*.json`
4. **Geçerli çalışma dizini** - `./.mongolgpt/themes/*.json`
