...
MongolGPT Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, MongolGPT'daki diğer sağlayıcılar gibi çalışır. MongolGPT Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve MongolGPT'u kullanmak için bunu kullanmanıza gerek yoktur.
...
MongolGPT Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

MongolGPT Go, MongolGPT'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>MongolGPT Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `MongolGPT Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye MongolGPT Go'ya abone olabilir.
:::
...
MongolGPT Go aşağıdaki limitleri içerir:
...
MongolGPT yapılandırmanızdaki [model id](/docs/config/#models) formatı `mongolgpt-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `mongolgpt-go/kimi-k2.7-code` kullanmalısınız.
...
MongolGPT Go'yu şu amaçlarla oluşturduk:
...
4. MongolGPT ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
