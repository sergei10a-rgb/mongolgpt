...
description: MongolGPT, LSP sunucularınızla bütünleşir.
---

MongolGPT, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

MongolGPT, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`MONGOLGPT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve mongolgpt bir dosyayı açtığında:
...
LSP sunucularını, mongolgpt yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
