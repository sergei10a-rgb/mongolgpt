---
title: TUI
description: MongolGPT terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MongolGPT, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

MongolGPT'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
MongolGPT TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
MongolGPT'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
MongolGPT'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `mongolgpt.json` dosyasından ayrıdır.
...
- `leader_timeout` - MongolGPT'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
Özel bir TUI yapılandırma yolu yüklemek için `MONGOLGPT_TUI_CONFIG` kullanın.

### Attention

Attention, MongolGPT bir yanıt beklediğinde, izin onayı gerektiğinde, oturum hatası oluştuğunda veya oturum tamamlandığında TUI'nin sizi ses ve masaüstü bildirimiyle uyarmasını sağlar. `attention.enabled` ile etkinleştirin; yerleşik olaylar gerçekleştiğinde ses çalar. Masaüstü bildirimleri yalnızca terminal penceresi odakta değilken gönderilir ve subagent olayları için kullanılmaz.

- `enabled` - Attention için tüm bildirimleri ve sesleri etkinleştirir. Varsayılan değer `false`.
-
otifications` - Attention etkin olduğunda TUI'nin terminal üzerinden masaüstü bildirimi göndermesine izin verir. Varsayılan değer `true`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `mongolgpt.default`.
