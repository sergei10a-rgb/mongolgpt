---
title: Eklentiler
description: MongolGPT'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek MongolGPT'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya MongolGPT'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.mongolgpt/plugins/` - Proje düzeyinde eklentiler
- `~/.config/mongolgpt/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/mongolgpt/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/mongolgpt/mongolgpt.json`)
2. Project config (`mongolgpt.json`)
3. Global eklenti dizini (`~/.config/mongolgpt/plugins/`)
4. Proje eklenti dizini (`.mongolgpt/plugins/`)
...
MongolGPT bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir MongolGPT SDK istemcisi.
...
MongolGPT'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
MongolGPT masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
MongolGPT'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca MongolGPT'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, MongolGPT'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra MongolGPT'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://mongolgpt.duckdns.org/docs/sdk) bakın.
