...
Ajan becerileri, mongolgpt'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
mongolgpt su konumlari tarar:

- Proje konfigurasyonu: `.mongolgpt/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
Proje yerel yollarinda mongolgpt, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.mongolgpt/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.mongolgpt/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
mongolgpt, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`mongolgpt.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`mongolgpt.json` icinde):
...
2. Frontmatter icinde
ame` ve `description` oldugunu kontrol edin
