...
_Model Bağlam Protokolü_ veya MCP kullanarak mongolgpt'a harici araçlar ekleyebilirsiniz. mongolgpt hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [mongolgpt Config](https://mongolgpt.duckdns.org/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/mongolgpt` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
mongolgpt, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde mongolgpt şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa mongolgpt, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `mongolgpt mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından mongolgpt, belirteçleri `~/.local/share/mongolgpt/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra mongolgpt'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi mongolgpt yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve mongolgpt'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
