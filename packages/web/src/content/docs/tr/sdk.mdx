---
title: SDK
description: mongolgpt sunucusu için type-safe JS istemcisi.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

mongolgpt JS/TS SDK, sunucu ile etkileşmek için type-safe bir istemci sağlar.
mongolgpt'u programatik olarak kontrol etmek ve entegrasyonlar oluşturmak için kullanabilirsiniz.
...
Bir mongolgpt örneği oluşturun:
...
| `port`     |
umber`      | Sunucu bağlantı noktası                       | `4096`      |
| `signal`   | `AbortSignal` | İptal için durdurma sinyali                   | `undefined` |
| `timeout`  |
umber`      | Sunucu başlatma için ms cinsinden zaman aşımı | `5000`      |
...
Davranışı özelleştirmek için bir yapılandırma nesnesi iletebilirsiniz. Örnek yine de `mongolgpt.json` dosyanızı alır, ancak yapılandırmayı satır içi olarak geçersiz kılabilir veya ekleyebilirsiniz:
...
Halihazırda çalışan bir mongolgpt örneğiniz varsa, ona bağlanmak için bir istemci örneği oluşturabilirsiniz:
...
| `retryCount` |
umber`        | İsteğe bağlı. Doğrulama yeniden deneme sayısı (varsayılan: 2) |
