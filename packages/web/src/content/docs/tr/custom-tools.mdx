---
title: Özel Araçlar
description: LLM'un mongolgpt'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. mongolgpt'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.mongolgpt/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/mongolgpt/tools/` içine yerleştirerek.
