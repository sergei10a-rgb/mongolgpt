---
title: Sunucu
description: mongolgpt sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`mongolgpt serve` komutu, mongolgpt istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `mongolgpt.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `MONGOLGPT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `mongolgpt` değeridir; değiştirmek isterseniz `MONGOLGPT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `mongolgpt serve` hem de `mongolgpt web` için geçerlidir.
...
`mongolgpt` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
mongolgpt ile programatik etkileşim için mongolgpt sunucusunu kullanın.
:::

Bu mimari, mongolgpt'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`mongolgpt serve` ile bağımsız bir sunucu başlatabilirsiniz. mongolgpt TUI açıksa, `mongolgpt serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum mongolgpt [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
mongolgpt sunucusu aşağıdaki API'leri sunar.
