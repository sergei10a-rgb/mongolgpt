...
mongolgpt'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
mongolgpt, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
MongolGPT, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

MongolGPT iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `mongolgpt.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse mongolgpt modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global mongolgpt yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
mongolgpt yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [MongolGPT Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `mongolgpt/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `mongolgpt models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/sergei10a-rgb/mongolgpt).
:::
