---
title: Sağlayıcılar
description: mongolgpt'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

mongolgpt, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı mongolgpt yapılandırmanızda yapılandırın.
...
`~/.local/share/mongolgpt/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları mongolgpt'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
MongolGPT Zen, mongolgpt ekibi tarafından sağlanan modellerin bir listesidir.
mongolgpt ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz MongolGPT Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, mongolgpt'u seçin ve [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/auth)'ye gidin.
...
mongolgpt'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## MongolGPT Go

MongolGPT Go, mongolgpt ile iyi çalıştığı test edilmiş ve doğrulanmış, mongolgpt ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `MongolGPT Go`'yu seçin ve [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen) adresine gidin.
...
Amazon Bedrock'u mongolgpt ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   mongolgpt'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `mongolgpt.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi mongolgpt'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
mongolgpt'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
-
pm` bu sağlayıcı için kullanılacak paketi belirtir. Burada, herhangi bir OpenAI uyumlu API için `@ai-sdk/openai-compatible` kullanılır.
-
ame` sağlayıcının arayüzde görüntülenen adıdır.
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   mongolgpt'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   MongolGPT yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
mongolgpt, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. mongolgpt'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`mongolgpt.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge MongolGPT GitLab auth adapter ana sayfasında.

##### Yapılandırma

`mongolgpt.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi mongolgpt'la kullanmak için:
...
Google Vertex AI'yi mongolgpt ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları mongolgpt çalıştırılırken ayarlayın.
...
Helicone'un mongolgpt aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her mongolgpt görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için local Helicone session plugin eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her mongolgpt görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
mongolgpt'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
-
pm` bu sağlayıcı için kullanılacak paketi belirtir. Burada `@ai-sdk/openai-compatible`, herhangi bir OpenAI uyumlu API için kullanılır.
-
ame` sağlayıcının kullanıcı arayüzündeki görünen adıdır.
...
LM Studio aracılığıyla mongolgpt'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla mongolgpt'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini mongolgpt için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/mongolgpt)'a bakın.
:::
...
:::tip
Araç çağrıları çalışmıyorsa Ollama'da
um_ctx` değerini artırmayı deneyin. 16k - 32k civarında başlayın.
:::
...
Ollama Cloud'u mongolgpt ile kullanmak için:
...
3. mongolgpt'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: mongolgpt'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
MongolGPT Zen, mongolgpt ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>MongolGPT Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **MongolGPT Zen**'i arayın.

   ```txt
   /connect
   ```

3. mongolgpt API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   mongolgpt yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca mongolgpt yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi MongolGPT ile kullanmak için:
...
Modelleri mongolgpt yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı mongolgpt ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `mongolgpt.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları mongolgpt'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `mongolgpt auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için mongolgpt yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, mongolgpt yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
