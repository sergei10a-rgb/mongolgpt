---
title: Servidor
description: Interaja com o servidor mongolgpt via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `mongolgpt serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente mongolgpt pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `mongolgpt.local` |
...
Defina `MONGOLGPT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `mongolgpt`, ou defina `MONGOLGPT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `mongolgpt serve` quanto ao `mongolgpt web`.
...
Quando você executa `mongolgpt`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor mongolgpt para interagir com o mongolgpt programaticamente.
:::

Essa arquitetura permite que o mongolgpt suporte múltiplos clientes e permite que você interaja com o mongolgpt programaticamente.

Você pode executar `mongolgpt serve` para iniciar um servidor autônomo. Se você tiver o TUI do mongolgpt em execução, `mongolgpt serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do mongolgpt [IDE](/docs/ide).
...
O servidor mongolgpt expõe as seguintes APIs.
