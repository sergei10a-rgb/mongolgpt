...
Com o mongolgpt, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o mongolgpt usa nosso próprio tema `mongolgpt`.
...
O mongolgpt vem com vários temas embutidos.
...
|
ord`                 | Baseado no tema [Nord](https://github.com/nordtheme/nord)                   |
...
- **Preserva padrões do terminal**: Usa
one` para cores de texto e fundo para manter a aparência nativa do seu terminal.

O tema do sistema é para usuários que:

- Querem que o mongolgpt corresponda à aparência do seu terminal
...
O mongolgpt suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/mongolgpt/themes/*.json` ou `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.mongolgpt/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.mongolgpt/themes/*.json`
