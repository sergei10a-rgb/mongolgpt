---
title: SDK
description: Cliente JS seguro em tipos para o servidor mongolgpt.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O SDK JS/TS do mongolgpt fornece um cliente seguro em tipos para interagir com o servidor.
Use-o para construir integrações e controlar o mongolgpt programaticamente.
...
Crie uma instância do mongolgpt:
...
| `port`     |
umber`      | Porta do servidor                          | `4096`      |
| `signal`   | `AbortSignal` | Sinal de abortar para cancelamento         | `undefined` |
| `timeout`  |
umber`      | Tempo limite em ms para iniciar o servidor | `5000`      |
...
Você pode passar um objeto de configuração para personalizar o comportamento. A instância ainda pega seu `mongolgpt.json`, mas você pode substituir ou adicionar configuração inline:
...
Se você já tem uma instância do mongolgpt em execução, pode criar uma instância de cliente para se conectar a ela:
...
| `retryCount` |
umber`        | Opcional. Número de tentativas de validação (padrão: 2)        |
