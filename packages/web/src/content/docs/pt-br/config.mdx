---
title: Configuração
description: Usando a configuração JSON do mongolgpt.
---

Você pode configurar o mongolgpt usando um arquivo de configuração JSON.

---

## Formato

O mongolgpt suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/mongolgpt`) - padrões organizacionais
2. **Configuração global** (`~/.config/mongolgpt/mongolgpt.json`) - preferências do usuário
3. **Configuração personalizada** (`MONGOLGPT_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`mongolgpt.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.mongolgpt`** - agentes, comandos, plugins
6. **Configuração inline** (`MONGOLGPT_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.mongolgpt` e `~/.config/mongolgpt` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/mongolgpt`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do mongolgpt em `~/.config/mongolgpt/mongolgpt.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/mongolgpt/tui.json`.
...
Adicione `mongolgpt.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o mongolgpt é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `MONGOLGPT_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `MONGOLGPT_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.mongolgpt`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.mongolgpt`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`mongolgpt.duckdns.org/config.json`**](https://mongolgpt.duckdns.org/config.json).

A configuração do TUI usa [**`mongolgpt.duckdns.org/tui.json`**](https://mongolgpt.duckdns.org/tui.json).
...
Use `MONGOLGPT_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `mongolgpt.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `mongolgpt serve` e `mongolgpt web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor mongolgpt.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `mongolgpt.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do mongolgpt através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o mongolgpt tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/mongolgpt/agents/` ou `.mongolgpt/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o mongolgpt recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`mongolgpt run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/mongolgpt/commands/` ou `.mongolgpt/commands/`. [Saiba mais aqui](/docs/commands).
...
O mongolgpt fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o mongolgpt **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o mongolgpt com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.mongolgpt/plugins/` ou `~/.config/mongolgpt/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o mongolgpt para usar apenas provedores específicos, em vez de desabilitá-los um a um.
