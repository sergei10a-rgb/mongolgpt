...
As habilidades do agente permitem que o mongolgpt descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O mongolgpt pesquisa nesses locais:

- Configuração do projeto: `.mongolgpt/skills/<name>/SKILL.md`
- Configuração global: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o mongolgpt sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.mongolgpt/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.mongolgpt/skills/git-release/SKILL.md` assim:
...
O mongolgpt lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `mongolgpt.json`:
...
**Para agentes embutidos** (em `mongolgpt.json`):
...
2. Verifique se o frontmatter inclui
ame` e `description`
