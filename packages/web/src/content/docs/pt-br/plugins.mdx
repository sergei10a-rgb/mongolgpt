---
title: Plugins
description: Escreva seus próprios plugins para estender o mongolgpt.
---

Plugins permitem que você estenda o mongolgpt conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do mongolgpt.
...
- `.mongolgpt/plugins/` - Plugins em nível de projeto
- `~/.config/mongolgpt/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/mongolgpt/node_modules/`.
...
1. Configuração global (`~/.config/mongolgpt/mongolgpt.json`)
2. Configuração do projeto (`mongolgpt.json`)
3. Diretório de plugins global (`~/.config/mongolgpt/plugins/`)
4. Diretório de plugins do projeto (`.mongolgpt/plugins/`)
...
O mongolgpt executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do mongolgpt para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o mongolgpt.
...
:::note
Se você estiver usando o aplicativo desktop mongolgpt, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o mongolgpt de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao mongolgpt:
...
O helper `tool` cria uma ferramenta personalizada que o mongolgpt pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o mongolgpt junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://mongolgpt.duckdns.org/docs/sdk) para detalhes.
