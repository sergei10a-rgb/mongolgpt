...
As ferramentas permitem que o LLM execute ações em sua base de código. O mongolgpt vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no mongolgpt.
...
Esta ferramenta permite que o LLM execute comandos de terminal como
pm install`, `git status` ou qualquer outro comando de shell.
...
:::note
Esta ferramenta está disponível apenas quando `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (ou `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor mongolgpt ou quando a variável de ambiente `MONGOLGPT_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o mongolgpt:
...
Por exemplo, este arquivo `.ignore` permite que o ripgrep busque dentro dos diretórios
ode_modules/`, `dist/` e `build/`, mesmo que estejam listados em `.gitignore`.
