...
O MongolGPT Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no MongolGPT. Você assina o MongolGPT Go e
...
O MongolGPT Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O MongolGPT Go funciona como qualquer outro provedor no MongolGPT.

1. Você entra no **<a href={console}>MongolGPT Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `MongolGPT Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o MongolGPT Go.
:::
...
O MongolGPT Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do MongolGPT
usa o formato `mongolgpt-go/<model-id>`. Por exemplo, para o Kimi K2.7 Code, você usaria
`mongolgpt-go/kimi-k2.7-code` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o MongolGPT também.
