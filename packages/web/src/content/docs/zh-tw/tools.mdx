...
工具允許 LLM 在您的程式碼庫中執行操作。MongolGPT 自帶一組內建工具，您也可以透過[自訂工具](/docs/custom-tools)或 [MCP 伺服器](/docs/mcp-servers)來擴充它。
...
該工具允許 LLM 執行終端機指令，例如
pm install`、`git status` 或其他任何 shell 指令。
...
:::note
該工具僅在設定 `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true`（或 `MONGOLGPT_EXPERIMENTAL=true`）時可用。
:::
...
:::note
該工具僅在使用 MongolGPT 供應商時，或當 `MONGOLGPT_ENABLE_EXA` 環境變數設定為任意真值（例如 `true` 或 `1`）時可用。
...
例如，這個 `.ignore` 檔案允許 ripgrep 在
ode_modules/`、`dist/` 和 `build/` 目錄中進行搜尋，即使它們已在 `.gitignore` 中列出。
