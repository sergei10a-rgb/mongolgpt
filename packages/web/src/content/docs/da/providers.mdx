---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i MongolGPT-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/mongolgpt/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i MongolGPT-konfigurationen.
...
MongolGPT Zen er en liste over modeller leveret af MongolGPT-teamet, som er blevet testet og verificeret til at fungere godt med MongolGPT. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med MongolGPT Zen.
:::
...
Det fungerer som alle andre udbydere i MongolGPT og er helt valgfrit at bruge.

---

## MongolGPT Go

MongolGPT Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af MongolGPT-teamet, som er testet og verificeret til at fungere godt med MongolGPT.

1. Kør kommandoen `/connect` i TUI, vælg `MongolGPT Go`, og gå til [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Sådan bruger du Amazon Bedrock med MongolGPT:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører mongolgpt:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `mongolgpt.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i MongolGPT understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere mongolgpt til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
-
pm` specificerer pakken, der skal bruges for denne udbyder. Her bruges `@ai-sdk/openai-compatible` til enhver OpenAI-kompatibel API.
-
ame` er det viste navn på udbyderen i grænsefladen.
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at mongolgpt skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem mongolgpt-konfigurationen.
...
gemme tokenet i mongolgpt auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
MongolGPT bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse MongolGPT
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`mongolgpt.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på MongolGPT GitLab auth adapter hjemmesiden.

##### Konfiguration

Tilpas gennem `mongolgpt.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med MongolGPT:
...
Sådan bruger du Google Vertex AI med MongolGPT:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører mongolgpt.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem mongolgpt, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug local Helicone session plugin for automatisk at logge hver MongolGPT-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver MongolGPT-samtale opført som en separat session.
...
Du kan konfigurere mongolgpt til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
-
pm` specificerer pakken som skal bruges for denne udbyder. Her bruges `@ai-sdk/openai-compatible` for enhver OpenAI-kompatibel API.
-
ame` er visningsnavnet for udbyderen i UI.
...
Du kan konfigurere mongolgpt til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere mongolgpt til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for MongolGPT. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/mongolgpt) for detaljer.
:::
...
:::tip
Hvis værktøjskald ikke fungerer, prøv at øge
um_ctx` i Ollama. Start omkring 16k - 32k.
:::
...
Sådan bruger du Ollama Cloud med MongolGPT:
...
3. Kopier API-nøglen til brug i MongolGPT.
...
6. **Vigtigt**: Før du bruger skymodeller i MongolGPT, skal du hente modelinformationen lokalt:
...
MongolGPT Zen er en liste over testede og verificerede modeller leveret af MongolGPT-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>MongolGPT Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Indtast MongolGPT API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem mongolgpt-konfigurationen.
...
5. Du kan også tilpasse dem gennem mongolgpt-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MongolGPT:
...
Du kan også tilpasse modeller gennem mongolgpt-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med mongolgpt. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `mongolgpt.json`-filen i projektkataloget:
...
`limit`-felterne lader MongolGPT forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `mongolgpt auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek mongolgpt-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i mongolgpt-konfigurationen.
