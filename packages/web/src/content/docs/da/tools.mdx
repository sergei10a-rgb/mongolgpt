...
Her er alle de innebygde verktøyene som er tilgængelige i MongolGPT.
...
pm install`, `git status` eller en hvilken som helst annen shell-kommando.
...
:::note
Dette verktøyet er bare tilgængelig når `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (eller `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger MongolGPT-leverandøren eller når miljøvariabelen `MONGOLGPT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
