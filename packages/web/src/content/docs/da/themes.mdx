...
Med MongolGPT kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger MongolGPT vårt eget `mongolgpt`-tema.
...
- **Bevarer terminalens standardinnstillinger**: Bruger
one` for tekst- og bakgrunnsfarver for at opretholde terminalens opprinnelige utseende.
...
MongolGPT støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/mongolgpt/themes/*.json` eller `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.mongolgpt/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.mongolgpt/themes/*.json`
