---
title: Server
description: Interager med mongolgpt-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `mongolgpt serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en mongolgpt-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `mongolgpt.local` |
...
Indstil `MONGOLGPT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `mongolgpt`, eller indstil `MONGOLGPT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `mongolgpt serve` og `mongolgpt web`.
...
Når du kører `mongolgpt` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug mongolgpt-serveren til at interagere med mongolgpt programmatisk.
:::

Denne arkitektur lader mongolgpt understøtte flere klienter og lader dig interagere med mongolgpt programmatisk.

Du kan køre `mongolgpt serve` for at starte en selvstændig server. Hvis du har
mongolgpt TUI kørende, vil `mongolgpt serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af MongolGPT [IDE](/docs/ide) plugins.
...
MongolGPT-serveren viser følgende API'er.
