...
For at fejlfinde problemer med MongolGPT, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\mongolgpt\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `mongolgpt --log-level DEBUG`.

---

## Lagring

mongolgpt gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/mongolgpt/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\mongolgpt`
...
MongolGPT Desktop kører en lokal MongolGPT-server (`mongolgpt-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `MongolGPT`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/mongolgpt/mongolgpt.jsonc` (eller `~/.config/mongolgpt/mongolgpt.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/mongolgpt/mongolgpt.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\mongolgpt\mongolgpt.jsonc`
...
MongolGPT kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/mongolgpt/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\mongolgpt\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så MongolGPT kan genopbygge den.

1. Afslut MongolGPT Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/mongolgpt`
- **Linux**: slet `~/.cache/mongolgpt` (eller kør `rm -rf ~/.cache/mongolgpt`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\mongolgpt`

3. Start MongolGPT Desktop på ny.

---

### Løs problemer med serverforbindelse

MongolGPT Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `mongolgpt.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `MONGOLGPT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `MONGOLGPT_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver MongolGPT Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i MongolGPT.

---

### Meddelelser vises ikke

MongolGPT Desktop viser kun systemvarsler når:

- varsler er aktiveret for MongolGPT i dine OS-indstillinger, og
...
1. Afslut MongolGPT Desktop.
2. Find og slet disse filer (de findes i MongolGPT Desktop-appens datamappe):

- `mongolgpt.settings.dat` (skrivebordsstandardserver URL)
- `mongolgpt.global.dat` og `mongolgpt.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med MongolGPT:
...
For at finde ud af hvilke modeller du har adgang til, kør `mongolgpt models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\mongolgpt`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. mongolgpt installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\mongolgpt`

2. Start mongolgpt på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge mongolgpt til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
mongolgpt vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
