---
title: Plugins
description: Skriv dine egne plugins for at udvide MongolGPT.
---

Plugins giver dig mulighed for at udvide MongolGPT ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre MongolGPTs standardadfærd.
...
- `.mongolgpt/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/mongolgpt/node_modules/`.
...
1. Global konfiguration (`~/.config/mongolgpt/mongolgpt.json`)
2. Projektkonfiguration (`mongolgpt.json`)
3. Global plugin-mappe (`~/.config/mongolgpt/plugins/`)
4. Projekt plugin bibliotek (`.mongolgpt/plugins/`)
...
MongolGPT kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En mongolgpt SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide mongolgpt.
...
:::note
Hvis du bruger MongolGPT desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå mongolgpt i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til mongolgpt:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som mongolgpt kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for mongolgpt sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://mongolgpt.duckdns.org/docs/sdk) for detaljer.
