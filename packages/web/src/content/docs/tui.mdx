---
title: TUI
description: Using the MongolGPT terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MongolGPT provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running MongolGPT starts the TUI for the current directory.
...
When using the MongolGPT TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to MongolGPT. Allows you to select from available providers and add their API keys.
...
Exit MongolGPT. _Aliases_: `/quit`, `/q`
...
This is separate from `mongolgpt.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long MongolGPT waits after the leader key. Defaults to `2000`.
...
Use `MONGOLGPT_TUI_CONFIG` to load a custom TUI config path.
...
-
otifications` - Allow terminal-mediated desktop notifications when attention is enabled. Defaults to `true`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `mongolgpt.default`.
