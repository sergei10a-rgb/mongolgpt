...
Create `.mongolgpt/commands/test.md`:
...
You can add custom commands through the MongolGPT config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your MongolGPT [config](/docs/config):
...
- Per-project: `.mongolgpt/commands/`
...
mongolgpt includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
