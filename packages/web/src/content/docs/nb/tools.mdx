...
Verktøy lar LLM utføre handlinger i kodebasen din. MongolGPT kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i MongolGPT.
...
Dette verktøyet lar LLM kjøre terminalkommandoer som
pm install`, `git status` eller en hvilken som helst annen skallkommando.
...
:::note
Dette verktøyet er bare tilgjengelig når `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (eller `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker MongolGPT-leverandøren eller når miljøvariabelen `MONGOLGPT_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter MongolGPT:
...
For eksempel lar denne `.ignore` filen ripgrep søke i
ode_modules/`, `dist/` og `build/` kataloger selv om de er oppført i `.gitignore`.
