---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i MongolGPT-konfigurasjonen.
...
i `~/.local/share/mongolgpt/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i MongolGPT
konfig.
...
MongolGPT Zen er en liste over modeller levert av MongolGPT-teamet som har vært
testet og verifisert for å fungere godt med MongolGPT. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med MongolGPT Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `MongolGPT Zen` og gå til [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Det fungerer som alle andre leverandører i MongolGPT og er helt valgfritt å bruke.

---

## MongolGPT Go

MongolGPT Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av MongolGPT-teamet som har vært
testet og verifisert for å fungere godt med MongolGPT.

1. Kjør kommandoen `/connect` i TUI, velg `MongolGPT Go`, og gå til [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Slik bruker du Amazon Bedrock med MongolGPT:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører MongolGPT:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `mongolgpt.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i mongolgpt støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere mongolgpt til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
-
pm` spesifiserer pakken som skal brukes for denne leverandøren. Her brukes `@ai-sdk/openai-compatible` for enhver OpenAI-kompatibel API.
-
ame` er visningsnavnet for leverandøren i grensesnittet.
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at MongolGPT skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom MongolGPT-konfigurasjonen.
...
for å lagre token i MongolGPT auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
MongolGPT bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse MongolGPT
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`mongolgpt.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på MongolGPT GitLab auth adapter hjemmeside.

##### Konfigurasjon

Tilpass gjennom `mongolgpt.json`:
...
Slik bruker du GitHub Copilot-abonnementet med MongolGPT:
...
Slik bruker du Google Vertex AI med MongolGPT:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører MongolGPT.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom MongolGPT, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk local Helicone session plugin for automatisk å logge hver MongolGPT-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver MongolGPT-samtale oppført som en separat økt.
...
Du kan konfigurere MongolGPT for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
pm` spesifiserer pakken som skal brukes for denne leverandøren. Her brukes `@ai-sdk/openai-compatible` for enhver OpenAI-kompatibel API.
-
ame` er visningsnavnet for leverandøren i UI.
...
Du kan konfigurere MongolGPT for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere MongolGPT for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for MongolGPT. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/mongolgpt) for detaljer.
:::
...
:::tip
Hvis verktøykall ikke fungerer, prøv å øke
um_ctx` i Ollama. Start rundt 16k - 32k.
:::
...
Slik bruker du Ollama Cloud med MongolGPT:
...
3. Kopier API-nøkkelen for bruk i MongolGPT.
...
6. **Viktig**: Før du bruker skymodeller i MongolGPT, må du hente modellinformasjonen lokalt:
...
MongolGPT Zen er en liste over testede og verifiserte modeller levert av MongolGPT-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>MongolGPT Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Skriv inn MongolGPT API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom MongolGPT-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom MongolGPT-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MongolGPT:
...
Du kan også tilpasse modeller gjennom MongolGPT-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med MongolGPT. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `mongolgpt.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar MongolGPT forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `mongolgpt auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk MongolGPT-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i MongolGPT-konfigurasjonen.
