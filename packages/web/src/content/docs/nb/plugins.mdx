...
description: Skriv dine egne programtillegg for å utvide MongolGPT.
---

Plugins lar deg utvide MongolGPT ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til MongolGPT.
...
- `.mongolgpt/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/mongolgpt/node_modules/`.
...
1. Global konfigurasjon (`~/.config/mongolgpt/mongolgpt.json`)
2. Prosjektkonfigurasjon (`mongolgpt.json`)
3. Global plugin-katalog (`~/.config/mongolgpt/plugins/`)
4. Prosjektpluginkatalog (`.mongolgpt/plugins/`)
...
MongolGPT kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En MongolGPT SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide MongolGPT.
...
:::note
Hvis du bruker MongolGPT-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre MongolGPT fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til MongolGPT:
...
`tool`-hjelperen lager et tilpasset verktøy som MongolGPT kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for MongolGPT sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://mongolgpt.duckdns.org/docs/sdk) for detaljer.
