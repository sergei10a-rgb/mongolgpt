...
Agentferdigheter lar MongolGPT oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
mongolgpt søker etter disse stedene:

- Prosjektkonfigurasjon: `.mongolgpt/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
For prosjektlokale stier går mongolgpt opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.mongolgpt/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.mongolgpt/skills/git-release/SKILL.md` slik:
...
mongolgpt viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `mongolgpt.json`:
...
**For innebygde agenter** (i `mongolgpt.json`):
...
2. Sjekk at frontmatter inkluderer
ame` og `description`
