---
title: TUI
description: Bruke mongolgpt-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MongolGPT gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre MongolGPT starter TUI for gjeldende katalog.
...
Når du bruker MongolGPT TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til MongolGPT. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt MongolGPT. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `mongolgpt.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge MongolGPT venter etter leader key. Standard er `2000`.
...
Bruk `MONGOLGPT_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
...
-
otifications` - Når Attention er aktivert, lar dette TUI sende skrivebordsvarsler via terminalen. Standard er `true`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `mongolgpt.default`.
