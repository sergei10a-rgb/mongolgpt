...
Med MongolGPT kan du velge fra ett av flere innebygde temaer, bruke et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruker MongolGPT vårt eget `mongolgpt`-tema.
...
MongolGPT kommer med flere innebygde temaer.
...
|
ord`                 | Basert på [Nord](https://github.com/nordtheme/nord)-temaet                |
...
- **Bevarer terminalens standardinnstillinger**: Bruker
one` for tekst- og bakgrunnsfarger for å opprettholde terminalens opprinnelige utseende.

Systemtemaet er for brukere som:

- Vil at MongolGPT skal matche terminalens utseende
...
MongolGPT støtter et fleksibelt JSON-basert temasystem som lar brukere enkelt lage og tilpasse temaer.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/mongolgpt/themes/*.json` eller `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.mongolgpt/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.mongolgpt/themes/*.json`
