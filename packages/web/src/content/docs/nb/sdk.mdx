---
title: SDK
description: Typesikker JS-klient for MongolGPT-server.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

MongolGPT JS/TS SDK gir en typesikker klient for samhandling med serveren.
Bruk den til å bygge integrasjoner og kontrollere MongolGPT programmatisk.
...
Opprett en forekomst av MongolGPT:
...
| `port`     |
umber`      | Serverport                       | `4096`      |
| `signal`   | `AbortSignal` | AbortSignal for avbrudd          | `undefined` |
| `timeout`  |
umber`      | Tidsavbrudd i ms for serverstart | `5000`      |
...
Du kan sende et konfigurasjonsobjekt for å tilpasse virkemåten. Forekomsten henter fortsatt din `mongolgpt.json`, men du kan overstyre eller legge til konfigurasjon inline:
...
Hvis du allerede har en kjørende forekomst av MongolGPT, kan du opprette en klientforekomst for å koble til den:
...
| `retryCount` |
umber`        | Valgfritt. Antall valideringsforsøk (standard: 2)          |
