---
title: Tjener
description: Samhandle med MongolGPT-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `mongolgpt serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en MongolGPT-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `mongolgpt.local` |
...
Sett `MONGOLGPT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `mongolgpt` som standard, eller sett `MONGOLGPT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `mongolgpt serve` og `mongolgpt web`.
...
Når du kjører `mongolgpt` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk MongolGPT-serveren til å samhandle med MongolGPT programmatisk.
:::

Denne arkitekturen lar MongolGPT støtte flere klienter og lar deg samhandle med MongolGPT programmatisk.

Du kan kjøre `mongolgpt serve` for å starte en frittstående server. Hvis du har
MongolGPT TUI kjører, vil `mongolgpt serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av MongolGPT [IDE](/docs/ide) plugins.
...
mongolgpt-serveren viser følgende APIer.
