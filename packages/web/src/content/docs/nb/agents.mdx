...
Det er to typer agenter i MongolGPT; primære agenter og underagenter.
...
MongolGPT kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
MongolGPT kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

MongolGPT kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `mongolgpt.json`:
...
Hvis ingen temperatur er spesifisert, bruker MongolGPT modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale MongolGPT-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i MongolGPT-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [MongolGPT Zen](/docs/zen), vil du bruke `mongolgpt/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `mongolgpt models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/sergei10a-rgb/mongolgpt).
:::
