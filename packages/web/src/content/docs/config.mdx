---
title: Config
description: Using the MongolGPT JSON config.
---

You can configure MongolGPT using a JSON config file.

---

## Format

MongolGPT supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/mongolgpt`) - organizational defaults
2. **Global config** (`~/.config/mongolgpt/mongolgpt.json`) - user preferences
3. **Custom config** (`MONGOLGPT_CONFIG` env var) - custom overrides
4. **Project config** (`mongolgpt.json` in project) - project-specific settings
5. **`.mongolgpt` directories** - agents, commands, plugins
6. **Inline config** (`MONGOLGPT_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/mongolgpt/` on macOS) - admin-controlled
...
:::note
The `.mongolgpt` and `~/.config/mongolgpt` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/mongolgpt` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global MongolGPT config in `~/.config/mongolgpt/mongolgpt.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/mongolgpt/tui.json`.
...
Add `mongolgpt.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When MongolGPT starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `MONGOLGPT_CONFIG` environment variable.
...
Specify a custom config directory using the `MONGOLGPT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.mongolgpt` directory, and should
...
The custom directory is loaded after the global config and `.mongolgpt` directories, so it **can override** their settings.
...
Drop an `mongolgpt.json` or `mongolgpt.jsonc` file in the system managed config directory:
...
On macOS, MongolGPT reads managed preferences from the `ai.mongolgpt.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

MongolGPT checks these paths:

1. `/Library/Managed Preferences/<user>/ai.mongolgpt.managed.plist`
2. `/Library/Managed Preferences/ai.mongolgpt.managed.plist`

The plist keys map directly to `mongolgpt.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.mongolgpt.managed` PayloadType. The MongolGPT config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`mongolgpt.duckdns.org/config.json`**](https://mongolgpt.duckdns.org/config.json).

TUI config uses [**`mongolgpt.duckdns.org/tui.json`**](https://mongolgpt.duckdns.org/tui.json).
...
Use `MONGOLGPT_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `mongolgpt.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `mongolgpt serve` and `mongolgpt web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your MongolGPT server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `mongolgpt.local`. Useful for running multiple instances on the same network.
...
If not specified, MongolGPT will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your MongolGPT config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, MongolGPT tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny MongolGPT actions on configured resources. Currently, policies can control which providers MongolGPT may use.
...
MongolGPT normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, MongolGPT omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/mongolgpt/agents/` or `.mongolgpt/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, MongolGPT will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`mongolgpt run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/mongolgpt/commands/` or `.mongolgpt/commands/`. [Learn more here](/docs/commands).
...
MongolGPT uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
MongolGPT will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, mongolgpt **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend MongolGPT with custom tools, hooks, and integrations.

Place plugin files in `.mongolgpt/plugins/` or `~/.config/mongolgpt/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict MongolGPT to only use specific providers rather than disabling them one by one.
