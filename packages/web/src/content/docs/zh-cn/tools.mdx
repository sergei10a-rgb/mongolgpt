...
工具允许 LLM 在您的代码库中执行操作。MongolGPT 自带一组内置工具，您也可以通过[自定义工具](/docs/custom-tools)或 [MCP 服务器](/docs/mcp-servers)来扩展它。
...
该工具允许 LLM 运行终端命令，例如
pm install`、`git status` 或其他任何 shell 命令。
...
:::note
该工具仅在设置 `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true`（或 `MONGOLGPT_EXPERIMENTAL=true`）时可用。
:::
...
:::note
该工具仅在使用 MongolGPT 提供商时，或当 `MONGOLGPT_ENABLE_EXA` 环境变量设置为任意真值（例如 `true` 或 `1`）时可用。
...
例如，这个 `.ignore` 文件允许 ripgrep 在
ode_modules/`、`dist/` 和 `build/` 目录中进行搜索，即使它们已在 `.gitignore` 中列出。
