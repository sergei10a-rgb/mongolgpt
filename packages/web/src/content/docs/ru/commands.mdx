...
Создайте `.mongolgpt/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию mongolgpt или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем mongolgpt [config](/docs/config):
...
- Глобальный: `~/.config/mongolgpt/commands/`
- Для каждого проекта: `.mongolgpt/commands/`
...
mongolgpt включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
