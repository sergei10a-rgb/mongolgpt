...
Навыки агента позволяют mongolgpt обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
mongolgpt выполняет поиск в следующих местах:

- Конфигурация проекта: `.mongolgpt/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/mongolgpt/skills/<name>/SKILL.md`.
...
Для локальных путей проекта mongolgpt переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.mongolgpt/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.mongolgpt/skills/git-release/SKILL.md` следующим образом:
...
mongolgpt перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `mongolgpt.json`:
...
**Для встроенных агентов** (в формате `mongolgpt.json`):
...
2. Убедитесь, что заголовок включает
ame` и `description`.
