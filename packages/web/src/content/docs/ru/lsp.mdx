---
title: LSP-серверы
description: mongolgpt интегрируется с вашими серверами LSP.
---

mongolgpt может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

mongolgpt поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `MONGOLGPT_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и mongolgpt открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации mongolgpt.
