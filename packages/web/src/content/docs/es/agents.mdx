...
Hay dos tipos de agentes en MongolGPT; agentes primarios y subagentes.
...
MongolGPT viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
MongolGPT viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

MongolGPT viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de MongolGPT, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Configure agentes en su archivo de configuración `mongolgpt.json`:
...
Si no se especifica ninguna temperatura, MongolGPT utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global MongolGPT como para la configuración específica del proyecto.
...
El ID del modelo en su configuración MongolGPT usa el formato `provider/model-id`. Por ejemplo, si usa [MongolGPT Zen](/docs/zen), usaría `mongolgpt/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `mongolgpt models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/sergei10a-rgb/mongolgpt).
:::
