...
Las habilidades del agente permiten a MongolGPT descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
MongolGPT busca estas ubicaciones:

- Configuración del proyecto: `.mongolgpt/skills/<name>/SKILL.md`
- Configuración global: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, MongolGPT sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.mongolgpt/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.mongolgpt/skills/git-release/SKILL.md` así:
...
MongolGPT enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `mongolgpt.json`:
...
**Para agentes integrados** (en `mongolgpt.json`):
...
2. Verifique que el frontmatter incluya
ame` y `description`
