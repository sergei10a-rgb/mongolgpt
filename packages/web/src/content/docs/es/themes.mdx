...
Con MongolGPT puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, MongolGPT usa nuestro propio tema `mongolgpt`.
...
MongolGPT viene con varios temas integrados.
...
|
ord`                 | Basado en el tema [Nord](https://github.com/nordtheme/nord)                   |
...
- **Conserva los valores predeterminados del terminal**: utiliza
one` para el texto y los colores de fondo para mantener la apariencia nativa de su terminal.

El tema del sistema es para usuarios que:

- Quiere que MongolGPT coincida con la apariencia de su terminal
...
MongolGPT admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/mongolgpt/themes/*.json` o `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.mongolgpt/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.mongolgpt/themes/*.json`
