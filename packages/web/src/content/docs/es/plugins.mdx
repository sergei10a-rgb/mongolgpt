---
title: Complementos
description: Escriba sus propios complementos para extender MongolGPT.
---

Los complementos le permiten extender MongolGPT al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de MongolGPT.
...
- `.mongolgpt/plugins/` - Complementos a nivel de proyecto
- `~/.config/mongolgpt/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/mongolgpt/node_modules/`.
...
1. Configuración global (`~/.config/mongolgpt/mongolgpt.json`)
2. Configuración del proyecto (`mongolgpt.json`)
3. Directorio global de complementos (`~/.config/mongolgpt/plugins/`)
4. Directorio de complementos del proyecto (`.mongolgpt/plugins/`)
...
MongolGPT ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK mongolgpt para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar mongolgpt.
...
:::note
Si está utilizando la aplicación de escritorio MongolGPT, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que mongolgpt lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a mongolgpt:
...
El ayudante `tool` crea una herramienta personalizada a la que mongolgpt puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para mongolgpt junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://mongolgpt.duckdns.org/docs/sdk) para obtener más detalles.
