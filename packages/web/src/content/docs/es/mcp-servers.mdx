...
Puede agregar herramientas externas a MongolGPT utilizando el _Model Context Protocol_ o MCP. MongolGPT admite servidores locales y remotos.
...
Puede definir servidores MCP en su [MongolGPT Config](https://mongolgpt.duckdns.org/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/mongolgpt`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
MongolGPT maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, MongolGPT:
...
Si el servidor requiere autenticación, MongolGPT le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `mongolgpt mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, MongolGPT almacenará los tokens de forma segura en `~/.local/share/mongolgpt/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en MongolGPT, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración MongolGPT como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar MongolGPT a su cuenta Sentry.
