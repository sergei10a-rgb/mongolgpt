---
title: Proveedores
description: Usando cualquier proveedor LLM en MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración MongolGPT.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu MongolGPT
configuración.
...
MongolGPT Zen es una lista de modelos proporcionados por el equipo MongolGPT que han sido
probado y verificado para funcionar bien con MongolGPT. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con MongolGPT Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione mongolgpt y diríjase a [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/auth).
...
Funciona como cualquier otro proveedor en MongolGPT y su uso es completamente opcional.

---

## MongolGPT Go

MongolGPT Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de MongolGPT que han sido
probado y verificado para funcionar bien con MongolGPT.

1. Ejecute el comando `/connect` en TUI, seleccione `MongolGPT Go` y diríjase a [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Para usar Amazon Bedrock con MongolGPT:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta mongolgpt:
   ...
   Para una configuración persistente o específica del proyecto, utilice `mongolgpt.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en MongolGPT no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar mongolgpt para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
-
pm` especifica el paquete que se usará para este proveedor. Aquí se usa `@ai-sdk/openai-compatible` para cualquier API compatible con OpenAI.
-
ame` es el nombre que se muestra para el proveedor en la interfaz.
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que mongolgpt funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración mongolgpt.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `MongolGPT`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación mongolgpt.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
MongolGPT utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear MongolGPT
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `mongolgpt.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de MongolGPT GitLab auth adapter.

##### Configuración

Personalizar a través de `mongolgpt.json`:
...
Para utilizar su suscripción GitHub Copilot con mongolgpt:
...
Para utilizar Google Vertex AI con MongolGPT:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta mongolgpt.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de mongolgpt, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento local Helicone session plugin para registrar automáticamente cada conversación MongolGPT como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación MongolGPT enumerada como una sesión separada.
...
Puede configurar mongolgpt para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
-
pm` especifica el paquete que se utilizará para este proveedor. Aquí, `@ai-sdk/openai-compatible` se utiliza para cualquier OpenAI compatible con API.
-
ame` es el nombre para mostrar del proveedor en la interfaz de usuario.
...
Puede configurar mongolgpt para usar modelos locales a través de LM Studio.
...
Puedes configurar mongolgpt para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para MongolGPT. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/mongolgpt) para obtener más detalles.
:::
...
um_ctx` en Ollama. Comience alrededor de 16k - 32k.
:::
...
Para usar Ollama Cloud con MongolGPT:
...
3. Copie la clave API para usarla en MongolGPT.
...
6. **Importante**: Antes de usar modelos en la nube en MongolGPT, debe obtener la información del modelo localmente:
...
MongolGPT Zen es una lista de modelos probados y verificados proporcionada por el equipo MongolGPT. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>MongolGPT Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave MongolGPT API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración mongolgpt.
...
5. También puedes personalizarlos a través de tu configuración mongolgpt. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con MongolGPT:
...
También puede personalizar modelos a través de su configuración mongolgpt. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con mongolgpt. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `mongolgpt.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a MongolGPT comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `mongolgpt auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración mongolgpt y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración mongolgpt.
