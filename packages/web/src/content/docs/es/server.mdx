---
title: Servidor
description: Interactuar con el servidor mongolgpt a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `mongolgpt serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente mongolgpt puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `mongolgpt.local` |
...
Configure `MONGOLGPT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `mongolgpt`, o configure `MONGOLGPT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `mongolgpt serve` como a `mongolgpt web`.
...
Cuando ejecuta `mongolgpt`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor mongolgpt para interactuar con mongolgpt mediante programación.
:::

Esta arquitectura permite que mongolgpt admita múltiples clientes y le permite interactuar con mongolgpt mediante programación.

Puede ejecutar `mongolgpt serve` para iniciar un servidor independiente. Si tienes el
mongolgpt TUI ejecutándose, `mongolgpt serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos MongolGPT [IDE](/docs/ide).
...
El servidor mongolgpt expone las siguientes API.
