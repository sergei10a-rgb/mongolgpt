...
Las herramientas permiten que LLM realice acciones en su código base. MongolGPT viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en MongolGPT.
...
Esta herramienta permite que LLM ejecute comandos de terminal como
pm install`, `git status` o cualquier otro comando de shell.
...
:::note
Esta herramienta solo está disponible cuando `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (o `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor MongolGPT o cuando la variable de entorno `MONGOLGPT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar MongolGPT:
...
Por ejemplo, este archivo `.ignore` permite que ripgrep busque dentro de los directorios
ode_modules/`, `dist/` y `build/` incluso si figuran en `.gitignore`.
