...
Da biste otklonili probleme s MongolGPT, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\mongolgpt\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `mongolgpt --log-level DEBUG`.

---

## Pohrana

MongolGPT pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/mongolgpt/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\mongolgpt`
...
MongolGPT Desktop pokreće lokalni MongolGPT server (`mongolgpt-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `MongolGPT` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/mongolgpt/mongolgpt.jsonc` (ili `~/.config/mongolgpt/mongolgpt.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/mongolgpt/mongolgpt.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\mongolgpt\mongolgpt.jsonc`
...
MongolGPT također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/mongolgpt/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\mongolgpt\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga MongolGPT mogao ponovo izgraditi.

1. Potpuno zatvorite MongolGPT Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/mongolgpt`
- **Linux**: obrišite `~/.cache/mongolgpt` (ili pokrenite `rm -rf ~/.cache/mongolgpt`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\mongolgpt`

3. Ponovo pokrenite MongolGPT Desktop.

---

### Rješavanje problema sa vezom na serveru

MongolGPT Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `mongolgpt.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `MONGOLGPT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `MONGOLGPT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, MongolGPT Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa MongolGPT karakteristikama.

---

### Obavještenja se ne prikazuju

MongolGPT Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za MongolGPT u postavkama vašeg OS-a, i
...
1. Zatvorite MongolGPT Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka MongolGPT Desktop aplikacije):

- `mongolgpt.settings.dat` (URL zadanog servera za desktop)
- `mongolgpt.global.dat` i `mongolgpt.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s MongolGPT:
...
Da saznate kojim modelima imate pristup, pokrenite `mongolgpt models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\mongolgpt`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. MongolGPT dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\mongolgpt`

2. Ponovo pokrenite MongolGPT da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti MongolGPT da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
MongolGPT će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
