...
Možete dodati vanjske alate u MongolGPT koristeći _Model Context Protocol_, ili MCP. MongolGPT podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [MongolGPT Config](https://mongolgpt.duckdns.org/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/mongolgpt`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
MongolGPT automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, MongolGPT će:
...
Ako server zahtijeva autentifikaciju, MongolGPT će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `mongolgpt mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, MongolGPT sigurno čuva tokene u `~/.local/share/mongolgpt/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u MongolGPT, zajedno s ugrađenim alatima. Možete njima upravljati kroz MongolGPT konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete MongolGPT sa vašim Sentry nalogom.
