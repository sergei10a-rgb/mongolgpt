---
title: Konfiguracija
description: Korištenje MongolGPT JSON konfiguracije.
---

Možete konfigurirati MongolGPT koristeći JSON konfiguracijski fajl.

---

## Format

MongolGPT podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/mongolgpt`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/mongolgpt/mongolgpt.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`MONGOLGPT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`mongolgpt.json` u projektu) - postavke specifične za projekat
5. **`.mongolgpt` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`MONGOLGPT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.mongolgpt` i `~/.config/mongolgpt` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/mongolgpt` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu MongolGPT konfiguraciju u `~/.config/mongolgpt/mongolgpt.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/mongolgpt/tui.json`.
...
Dodajte `mongolgpt.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se MongolGPT pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `MONGOLGPT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `MONGOLGPT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.mongolgpt` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.mongolgpt`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`mongolgpt.duckdns.org/config.json`**](https://mongolgpt.duckdns.org/config.json).

TUI konfiguracija koristi [**`mongolgpt.duckdns.org/tui.json`**](https://mongolgpt.duckdns.org/tui.json).
...
Koristite `MONGOLGPT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `mongolgpt.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `mongolgpt serve` i `mongolgpt web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš MongolGPT server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `mongolgpt.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj MongolGPT konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, MongolGPT pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/mongolgpt/agents/` ili `.mongolgpt/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, MongolGPT će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`mongolgpt run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/mongolgpt/commands/` ili `.mongolgpt/commands/`. [Saznajte više ovdje](/docs/commands).
...
MongolGPT će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, MongolGPT **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju MongolGPT sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.mongolgpt/plugins/` ili `~/.config/mongolgpt/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite MongolGPT da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
