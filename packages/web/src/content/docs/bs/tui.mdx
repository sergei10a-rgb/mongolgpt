---
title: TUI
description: Korištenje korisničkog interfejsa MongolGPT terminala.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MongolGPT pruža interaktivni terminalski interfejs ili TUI za rad na vašim projektima sa LLM.

Pokretanje MongolGPT pokreće TUI za trenutni direktorij.
...
Kada koristite MongolGPT TUI, možete upisati `/` nakon čega slijedi ime komande da biste brzo izvršili radnje. na primjer:
...
Dodajte provajdera u MongolGPT. Omogućava vam da odaberete između dostupnih provajdera i dodate njihove API ključeve.
...
Izađite iz MongolGPT. _Aliases_: `/quit`, `/q`
...
Ovo je odvojeno od `mongolgpt.json`, koji konfiguriše ponašanje servera/izvršavanja.
...
- `leader_timeout` - Kontroliše koliko dugo MongolGPT čeka nakon leader key. Podrazumijevano je `2000`.
...
Koristite `MONGOLGPT_TUI_CONFIG` da učitate prilagođenu putanju TUI konfiguracije.

### Attention

Attention omogućava da vas TUI obavijesti kada MongolGPT čeka odgovor, treba odobrenje dozvole, prijavi grešku sesije ili završi sesiju. Omogućite ga pomoću `attention.enabled`; ugrađeni događaji reproduciraju zvuk kada se dogode. Desktop obavještenja se šalju samo kada prozor terminala nije u fokusu i ne koriste se za subagent događaje.

- `enabled` - Omogućava sva obavještenja i zvukove za Attention. Podrazumijevano je `false`.
-
otifications` - Kada je Attention omogućen, dozvoljava TUI-ju da šalje desktop obavještenja putem terminala. Podrazumijevano je `true`.
...
- `sound_pack` - Sound pack ID koji se koristi. Podrazumijevano je `mongolgpt.default`.
