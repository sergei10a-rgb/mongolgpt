...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. MongolGPT dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u MongolGPT.
...
Ovaj alat omogucava LLM-u da pokrece terminalske komande kao
pm install`, `git status` i druge shell komande.
...
:::note
Ovaj alat je dostupan samo kada je `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (ili `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz MongolGPT provajdera ili kada je varijabla `MONGOLGPT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju MongolGPT:
...
Na primjer, ova `.ignore` datoteka dozvoljava ripgrep-u da pretrazuje
ode_modules/`, `dist/` i `build/` direktorije i kada su navedeni u `.gitignore`.
