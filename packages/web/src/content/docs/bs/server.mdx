---
title: Server
description: Komunicirajte s mongolgpt serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `mongolgpt serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji mongolgpt klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `mongolgpt.local` |
...
Postavite `MONGOLGPT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `mongolgpt`, ili postavite `MONGOLGPT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `mongolgpt serve` i za `mongolgpt web`.
...
Kada pokrenete `mongolgpt`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite mongolgpt server za programsku interakciju sa mongolgpt-om.
:::

Ova arhitektura omogucava mongolgpt podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `mongolgpt serve` da startate standalone server. Ako je mongolgpt TUI vec pokrenut, `mongolgpt serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste MongolGPT [IDE](/docs/ide) pluginovi.
...
mongolgpt server izlaže sljedece API-je.
