...
Agent skills omogucavaju MongolGPT da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
MongolGPT pretrazuje ove lokacije:

- Konfiguracija projekta: `.mongolgpt/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, MongolGPT ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.mongolgpt/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.mongolgpt/skills/git-release/SKILL.md` ovako:
...
MongolGPT navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `mongolgpt.json`:
...
**Za ugradene agente** (u `mongolgpt.json`):
...
2. Provjerite da frontmatter sadrzi
ame` i `description`
