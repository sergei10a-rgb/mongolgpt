---
title: Dodaci
description: Napišite vlastite dodatke za proširenje MongolGPT.
---

Dodaci vam omogućavaju da proširite MongolGPT spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja MongolGPT.
...
- `.mongolgpt/plugins/` - Dodaci na nivou projekta
- `~/.config/mongolgpt/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/mongolgpt/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/mongolgpt/mongolgpt.json`)
2. Konfiguracija projekta (`mongolgpt.json`)
3. Globalni direktorij dodataka (`~/.config/mongolgpt/plugins/`)
4. Direktorij dodataka projekta (`.mongolgpt/plugins/`)
...
MongolGPT pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: MongolGPT SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje MongolGPT.
...
Spriječite mongolgpt da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u mongolgpt:
...
Pomoćnik `tool` kreira prilagođeni alat koji mongolgpt može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za mongolgpt zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://mongolgpt.duckdns.org/docs/sdk) za detalje.
