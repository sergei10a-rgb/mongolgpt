---
title: LSP serveri
description: MongolGPT se integriše sa vašim LSP serverima.
---

MongolGPT se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

MongolGPT dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `MONGOLGPT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i mongolgpt otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj mongolgpt konfiguraciji.
