...
U MongolGPT mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, MongolGPT koristi nasu `mongolgpt` temu.
...
MongolGPT dolazi sa vise ugradenih tema.
...
|
ord`                 | Bazirana na [Nord](https://github.com/nordtheme/nord) temi                 |
...
- **Cuva terminalske defaulte**: Koristi
one` za boju teksta i pozadine da zadrzi izvorni izgled terminala.

System tema je za korisnike koji:

- Zele da MongolGPT odgovara izgledu njihovog terminala
...
MongolGPT podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/mongolgpt/themes/*.json` ili `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.mongolgpt/themes/*.json`
4. **Trenutni radni direktorij** - `./.mongolgpt/themes/*.json`
