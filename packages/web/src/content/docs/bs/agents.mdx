...
Postoje dvije vrste agenata u MongolGPT: primarni agenti i podagenti.
...
MongolGPT dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
MongolGPT dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

MongolGPT dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u MongolGPT-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `mongolgpt.json`:
...
- Globalno: `~/.config/mongolgpt/agents/`
- Po projektu: `.mongolgpt/agents/`
...
Ako temperatura nije navedena, MongolGPT koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu MongolGPT konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj MongolGPT konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [MongolGPT Zen](/docs/zen), koristili biste `mongolgpt/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `mongolgpt models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/sergei10a-rgb/mongolgpt).
:::
