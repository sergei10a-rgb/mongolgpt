---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj MongolGPT konfiguraciji.
...
u `~/.local/share/mongolgpt/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem MongolGPT
config.
...
MongolGPT Zen je lista modela koje je obezbedio MongolGPT tim koji su bili
testirano i potvrđeno da dobro radi sa MongolGPT. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa MongolGPT Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite mongolgpt i idite na [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/auth).
...
Radi kao i svaki drugi provajder u MongolGPT i potpuno je opcionalan za korištenje.

---

## MongolGPT Go

MongolGPT Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža MongolGPT tim i koji su testirani i verificirani da dobro rade s MongolGPT-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `MongolGPT Go` i idite na [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Da biste koristili Amazon Bedrock s MongolGPT:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete mongolgpt:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `mongolgpt.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u MongolGPT.
:::
...
Možete konfigurirati mongolgpt za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
-
pm` specificira paket koji se koristi za ovog provajdera. Ovdje se koristi `@ai-sdk/openai-compatible` za svaki OpenAI-kompatibilni API.
-
ame` je prikazano ime provajdera u interfejsu.
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi mongolgpt ispravno radio.
   :::
...
Također možete dodati modele kroz svoju mongolgpt konfiguraciju.
...
da pohrani token u mongolgpt auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
MongolGPT koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate MongolGPT
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`mongolgpt.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici MongolGPT GitLab auth adapter.

##### Konfiguracija

Prilagodite putem `mongolgpt.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s mongolgpt:
...
Za korištenje Google Vertex AI s MongolGPT:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće mongolgpt.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem mongolgpta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak local Helicone session plugin da automatski prijavite svaki MongolGPT razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki MongolGPT razgovor naveden kao zasebna sesija.
...
Možete konfigurirati mongolgpt za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
-
pm` specificira paket koji će se koristiti za ovog provajdera. Ovdje se `@ai-sdk/openai-compatible` koristi za bilo koji OpenAI kompatibilan API.
-
ame` je ime za prikaz za provajdera u korisničkom sučelju.
...
Možete konfigurirati mongolgpt za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati mongolgpt za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za MongolGPT. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/mongolgpt) za detalje.
:::
...
:::tip
Ako pozivi alata ne rade, pokušajte povećati
um_ctx` u Ollama. Počnite oko 16k - 32k.
:::
...
Da biste koristili Ollama Cloud s MongolGPT:
...
3. Kopirajte API ključ za korištenje u MongolGPT.
...
6. **Važno**: Prije upotrebe modela oblaka u MongolGPT, morate lokalno povući informacije o modelu:
...
MongolGPT Zen je lista testiranih i verifikovanih modela koju je obezbedio MongolGPT tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>MongolGPT Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **MongolGPT Zen**.

```txt
   /connect
```

3. Unesite svoj MongolGPT API ključ.
...
Također možete dodati dodatne modele putem vaše mongolgpt konfiguracije.
...
5. Također ih možete prilagoditi putem vaše mongolgpt konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa MongolGPTom:
...
Također možete prilagoditi modele kroz svoju mongolgpt konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s mongolgpt-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `mongolgpt.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju MongolGPT da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `mongolgpt auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite MongolGPT konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj mongolgpt konfiguraciji.
