---
title: Regeln
description: Lege eigene Anweisungen fuer mongolgpt fest.
---

Du kannst mongolgpt ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in mongolgpt den Befehl `/init` aus.
...
So kann mongolgpt sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht mongolgpt sie zu erweitern.
...
mongolgpt kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/mongolgpt/AGENTS.md` speichern.
Diese gelten in allen mongolgpt-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt MongolGPT dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/mongolgpt/AGENTS.md` existiert)
...
Beim Start sucht mongolgpt in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/mongolgpt/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `mongolgpt.json` oder in `~/.config/mongolgpt/mongolgpt.json` angeben.
...
mongolgpt parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit mongolgpt.json

Empfohlen ist das `instructions`-Feld in der `mongolgpt.json`:
...
Du kannst mongolgpt explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `mongolgpt.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
