---
title: Server
description: Nutze den mongolgpt-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `mongolgpt serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein mongolgpt-Client nutzen kann.
...
Setze `MONGOLGPT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `mongolgpt`, kann aber mit `MONGOLGPT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `mongolgpt serve` und `mongolgpt web`.
...
Wenn du `mongolgpt` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den mongolgpt-Server, um mongolgpt programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `mongolgpt serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `mongolgpt serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die MongolGPT-[IDE](/docs/ide)-Plugins.
...
Der mongolgpt-Server stellt folgende APIs bereit.
