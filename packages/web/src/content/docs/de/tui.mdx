---
title: TUI
description: Arbeite mit MongolGPT in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MongolGPT bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet MongolGPT die TUI im aktuellen Verzeichnis.
...
In der MongolGPT-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu MongolGPT hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet MongolGPT. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `mongolgpt.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange MongolGPT nach der leader key wartet. Standard ist `2000`.
...
Verwende `MONGOLGPT_TUI_CONFIG`, um einen benutzerdefinierten TUI-Konfigurationspfad zu laden.

### Attention

Attention lässt das TUI dich benachrichtigen, wenn MongolGPT auf eine Antwort wartet, eine Berechtigung bestätigt werden muss, ein Sitzungsfehler aufgetreten ist oder eine Sitzung abgeschlossen wurde. Aktiviere es mit `attention.enabled`; integrierte Ereignisse spielen beim Auftreten einen Sound ab. Desktop-Benachrichtigungen werden nur gesendet, wenn das Terminalfenster nicht fokussiert ist, und nicht für subagent-Ereignisse verwendet.

- `enabled` - Aktiviert alle Benachrichtigungen und Sounds von Attention. Standard ist `false`.
-
otifications` - Erlaubt dem TUI bei aktiviertem Attention, Desktop-Benachrichtigungen über das Terminal zu senden. Standard ist `true`.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `mongolgpt.default`.
