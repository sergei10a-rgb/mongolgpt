...
Wenn MongolGPT Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\mongolgpt\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `mongolgpt --log-level DEBUG`.

---

## Speicher

mongolgpt speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/mongolgpt/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\mongolgpt`
...
MongolGPT Desktop startet im Hintergrund einen lokalen MongolGPT-Server (`mongolgpt-cli`-Sidecar).
...
- Nur macOS: `MongolGPT`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/mongolgpt/mongolgpt.jsonc` (oder `~/.config/mongolgpt/mongolgpt.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/mongolgpt/mongolgpt.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\mongolgpt\mongolgpt.jsonc`
...
MongolGPT kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/mongolgpt/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\mongolgpt\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit MongolGPT ihn neu aufbauen kann.

1. Beende MongolGPT Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/mongolgpt`
- **Linux**: loesche `~/.cache/mongolgpt` (oder fuehre aus: `rm -rf ~/.cache/mongolgpt`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\mongolgpt`

3. Starte MongolGPT Desktop neu.

---

### Server-Verbindungsprobleme beheben

MongolGPT Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `mongolgpt.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `MONGOLGPT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `MONGOLGPT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt MongolGPT Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit MongolGPT-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

MongolGPT Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer MongolGPT in den OS-Einstellungen aktiviert sind, und
...
1. Beende MongolGPT Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von MongolGPT Desktop):

- `mongolgpt.settings.dat` (desktop default server URL)
- `mongolgpt.global.dat` und `mongolgpt.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit MongolGPT hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `mongolgpt models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\mongolgpt`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. mongolgpt installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\mongolgpt`

2. Starte mongolgpt neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt mongolgpt, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
mongolgpt erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
