---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer MongolGPT-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem MongolGPT anpassen
config.
...
MongolGPT Zen ist eine Liste von Modellen, die vom MongolGPT-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit MongolGPT funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit MongolGPT Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in MongolGPT und ist völlig optional.

---

## MongolGPT Go

MongolGPT Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom MongolGPT-Team getestet und verifiziert wurden, dass sie gut mit MongolGPT funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `MongolGPT Go` und gehen Sie zu [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
So verwenden Sie Amazon Bedrock mit MongolGPT:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie MongolGPT ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `mongolgpt.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in MongolGPT wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können mongolgpt so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
-
pm` gibt das für diesen Provider zu verwendende Paket an. Hier wird `@ai-sdk/openai-compatible` für jede OpenAI-kompatible API verwendet.
-
ame` ist der in der UI angezeigte Name des Providers.
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit MongolGPT ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre MongolGPT-Konfiguration hinzufügen.
...
um das Token im MongolGPT-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
MongolGPT verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von MongolGPT
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`mongolgpt.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der MongolGPT GitLab auth adapter-Homepage.

##### Konfiguration

Anpassen über `mongolgpt.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit MongolGPT:
...
So verwenden Sie Google Vertex AI mit MongolGPT:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie MongolGPT ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über MongolGPT konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das local Helicone session plugin-Plugin, um jede MongolGPT-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede MongolGPT-Konversation als separate Sitzung aufgeführt.
...
Sie können MongolGPT für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
-
pm` gibt das Paket an, das für diesen Anbieter verwendet werden soll. Hier wird `@ai-sdk/openai-compatible` für jeden OpenAI-kompatiblen API verwendet.
-
ame` ist der Anzeigename für den Anbieter im UI.
...
Sie können MongolGPT über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können MongolGPT über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für MongolGPT konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/mongolgpt).
:::
...
:::tip
Wenn Toolaufrufe nicht funktionieren, versuchen Sie,
um_ctx` in Ollama zu erhöhen. Beginnen Sie bei ca. 16.000 – 32.000 km.
:::
...
So verwenden Sie Ollama Cloud mit MongolGPT:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in MongolGPT.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in MongolGPT verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
MongolGPT Zen ist eine Liste getesteter und verifizierter Modelle, die vom MongolGPT-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>MongolGPT Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren MongolGPT API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre MongolGPT-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre MongolGPT-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit MongolGPT:
...
Sie können Modelle auch über Ihre MongolGPT-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit MongolGPT verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `mongolgpt.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann MongolGPT erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `mongolgpt auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die MongolGPT-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer MongolGPT-Konfiguration übereinstimmt.
