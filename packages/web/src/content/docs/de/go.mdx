...
MongolGPT Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in MongolGPT. Du abonnierst MongolGPT Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
MongolGPT zu verwenden.
...
MongolGPT Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

MongolGPT Go funktioniert wie jeder andere Provider in MongolGPT.

1. Du meldest dich bei **<a href={console}>MongolGPT Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `MongolGPT Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann MongolGPT Go abonnieren.
:::
...
MongolGPT Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner MongolGPT Config verwendet das Format `mongolgpt-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `mongolgpt-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben MongolGPT Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit MongolGPT zu nutzen.
