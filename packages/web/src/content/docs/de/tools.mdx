...
MongolGPT bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in MongolGPT verfuegbaren eingebauten Tools.
...
Damit kann das LLM Terminal-Befehle wie
pm install`, `git status` oder andere Shell-Kommandos ausfuehren.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (oder `MONGOLGPT_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der MongolGPT-Provider genutzt wird oder die Umgebungsvariable `MONGOLGPT_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von MongolGPT:
...
Dieses Beispiel erlaubt ripgrep, in
ode_modules/`, `dist/` und `build/` zu suchen, auch wenn sie in `.gitignore` stehen.
