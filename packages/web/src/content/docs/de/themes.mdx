...
Mit MongolGPT kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet MongolGPT das `mongolgpt`-Theme.
...
MongolGPT liefert mehrere eingebaute Themes mit.
...
one` fuer Text und Hintergruende

Das System-Theme ist ideal fuer Nutzer, die:

- MongolGPT optisch ans Terminal anpassen wollen
...
MongolGPT unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/mongolgpt/themes/*.json` oder `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.mongolgpt/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.mongolgpt/themes/*.json`
