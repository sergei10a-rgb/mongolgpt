---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um MongolGPT zu erweitern.
---

Mit Plugins können Sie MongolGPT erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von MongolGPT zu ändern.
...
- `.mongolgpt/plugins/` – Plugins auf Projektebene
- `~/.config/mongolgpt/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/mongolgpt/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/mongolgpt/mongolgpt.json`)
2. Projektkonfiguration (`mongolgpt.json`)
3. Globales Plugin-Verzeichnis (`~/.config/mongolgpt/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.mongolgpt/plugins/`)
...
MongolGPT führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein MongolGPT-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie MongolGPT erweitern können.
...
:::note
Wenn Sie die MongolGPT-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass MongolGPT `.env`-Dateien liest:
...
Plugins können MongolGPT auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das MongolGPT aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für MongolGPT zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://mongolgpt.duckdns.org/docs/sdk).
