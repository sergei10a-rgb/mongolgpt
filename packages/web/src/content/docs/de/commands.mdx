...
Erstellen Sie `.mongolgpt/commands/test.md`:
...
Sie können benutzerdefinierte Befehle über die MongolGPT-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem MongolGPT [config](/docs/config):
...
- Global: `~/.config/mongolgpt/commands/`
- Pro Projekt: `.mongolgpt/commands/`
...
MongolGPT enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
