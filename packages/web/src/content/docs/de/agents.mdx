...
Es gibt zwei Arten von Agenten in MongolGPT; Primäragenten und Subagenten.
...
MongolGPT verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `mongolgpt.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet MongolGPT modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale MongolGPT-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer MongolGPT-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [MongolGPT Zen](/docs/zen) verwenden, würden Sie `mongolgpt/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `mongolgpt models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/sergei10a-rgb/mongolgpt).
:::
