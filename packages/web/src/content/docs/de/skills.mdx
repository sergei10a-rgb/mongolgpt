...
Agent Skills erlauben MongolGPT, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
MongolGPT sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft MongolGPT vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.mongolgpt/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.mongolgpt/skills/git-release/SKILL.md` zum Beispiel so:
...
MongolGPT listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `mongolgpt.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `mongolgpt.json`):
...
2. Pruefe, ob Frontmatter
ame` und `description` enthaelt
