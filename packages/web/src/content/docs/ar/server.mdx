---
title: الخادم
description: تفاعل مع خادم mongolgpt عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `mongolgpt serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل mongolgpt استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `mongolgpt.local` |
...
عيّن `MONGOLGPT_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `mongolgpt`، أو عيّن `MONGOLGPT_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `mongolgpt serve` و `mongolgpt web`.
...
عند تشغيل `mongolgpt` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم mongolgpt للتفاعل مع mongolgpt برمجيا.
:::

تتيح هذه البنية لـ mongolgpt دعم عدة عملاء وتمكّنك من التفاعل مع mongolgpt برمجيا.

يمكنك تشغيل `mongolgpt serve` لبدء خادم مستقل. إذا كانت واجهة mongolgpt في terminal (TUI)
قيد التشغيل، فسيبدأ `mongolgpt serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات MongolGPT لـ [IDE](/docs/ide).
...
يعرض خادم mongolgpt واجهات API التالية.
