...
تتيح مهارات الوكيل لـ MongolGPT اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث MongolGPT في هذه المواقع:

- إعدادات المشروع: `.mongolgpt/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج MongolGPT صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.mongolgpt/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/mongolgpt/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.mongolgpt/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض MongolGPT المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `mongolgpt.json`:
...
**للوكلاء المدمجين** (في `mongolgpt.json`):
...
2. تحقّق من أن `frontmatter` تتضمن
ame` و `description`
