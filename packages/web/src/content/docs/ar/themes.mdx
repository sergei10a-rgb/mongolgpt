...
مع MongolGPT يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم MongolGPT سمتنا `mongolgpt`.
...
يأتي MongolGPT مع عدة سمات مدمجة.
...
|
ord`                 | مبني على سمة [Nord](https://github.com/nordtheme/nord)                      |
...
- **الحفاظ على افتراضيات terminal**: تستخدم
one` لألوان النص والخلفية للحفاظ على مظهر terminal الأصلي.

سمة النظام مناسبة للمستخدمين الذين:

- يريدون أن يطابق MongolGPT مظهر terminal لديهم
...
يدعم MongolGPT نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/mongolgpt/themes/*.json` أو `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.mongolgpt/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.mongolgpt/themes/*.json`
