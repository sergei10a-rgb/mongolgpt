...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في mongolgpt.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في mongolgpt مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.mongolgpt/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/mongolgpt/tools/`.
