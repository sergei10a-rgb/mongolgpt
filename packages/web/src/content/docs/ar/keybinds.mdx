...
يوفّر MongolGPT قائمة باختصارات لوحة المفاتيح يمكنك تخصيصها عبر إعدادات MongolGPT.
...
يستخدم MongolGPT مفتاح `leader` لمعظم اختصارات لوحة المفاتيح. يساعد ذلك على تجنّب التعارضات في terminal لديك.

افتراضيا، يكون `ctrl+x` هو مفتاح القائد، وتتطلّب معظم الإجراءات أن تضغط أولا مفتاح القائد ثم الاختصار. على سبيل المثال، لبدء جلسة جديدة اضغط `ctrl+x` أولا ثم اضغط
`.
...
يدعم حقل إدخال الموجّه في تطبيق MongolGPT لسطح المكتب اختصارات شائعة لتحرير النص على نمط Readline/Emacs. هذه الاختصارات مدمجة حاليا ولا يمكن تخصيصها عبر `mongolgpt.json`.
