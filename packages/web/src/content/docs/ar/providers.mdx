---
title: المزوّدون
description: استخدام أي موفّر LLM في MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم MongolGPT ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات MongolGPT.
...
في `~/.local/share/mongolgpt/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات MongolGPT.
...
MongolGPT Zen هي قائمة نماذج يوفّرها فريق MongolGPT وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع MongolGPT. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع MongolGPT Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر mongolgpt، ثم انتقل إلى [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/auth).
...
يعمل مثل أي مزوّد آخر في MongolGPT واستخدامه اختياري بالكامل.

---

## MongolGPT Go

MongolGPT Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق MongolGPT، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع MongolGPT.

1. شغّل الأمر `/connect` في TUI، واختر `MongolGPT Go`، ثم انتقل إلى [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
لاستخدام Amazon Bedrock مع MongolGPT:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل mongolgpt:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `mongolgpt.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل MongolGPT غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين mongolgpt لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
-
pm` يحدد الحزمة المستخدمة لهذا الموفر. هنا يُستخدم `@ai-sdk/openai-compatible` لأي واجهة برمجة تطبيقات متوافقة مع OpenAI.
-
ame` هو الاسم المعروض للموفر في الواجهة.
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل mongolgpt بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات mongolgpt.
...
تخزين الرمز في مساحة تخزين مصادقة mongolgpt.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم MongolGPT نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر MongolGPT
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`mongolgpt.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة MongolGPT GitLab auth adapter.

##### التهيئة

خصّص عبر `mongolgpt.json`:
...
لاستخدام اشتراك GitHub Copilot مع mongolgpt:
...
لاستخدام Google Vertex AI مع MongolGPT:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر mongolgpt، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق local Helicone session plugin لتسجيل كل محادثة MongolGPT تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة MongolGPT مدرجة كجلسة مستقلة.
...
يمكنك ضبط mongolgpt لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
-
pm` يحدد الحزمة المستخدمة لهذا المزوّد. هنا يتم استخدام `@ai-sdk/openai-compatible` لأي API متوافق مع OpenAI.
-
ame` هو اسم العرض للمزوّد في واجهة المستخدم.
...
يمكنك ضبط mongolgpt لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط mongolgpt لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع MongolGPT. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/mongolgpt) للتفاصيل.
:::
...
:::tip
إذا لم تكن tool calls تعمل، جرّب زيادة
um_ctx` في Ollama. ابدأ بنحو 16k - 32k.
:::
...
لاستخدام Ollama Cloud مع MongolGPT:
...
3. انسخ مفتاح API لاستخدامه في MongolGPT.
...
6. **مهم**: قبل استخدام نماذج السحابة في MongolGPT، يجب سحب معلومات النموذج محليا:
...
MongolGPT Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق MongolGPT. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>MongolGPT Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ MongolGPT.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات mongolgpt.
...
5. يمكنك أيضا تخصيصها عبر إعدادات mongolgpt. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع MongolGPT:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات mongolgpt. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع mongolgpt. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `mongolgpt.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ MongolGPT بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `mongolgpt auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات mongolgpt وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات mongolgpt.
