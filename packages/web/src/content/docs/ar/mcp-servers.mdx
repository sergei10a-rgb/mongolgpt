...
يمكنك إضافة أدوات خارجية إلى MongolGPT باستخدام _Model Context Protocol_، أو MCP. يدعم MongolGPT خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات MongolGPT](https://mongolgpt.duckdns.org/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/mongolgpt` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى MongolGPT تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم MongolGPT بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك MongolGPT المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `mongolgpt mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن MongolGPT الرموز بشكل آمن في `~/.local/share/mongolgpt/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل MongolGPT إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات MongolGPT مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط MongolGPT بحساب Sentry الخاص بك.
