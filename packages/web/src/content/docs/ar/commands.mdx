...
أنشئ الملف `.mongolgpt/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات MongolGPT أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ MongolGPT:
...
- على مستوى النظام: `~/.config/mongolgpt/commands/`
- لكل مشروع: `.mongolgpt/commands/`
...
يتضمن mongolgpt عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
