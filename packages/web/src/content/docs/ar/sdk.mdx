...
description: عميل JavaScript آمن الأنواع لخادم mongolgpt.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

توفر SDK الخاصة بـ mongolgpt لـ JS/TS عميلا آمنا للأنواع للتفاعل مع الخادم.
استخدمها لبناء التكاملات والتحكم في mongolgpt برمجيا.
...
| `port`     |
umber`      | منفذ الخادم                   | `4096`      |
| `signal`   | `AbortSignal` | إشارة إلغاء للإيقاف           | `undefined` |
| `timeout`  |
umber`      | مهلة بدء الخادم بالمللي ثانية | `5000`      |
...
يمكنك تمرير كائن إعدادات لتخصيص السلوك. سيستمر المثيل في التقاط `mongolgpt.json` لديك، لكن يمكنك تجاوز الإعدادات أو إضافة إعدادات مباشرة:
...
إذا كان لديك مثيل mongolgpt يعمل بالفعل، يمكنك إنشاء مثيل عميل للاتصال به:
...
| `retryCount` |
umber`        | اختياري. عدد محاولات إعادة التحقق (الافتراضي: 2) |
