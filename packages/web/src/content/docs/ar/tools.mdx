...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي MongolGPT مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في MongolGPT.
...
تتيح هذه الأداة لـ LLM تشغيل أوامر terminal مثل
pm install` و`git status` أو أي أمر shell آخر.
...
:::note
هذه الأداة متاحة فقط عند ضبط `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (أو `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود MongolGPT أو عند ضبط متغير البيئة `MONGOLGPT_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل MongolGPT:
...
على سبيل المثال، يسمح ملف `.ignore` هذا لـ ripgrep بالبحث داخل مجلدات
ode_modules/` و`dist/` و`build/` حتى لو كانت مدرجة في `.gitignore`.
