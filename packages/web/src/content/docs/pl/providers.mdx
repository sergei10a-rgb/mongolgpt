---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w mongolgpt.
---

import config from "../../../../config.mjs"
export const console = config.console

mongolgpt używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji mongolgpt.
...
w `~/.local/share/mongolgpt/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim mongolgpt
config.
...
MongolGPT Zen to lista modeli dostarczonych przez zespół mongolgpt, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z mongolgpt. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od MongolGPT Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `MongolGPT Zen` i przejdź do [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Działa jak każdy inny dostawca w mongolgpt i jest całkowicie opcjonalny w użyciu.

---

## MongolGPT Go

MongolGPT Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół mongolgpt, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z mongolgpt.

1. Uruchom polecenie `/connect` w TUI, wybierz `MongolGPT Go` i przejdź do [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Aby używać Amazon Bedrock z mongolgpt:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania mongolgpt:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `mongolgpt.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w mongolgpt nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować mongolgpt do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
-
pm` określa pakiet używany dla tego dostawcy. Tutaj `@ai-sdk/openai-compatible` jest używany dla każdego API zgodnego z OpenAI.
-
ame` to nazwa wyświetlana dostawcy w interfejsie.
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod mongolgpt działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji mongolgpt.
...
mongolgpt używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować mongolgpt
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`mongolgpt.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie MongolGPT GitLab auth adapter.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem mongolgpt:
...
Aby używać Google Vertex AI z mongolgpt:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania mongolgpt.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą mongolgpt, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki local Helicone session plugin, aby automatycznie rejestrować każdą konwersację mongolgpt jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja mongolgpt będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować mongolgpt tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
-
pm` określa pakiet, który ma być używany dla tego dostawcy. Tutaj `@ai-sdk/openai-compatible` jest używany dla dowolnego API zgodnego z OpenAI.
-
ame` to nazwa wyświetlana dostawcy w interfejsie użytkownika.
...
Możesz skonfigurować mongolgpt tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować mongolgpt tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem mongolgpt. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/mongolgpt).
:::
...
:::tip
Jeśli wywołania narzędzi nie działają, spróbuj zwiększyć
um_ctx` w Ollama. Zacznij około 16 tys. - 32 tys.
:::
...
Aby korzystać z Ollama Cloud z mongolgpt:
...
3. Skopiuj klucz API do użycia w mongolgpt.
...
6. **Ważne**: Przed użyciem modeli chmurowych w mongolgpt musisz pobrać informacje o modelu lokalnie:
...
MongolGPT Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół mongolgpt. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>MongolGPT Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API mongolgpt.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji mongolgpt.
...
5. Możesz je także dostosować za pomocą konfiguracji mongolgpt. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z mongolgpt:
...
Możesz także dostosować modele za pomocą konfiguracji mongolgpt. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z mongolgpt. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `mongolgpt.json` w katalogu projektu:
...
Pola `limit` pozwalają mongolgpt zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `mongolgpt auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację mongolgpt i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji mongolgpt.
