---
title: Serwery LSP
description: MongolGPT integruje się z serwerami LSP.
---

MongolGPT może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

MongolGPT posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `MONGOLGPT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i MongolGPT otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji MongolGPT.
