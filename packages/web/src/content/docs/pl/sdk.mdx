---
title: SDK
description: Typowany klient JS dla serwera MongolGPT.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Pakiet SDK JS/TS MongolGPT zapewnia klienta z bezpiecznym typowaniem (type-safe) do interakcji z serwerem.
Użyj go do budowania integracji i programowej kontroli MongolGPT.
...
| `port`     |
umber`      | Port serwera                              | `4096`      |
| `signal`   | `AbortSignal` | Sygnał przerwania w celu anulowania       | `undefined` |
| `timeout`  |
umber`      | Limit czasu w ms dla uruchomienia serwera | `5000`      |
...
Można przekazać obiekt konfiguracyjny, aby dostosować zachowanie. Instancja nadal pobiera `mongolgpt.json`, ale możesz zastąpić lub dodać konfigurację bezpośrednio:
...
Jeśli masz już działającą instancję MongolGPT, możesz utworzyć instancję klienta, aby się z nią połączyć:
...
| `retryCount` |
umber`        | Opcjonalne. Liczba ponownych prób walidacji (domyślnie: 2)   |
