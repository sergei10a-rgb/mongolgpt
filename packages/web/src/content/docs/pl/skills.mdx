...
Umiejętności agenta pozwalają mongolgpt odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
mongolgpt przeszukuje te lokalizacje:

- Project config: `.mongolgpt/skills/<name>/SKILL.md`
- Global config: `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu mongolgpt przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.mongolgpt/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.mongolgpt/skills/git-release/SKILL.md` w ten sposób:
...
mongolgpt wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `mongolgpt.json`:
...
**Dla agentów wbudowanych** (w `mongolgpt.json`):
...
2. Sprawdź, czy frontmatter zawiera
ame` i `description`
