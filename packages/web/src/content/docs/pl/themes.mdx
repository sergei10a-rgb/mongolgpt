...
Dzięki mongolgpt możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie mongolgpt używa naszego własnego motywu `mongolgpt`.
...
mongolgpt ma kilka wbudowanych motywów.
...
|
ord`                 | Na podstawie motywu [Nord](https://github.com/nordtheme/nord)                     |
...
- **Zachowuje ustawienia domyślne terminala**: Używa
one` dla kolorów tekstu i tła, aby zachować natywny wygląd terminala.

Motyw systemu przeznaczony jest dla użytkowników, którzy:

- Chcą, aby MongolGPT pasował do wyglądu ich terminala
...
mongolgpt obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/mongolgpt/themes/*.json` lub `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.mongolgpt/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.mongolgpt/themes/*.json`
