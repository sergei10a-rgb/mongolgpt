---
title: Serwer
description: Interakcja z serwerem mongolgpt za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `mongolgpt serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient mongolgpt.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `mongolgpt.local` |
...
Ustaw `MONGOLGPT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `mongolgpt` lub ustaw `MONGOLGPT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `mongolgpt serve`, jak i `mongolgpt web`.
...
Kiedy uruchomisz `mongolgpt`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera mongolgpt do programowej interakcji z mongolgpt.
:::

Ta architektura umożliwia obsługę wielu klientów przez mongolgpt i programową interakcję z kodem otwartym.

Możesz uruchomić `mongolgpt serve`, aby uruchomić samodzielny serwer. Jeśli masz
mongolgpt TUI działa, `mongolgpt serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki mongolgpt [IDE](/docs/ide).
...
Serwer mongolgpt udostępnia następujące interfejsy API.
