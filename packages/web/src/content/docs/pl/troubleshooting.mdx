...
Aby debugować problemy z mongolgpt, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\mongolgpt\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `mongolgpt --log-level DEBUG`.

---

## Przechowywanie danych

mongolgpt przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/mongolgpt/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\mongolgpt`
...
mongolgpt Desktop uruchamia w tle lokalny serwer mongolgpt (sidecar `mongolgpt-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `MongolGPT` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/mongolgpt/mongolgpt.jsonc` (lub `~/.config/mongolgpt/mongolgpt.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/mongolgpt/mongolgpt.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\mongolgpt\mongolgpt.jsonc`
...
mongolgpt może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/mongolgpt/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\mongolgpt\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby mongolgpt mógł ją odbudować.

1. Całkowicie zamknij mongolgpt Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/mongolgpt`
- **Linux**: usuń `~/.cache/mongolgpt` (lub uruchom `rm -rf ~/.cache/mongolgpt`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\mongolgpt`

3. Uruchom ponownie mongolgpt Desktop.

---

### Napraw problemy z połączeniem z serwerem

mongolgpt Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `mongolgpt.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `MONGOLGPT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `MONGOLGPT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows mongolgpt Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami mongolgpt.

---

### Brak powiadomień

mongolgpt Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla mongolgpt w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie mongolgpt Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji mongolgpt Desktop):

- `mongolgpt.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `mongolgpt.global.dat` i `mongolgpt.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z mongolgpt:
...
3. Upewnij się, że masz najnowszą wersję z `mongolgpt upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `mongolgpt models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\mongolgpt`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. mongolgpt dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\mongolgpt`

2. Uruchom ponownie kod mongolgpt, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to mongolgpt do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
mongolgpt wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
