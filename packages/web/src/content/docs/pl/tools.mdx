...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. MongolGPT posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w MongolGPT.
...
To narzędzie umożliwia LLM uruchamianie poleceń terminalowych, takich jak
pm install`, `git status` lub dowolne inne polecenie powłoki.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (lub `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy MongolGPT lub gdy zmienna środowiskowa `MONGOLGPT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu MongolGPT:
...
Na przykład ten plik `.ignore` pozwala ripgrepowi przeszukiwać katalogi
ode_modules/`, `dist/` i `build/`, nawet jeśli są one wymienione w `.gitignore`.
