---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć mongolgpt.
---

Wtyczki umożliwiające wyłączenie mongolgpt poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania mongolgpt.
...
- `.mongolgpt/plugins/` - Wtyczki na poziomie projektu
- `~/.config/mongolgpt/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/mongolgpt/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/mongolgpt/mongolgpt.json`)
2. Konfiguracja projektu (`mongolgpt.json`)
3. Globalny katalog wtyczek (`~/.config/mongolgpt/plugins/`)
4. Katalog wtyczek projektu (`.mongolgpt/plugins/`)
...
mongolgpt uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z mongolgpt do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do mongolgpt.
...
:::note
Jeśli korzystasz z aplikacji komputerowej mongolgpt, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj mongolgpt czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do mongolgpt:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod mongolgpt. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://mongolgpt.duckdns.org/docs/sdk).
