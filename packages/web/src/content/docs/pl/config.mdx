---
title: Konfiguracja
description: Ustawienia MongolGPT w pliku JSON.
---

Możesz dostosować MongolGPT za pomocą pliku konfiguracyjnego JSON.

---

## Format

MongolGPT obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "mongolgpt"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/mongolgpt`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/mongolgpt/mongolgpt.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `MONGOLGPT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`mongolgpt.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.mongolgpt`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `MONGOLGPT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.mongolgpt` i `~/.config/mongolgpt` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/mongolgpt`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację MongolGPT w `~/.config/mongolgpt/mongolgpt.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/mongolgpt/tui.json`.
...
Dodaj `mongolgpt.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy MongolGPT się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `MONGOLGPT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `MONGOLGPT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.mongolgpt` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.mongolgpt`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`mongolgpt.duckdns.org/config.json`**](https://mongolgpt.duckdns.org/config.json).
...
Użyj `MONGOLGPT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `mongolgpt.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `mongolgpt serve` i `mongolgpt web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer MongolGPT.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `mongolgpt.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie MongolGPT wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/mongolgpt/agents/` lub `.mongolgpt/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, MongolGPT powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`mongolgpt run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/mongolgpt/commands/` lub `.mongolgpt/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
MongolGPT może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy MongolGPT nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie MongolGPT **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają MongolGPT o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.mongolgpt/plugins/` lub `~/.config/mongolgpt/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie MongolGPT tylko do określonych dostawców.
