...
Możesz dodać dodatkowe informacje do mongolgpt za pomocą _Model Context Protocol_, w skrócie MCP. mongolgpt obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [mongolgpt Config](https://mongolgpt.duckdns.org/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/mongolgpt`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
mongolgpt automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, mongolgpt:
...
Jeśli serwer wymaga uwierzytelnienia, mongolgpt poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `mongolgpt mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji mongolgpt bezpieczne przechowa tokeny w `~/.local/share/mongolgpt/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w mongolgpt, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie mongolgpt, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć mongolgpt z kontem Sentry.
