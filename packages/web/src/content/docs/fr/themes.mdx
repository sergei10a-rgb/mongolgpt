...
Avec MongolGPT, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, MongolGPT utilise notre propre thème `mongolgpt`.
...
MongolGPT est livré avec plusieurs thèmes intégrés.
...
|
ord`                 | Basé sur le thème [Nord](https://github.com/nordtheme/nord)                 |
...
- **Préserve les paramètres par défaut du terminal** : utilise
one` pour les couleurs du texte et de l'arrière-plan afin de conserver l'apparence native de votre terminal.

Le thème système est destiné aux utilisateurs qui :

- Vous souhaitez que MongolGPT corresponde à l'apparence de leur terminal
...
MongolGPT prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/mongolgpt/themes/*.json` ou `$XDG_CONFIG_HOME/mongolgpt/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.mongolgpt/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.mongolgpt/themes/*.json`
