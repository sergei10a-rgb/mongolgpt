---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en MongolGPT.
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration MongolGPT.
...
en `~/.local/share/mongolgpt/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre MongolGPT
configuration.
...
MongolGPT Zen est une liste de modèles fournis par l'équipe MongolGPT qui ont été
testé et vérifié pour fonctionner correctement avec MongolGPT. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par MongolGPT Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez mongolgpt et dirigez-vous vers [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans MongolGPT et son utilisation est totalement facultative.

---

## MongolGPT Go

MongolGPT Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe MongolGPT qui ont été
testé et vérifié pour fonctionner correctement avec MongolGPT.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `MongolGPT Go` et rendez-vous sur [mongolgpt.duckdns.org/auth](https://mongolgpt.duckdns.org/zen).
...
Pour utiliser Amazon Bedrock avec MongolGPT :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de mongolgpt :
...
Pour une configuration spécifique au projet ou persistante, utilisez `mongolgpt.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans MongolGPT n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer mongolgpt pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
-
pm` spécifie le paquet à utiliser pour ce fournisseur. Ici, `@ai-sdk/openai-compatible` est utilisé pour toute API compatible OpenAI.
-
ame` est le nom du fournisseur affiché dans l'interface.
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que mongolgpt fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration mongolgpt.
...
pour stocker le jeton dans le stockage d'authentification mongolgpt.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
MongolGPT utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller MongolGPT
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `mongolgpt.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil MongolGPT GitLab auth adapter.

##### Configuration

Personnalisez via `mongolgpt.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec mongolgpt :
...
Pour utiliser Google Vertex AI avec MongolGPT :
...
Définissez-les lors de l'exécution de mongolgpt.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via mongolgpt, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin local Helicone session plugin pour enregistrer automatiquement chaque conversation MongolGPT en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation MongolGPT répertoriée comme une session distincte.
...
Vous pouvez configurer mongolgpt pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
-
pm` spécifie le package à utiliser pour ce fournisseur. Ici, `@ai-sdk/openai-compatible` est utilisé pour tout API compatible OpenAI.
-
ame` est le nom d'affichage du fournisseur dans l'interface utilisateur.
...
Vous pouvez configurer mongolgpt pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer mongolgpt pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour MongolGPT. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/mongolgpt) pour plus de détails.
:::
...
:::tip
Si les appels d'outils ne fonctionnent pas, essayez d'augmenter
um_ctx` dans Ollama. Commencez vers 16k - 32k.
:::
...
Pour utiliser Ollama Cloud avec MongolGPT :
...
3. Copiez la clé API à utiliser dans MongolGPT.
...
6. **Important** : Avant d'utiliser des modèles cloud dans MongolGPT, vous devez extraire les informations du modèle localement :
...
MongolGPT Zen est une liste de modèles testés et vérifiés fournie par l'équipe MongolGPT. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>MongolGPT Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **MongolGPT Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé MongolGPT API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration mongolgpt.
...
5. Vous pouvez également les personnaliser via votre configuration mongolgpt. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec MongolGPT :
...
Vous pouvez également personnaliser les modèles via votre configuration mongolgpt. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec mongolgpt. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `mongolgpt.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à MongolGPT de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `mongolgpt auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration mongolgpt et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration mongolgpt.
