---
title: Plugins
description: Écrivez vos propres plugins pour étendre MongolGPT.
---

Les plugins vous permettent d'étendre MongolGPT en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de MongolGPT.
...
- `.mongolgpt/plugins/` - Plugins au niveau du projet
- `~/.config/mongolgpt/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/mongolgpt/node_modules/`.
...
1. Configuration globale (`~/.config/mongolgpt/mongolgpt.json`)
2. Configuration du projet (`mongolgpt.json`)
3. Répertoire global des plugins (`~/.config/mongolgpt/plugins/`)
4. Répertoire des plugins du projet (`.mongolgpt/plugins/`)
...
MongolGPT exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK mongolgpt pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre mongolgpt.
...
:::note
Si vous utilisez l'application de bureau MongolGPT, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez mongolgpt de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à mongolgpt :
...
L'assistant `tool` crée un outil personnalisé que mongolgpt peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour mongolgpt aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://mongolgpt.duckdns.org/docs/sdk) pour plus de détails.
