...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. MongolGPT est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans MongolGPT.
...
Cet outil permet au LLM d'exécuter des commandes de terminal telles que
pm install`, `git status` ou toute autre commande shell.
...
:::note
Cet outil n'est disponible que lorsque `MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (ou `MONGOLGPT_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur MongolGPT ou lorsque la variable d'environnement `MONGOLGPT_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de MongolGPT :
...
Par exemple, ce fichier `.ignore` permet à ripgrep de rechercher dans les répertoires
ode_modules/`, `dist/` et `build/` même s'ils sont répertoriés dans `.gitignore`.
