...
Il existe deux types d'agents dans MongolGPT ; agents primaires et sous-agents.
...
MongolGPT est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
MongolGPT est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

MongolGPT est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `mongolgpt.json` :
...
Si aucune température n'est spécifiée, MongolGPT utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale MongolGPT et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration MongolGPT utilise le format `provider/model-id`. Par exemple, si vous utilisez [MongolGPT Zen](/docs/zen), vous utiliserez `mongolgpt/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `mongolgpt models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/sergei10a-rgb/mongolgpt).
:::
