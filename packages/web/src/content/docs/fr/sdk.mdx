---
title: SDK
description: Client JS de type sécurisé pour le serveur mongolgpt.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK mongolgpt JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler mongolgpt par programme.
...
Créez une instance de mongolgpt :
...
| `port`     |
umber`      | Port du serveur                                    | `4096`      |
| `signal`   | `AbortSignal` | Signal d'abandon pour annulation                   | `undefined` |
| `timeout`  |
umber`      | Délai d'attente en ms pour le démarrage du serveur | `5000`      |
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `mongolgpt.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
Si vous disposez déjà d'une instance en cours d'exécution de mongolgpt, vous pouvez créer une instance client pour vous y connecter :
...
| `retryCount` |
umber`        | Facultatif. Nombre de tentatives de validation (par défaut : 2) |
