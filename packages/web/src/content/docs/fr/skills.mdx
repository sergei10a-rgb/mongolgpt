...
Les compétences d'agent permettent à MongolGPT de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
MongolGPT recherche ces emplacements :

- Configuration du projet : `.mongolgpt/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/mongolgpt/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, MongolGPT remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.mongolgpt/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/mongolgpt/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.mongolgpt/skills/git-release/SKILL.md` comme ceci :
...
MongolGPT répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `mongolgpt.json` :
...
**Pour les agents intégrés** (dans `mongolgpt.json`) :
...
2. Vérifiez que le frontmatter inclut
ame` et `description`
