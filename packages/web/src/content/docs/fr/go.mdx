...
MongolGPT Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

Go fonctionne comme n'importe quel autre fournisseur dans MongolGPT. Vous vous abonnez à MongolGPT Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser MongolGPT.
...
MongolGPT Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

MongolGPT Go fonctionne comme n'importe quel autre fournisseur dans MongolGPT.

1. Vous vous connectez à **<a href={console}>MongolGPT Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `MongolGPT Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à MongolGPT Go.
:::
...
MongolGPT Go inclut les limites suivantes :
...
| MiniMax M3        | minimax-m3        | `https://mongolgpt.duckdns.org/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| MiniMax M2.7      | minimax-m2.7      | `https://mongolgpt.duckdns.org/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| MiniMax M2.5      | minimax-m2.5      | `https://mongolgpt.duckdns.org/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| Qwen3.7 Max       | qwen3.7-max       | `https://mongolgpt.duckdns.org/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| Qwen3.7 Plus      | qwen3.7-plus      | `https://mongolgpt.duckdns.org/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| Qwen3.6 Plus      | qwen3.6-plus      | `https://mongolgpt.duckdns.org/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |

L'[ID de modèle](/docs/config/#models) dans votre configuration MongolGPT utilise le format `mongolgpt-go/<model-id>`. Par exemple, pour Kimi K2.7 Code, vous utiliseriez `mongolgpt-go/kimi-k2.7-code` dans votre configuration.
...
Nous avons créé MongolGPT Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec MongolGPT également.
