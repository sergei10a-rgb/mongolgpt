---
title: プロバイダー
description: MongolGPT で LLM プロバイダーを使用する。
---

import config from "../../../../config.mjs"
export const console = config.console

MongolGPT は [AI SDK](https://ai-sdk.dev/) および [Models.dev](https://models.dev) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。

プロバイダーを追加するには、次のことを行う必要があります。

1. `/connect` コマンドを使用して、プロバイダーの API キーを追加します。
2. MongolGPT 設定でプロバイダーを構成します。
...
`~/.local/share/mongolgpt/auth.json`で。

---

### 設定

MongolGPT の `provider` セクションを通じてプロバイダーをカスタマイズできます。
config.
...
MongolGPT Zen は、MongolGPT チームが提供するモデルのリストです。
MongolGPT で適切に動作することがテストおよび検証されています。 [詳細はこちら](/docs/zen)。

:::tip
初めての方は、MongolGPT Zen から始めることをお勧めします。
:::

1. TUI で `/connect` コマンドを実行し、`MongolGPT Zen` を選択して、[mongolgpt.duckdns.org/zen](https://mongolgpt.duckdns.org/zen) にアクセスします。
...
これは MongolGPT の他のプロバイダーと同様に機能し、使用は完全にオプションです。

---

## MongolGPT Go

MongolGPT Go は、MongolGPT チームによって提供される、人気のあるオープンコーディングモデルへの信頼性の高いアクセスを提供する低コストのサブスクリプションプランです。これらは MongolGPT でうまく機能することがテストおよび検証されています。

1. TUI で `/connect` コマンドを実行し、`MongolGPT Go` を選択して、[mongolgpt.duckdns.org/zen](https://mongolgpt.duckdns.org/zen) にアクセスします。
...
MongolGPT で Amazon Bedrock を使用するには:
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   mongolgpt の実行中に次の環境変数のいずれかを設定します。
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`mongolgpt.json` を使用します。
...
:::info
MongolGPT での Claude Pro/Max サブスクリプションの使用は、[Anthropic](https://anthropic.com) によって正式に許可されていませんが、一般的に機能します。
:::
...
mongolgpt は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://atomic.chat) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `http://127.0.0.1:1337/v1`）。
...
-
pm` はこのプロバイダーに使用するパッケージを指定します。ここでは、任意の OpenAI 互換 API に対して `@ai-sdk/openai-compatible` を使用しています。
-
ame` は UI に表示されるプロバイダー名です。
...
:::note
MongolGPT が正しく動作するには、デプロイメント名がモデル名と一致する必要があります。
:::
...
MongolGPT 設定を通じてモデルを追加することもできます。
...
トークンを MongolGPT 認証ストレージに保存します。
:::

##### 自己ホスト型 GitLab

:::note[コンプライアンスメモ]
MongolGPT は、セッションタイトルの生成などの一部の AI タスクに小規模なモデルを使用します。
デフォルトでは、Zen によってホストされる gpt-5-nano を使用するように構成されています。 MongolGPT をロックするには
自分の GitLab でホストされているインスタンスのみを使用するには、次の行を
`mongolgpt.json` ファイル。セッション共有を無効にすることもお勧めします。
...
詳細については、MongolGPT GitLab auth adapter ホームページ。

##### 設定

`mongolgpt.json` を通じてカスタマイズします。
...
GitHub Copilot サブスクリプションを MongolGPT で使用するには:
...
MongolGPT で Google Vertex AI を使用するには:
...
MongolGPT を通じて自動的に構成されていない Helicone の機能またはモデルを見つけた場合は、いつでも自分で構成できます。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 local Helicone session plugin プラグインを使用して、各 MongolGPT 会話を Helicone のセッションとして自動的に記録します。
...
プラグインは、リクエストに `Helicone-Session-Id` ヘッダーと `Helicone-Session-Name` ヘッダーを挿入します。 Helicone のセッションページでは、MongolGPT の各会話が個別のセッションとしてリストされています。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように mongolgpt を構成できます。
...
-
pm` は、このプロバイダーに使用するパッケージを指定します。ここで、`@ai-sdk/openai-compatible` は OpenAI 互換 API に使用されます。
-
ame` は、UI でのプロバイダーの表示名です。
...
LM Studio を通じてローカルモデルを使用するように mongolgpt を構成できます。
...
Ollama を通じてローカルモデルを使用するように mongolgpt を構成できます。

:::tip
Ollama は MongolGPT 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/mongolgpt)」を参照してください。
:::
...
:::tip
ツール呼び出しが機能しない場合は、Ollama の
um_ctx` を増やしてみてください。 16k〜32kあたりから始めてください。
:::
...
6. **重要**: MongolGPT でクラウドモデルを使用する前に、モデル情報をローカルに取得する必要があります。
...
MongolGPT Zen は、MongolGPT チームによって提供される、テストおよび検証されたモデルのリストです。 [詳細はこちら](/docs/zen)。

1. **<a href={console}>MongolGPT Zen</a>** にサインインし、**API キーの作成** をクリックします。

2. `/connect` コマンドを実行し、**MongolGPT Zen** を検索します。

   ```txt
   /connect
   ```

3. MongolGPT API キーを入力します。
...
MongolGPT 設定を通じて追加のモデルを追加することもできます。
...
5. MongolGPT 設定を通じてカスタマイズすることもできます。プロバイダーを指定する例を次に示します。
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を MongolGPT で使用するには:
...
MongolGPT 設定を通じてモデルをカスタマイズすることもできます。プロバイダーのルーティング順序を指定する例を次に示します。
...
:::tip
OpenAI と互換性のある任意のプロバイダーを MongolGPT で使用できます。最新の AI プロバイダーのほとんどは、OpenAI 互換の API を提供しています。
:::
...
4. プロジェクトディレクトリで `mongolgpt.json` ファイルを作成または更新します。
...
`limit` フィールドを使用すると、MongolGPT はコンテキストがどのくらい残っているかを理解できます。標準プロバイダーは、これらを models.dev から自動的に取得します。
...
1. **認証設定を確認します**: `mongolgpt auth list` を実行して、資格情報が正しいかどうかを確認します。
   プロバイダー用のものが設定に追加されます。

これは、認証に環境変数に依存する Amazon Bedrock のようなプロバイダーには当てはまりません。

2. カスタムプロバイダーの場合は、MongolGPT 設定を確認し、次のことを行います。
   - `/connect` コマンドで使用されるプロバイダー ID が、MongolGPT 設定内の ID と一致することを確認してください。
