---
title: サーバー
description: HTTP 経由で MongolGPT サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`mongolgpt serve` コマンドは、MongolGPT クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `mongolgpt.local` |
...
HTTP 基本認証でサーバーを保護するには、`MONGOLGPT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `mongolgpt` になるか、`MONGOLGPT_SERVER_USERNAME` を設定してオーバーライドします。これは、`mongolgpt serve` と `mongolgpt web` の両方に当てはまります。
...
`mongolgpt` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
MongolGPT サーバーを使用して、プログラムで MongolGPT と対話します。
:::

`mongolgpt serve` を実行してスタンドアロンサーバーを起動できます。すでに MongolGPT TUI を実行している場合でも、`mongolgpt serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、MongolGPT [IDE](/docs/ide) プラグインによって使用されます。
