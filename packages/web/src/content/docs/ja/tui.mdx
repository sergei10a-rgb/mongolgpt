---
title: TUI
description: MongolGPT ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MongolGPT は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

MongolGPT を実行すると、現在のディレクトリの TUI が開始されます。
...
MongolGPT TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
MongolGPT にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
MongolGPT を終了します。 _エイリアス_: `/quit`、`/q`
...
これは、サーバー/ランタイムの動作を構成する `mongolgpt.json` とは別です。
...
- `leader_timeout` - leader key を押したあと、MongolGPT が次のキー入力を待つ時間を制御します。デフォルトは `2000` です。
...
カスタム TUI 設定パスをロードするには、`MONGOLGPT_TUI_CONFIG` を使用します。

### Attention

Attention を有効にすると、MongolGPT が回答待ち、権限の承認待ち、セッションエラー、またはセッション完了を知らせたいときに、TUI がサウンドやデスクトップ通知で知らせます。`attention.enabled` で有効化します。組み込みイベントは発生時にサウンドを再生します。デスクトップ通知はターミナルウィンドウにフォーカスがない場合にのみ送信され、subagent イベントには使われません。

- `enabled` - Attention のすべての通知とサウンドを有効にします。デフォルトは `false` です。
-
otifications` - Attention が有効なとき、TUI がターミナル経由でデスクトップ通知を送信できるようにします。デフォルトは `true` です。
- `sound` - Attention が有効なとき、通知音の再生を許可します。デフォルトは `true` です。
- `volume` - 通知音のデフォルト音量を `0` から `1` で指定します。デフォルトは `0.4` です。
- `sound_pack` - 使用する sound pack ID です。デフォルトは `mongolgpt.default` です。
