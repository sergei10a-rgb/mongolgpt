---
title: プラグイン
description: MongolGPT を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで MongolGPT を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、MongolGPT のデフォルトの動作を変更したりできます。
...
- `.mongolgpt/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/mongolgpt/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/mongolgpt/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/mongolgpt/mongolgpt.json`)
2. プロジェクト設定 (`mongolgpt.json`)
3. グローバルプラグインディレクトリ (`~/.config/mongolgpt/plugins/`)
4. プロジェクトプラグインディレクトリ (`.mongolgpt/plugins/`)
...
MongolGPT は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための MongolGPT SDK クライアント。
...
ここでは、MongolGPT を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://mongolgpt.duckdns.org/docs/sdk) を参照してください。
...
MongolGPT が `.env` ファイルを読み取らないようにします。
...
プラグインは MongolGPT にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、MongolGPT が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して MongolGPT で使用できます。
