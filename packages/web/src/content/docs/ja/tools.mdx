...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 MongolGPT には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
MongolGPT で利用可能なすべての組み込みツールを次に示します。
...
このツールを使用すると、LLM は
pm install`、`git status`、またはその他のシェルコマンドなどのターミナルコマンドを実行できます。
...
:::note
このツールは、`MONGOLGPT_EXPERIMENTAL_LSP_TOOL=true` (または `MONGOLGPT_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、MongolGPT プロバイダーを使用している場合、または `MONGOLGPT_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

MongolGPT の起動時に有効にするには:
...
たとえば、この `.ignore` ファイルを使用すると、ripgrep は、
ode_modules/` にリストされている場合でも、`dist/`、`build/`、および `.gitignore` ディレクトリ内を検索できます。
