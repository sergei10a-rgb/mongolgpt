...
> current private `@mongolgpt/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@mongolgpt/ai`
...
- Secondary audience: MongolGPT and other durable agent runtimes
...
import { LLM } from "@mongolgpt/ai"
import { OpenAI } from "@mongolgpt/ai/providers/openai"
...
```ts
import { Message } from "@mongolgpt/ai"
...
import { LLM, Tool } from "@mongolgpt/ai"
...
MongolGPT and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@mongolgpt/ai/promise"
import { OpenAI } from "@mongolgpt/ai/providers/openai"
...
```ts
import { LLMSchema } from "@mongolgpt/ai/schema"
...
```ts
import { Provider, Protocol } from "@mongolgpt/ai/provider"
...
@mongolgpt/ai/promise
  Promise/AsyncIterable LLM facade

@mongolgpt/ai/schema
  serializable domain schemas

@mongolgpt/ai/provider
  experimental Provider and Protocol authoring APIs

@mongolgpt/ai/providers/openai
@mongolgpt/ai/providers/anthropic
@mongolgpt/ai/providers/google
...
```
...
MongolGPT should migrate to `generateTurn` / `streamTurn`, preserving its durable
