...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-mongolgpt-directory", "directory"],
    ["x-mongolgpt-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-mongolgpt-directory")
  next.headers.delete("x-mongolgpt-workspace")
  return next
}

export function createMongolGPTClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-mongolgpt-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-mongolgpt-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of MongolGPT Server (server responded with text/html)")
