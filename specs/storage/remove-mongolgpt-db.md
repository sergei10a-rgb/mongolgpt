# Remove `packages/mongolgpt/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/mongolgpt/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the mongolgpt legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/mongolgpt/src/storage/db.ts` are concentrated in 22 source files:

- `packages/mongolgpt/src/account/repo.ts`
- `packages/mongolgpt/src/cli/cmd/db.ts`
- `packages/mongolgpt/src/cli/cmd/import.ts`
- `packages/mongolgpt/src/cli/cmd/stats.ts`
- `packages/mongolgpt/src/control-plane/workspace.ts`
- `packages/mongolgpt/src/index.ts`
- `packages/mongolgpt/src/node.ts`
- `packages/mongolgpt/src/permission/index.ts`
- `packages/mongolgpt/src/project/project.ts`
- `packages/mongolgpt/src/server/projectors.ts`
- `packages/mongolgpt/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/mongolgpt/src/server/shared/fence.ts`
- `packages/mongolgpt/src/session/message-v2.ts`
- `packages/mongolgpt/src/session/projectors.ts`
- `packages/mongolgpt/src/session/prompt.ts`
- `packages/mongolgpt/src/session/session.ts`
- `packages/mongolgpt/src/session/todo.ts`
- `packages/mongolgpt/src/share/share-next.ts`
- `packages/mongolgpt/src/storage/db.ts`
- `packages/mongolgpt/src/sync/index.ts`
- `packages/mongolgpt/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy mongolgpt database wrapper; `packages/mongolgpt/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the mongolgpt wrapper.
...
Status: Completed. `SyncEvent` and the mongolgpt projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy mongolgpt database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy mongolgpt database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy mongolgpt database wrapper.
...
All migration groups are complete or superseded. `packages/mongolgpt/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No mongolgpt data-migration group remains.

The previous mongolgpt `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate mongolgpt data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/mongolgpt`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/mongolgpt/src`
- `bun typecheck` from `packages/mongolgpt`
- Relevant package tests from `packages/mongolgpt`, not the repo root
