...
Move the canonical MongolGPT terminal application from
`packages/mongolgpt/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/mongolgpt`,
`packages/cli`, or `@mongolgpt/core`.

The SDK is the TUI's MongolGPT boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/mongolgpt`.
- Do not replace private `packages/mongolgpt` imports with `@mongolgpt/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@mongolgpt/tui`.
...
- MongolGPT domain data displayed by the TUI
...
```text
packages/mongolgpt/src/cli/cmd/tui
```

Its private dependency on `packages/mongolgpt` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/mongolgpt/src/*`.
...
Status: Completed. The private `@mongolgpt/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@mongolgpt/tui`.
...
- The package has no dependency on `mongolgpt`, `@mongolgpt/cli`, or
  `@mongolgpt/core`.
...
primitives now live in `@mongolgpt/tui`. The legacy host consumes explicit
...
and build inputs through `@mongolgpt/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads MongolGPT globals or process state;
...
Replace process-global MongolGPT state with resolved TUI inputs.
...
`@mongolgpt/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/mongolgpt` initially.
...
canonical in `@mongolgpt/tui`. Configured references resolve through the new
...
adapters rather than MongolGPT domain implementations.

Make the SDK the only MongolGPT domain boundary used by the TUI.
...
- Domain-facing TUI code imports MongolGPT data and operations only from
  `@mongolgpt/sdk`.
...
`@mongolgpt/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/mongolgpt`.
...
- The package has no imports from `packages/mongolgpt`, `packages/cli`, or
  `@mongolgpt/core`.
...
public `@mongolgpt/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/mongolgpt`.
- Keep the legacy embedded worker and server startup in `packages/mongolgpt`.
- Change those adapters to load config, create transport inputs, and call the
  public `@mongolgpt/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/mongolgpt`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@mongolgpt/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@mongolgpt/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for MongolGPT domain state.
...
cd packages/mongolgpt && bun typecheck
...
rg '@mongolgpt/core|packages/mongolgpt|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/mongolgpt/src packages/mongolgpt/test
```
...
- Build the current-platform `packages/mongolgpt` binary.
