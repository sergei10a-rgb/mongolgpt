...
Both `action` and `resource` use MongolGPT's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.mongolgpt` files is intentionally deferred until `.mongolgpt` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether mongolgpt permits an operation through its managed execution paths.
