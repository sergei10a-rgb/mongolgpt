...
The mongolgpt server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@mongolgpt/llm` tool loop after replacing its
...
- plugins get global "mongolgpt" instance like in that post i showed
- mongolgpt instance has stuff like `mongolgpt.session.prompt()` or
  `mongolgpt.tool.register({...})`
...
- decide whether to preserve deprecated `@mongolgpt/llm` orchestration exports
